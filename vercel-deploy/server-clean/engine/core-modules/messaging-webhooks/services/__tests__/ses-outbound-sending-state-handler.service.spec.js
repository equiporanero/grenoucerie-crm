...
describe('SesOutboundSendingStateHandlerService.handle', ()=>{
    const setUp = ()=>{
        ...
        const service = new _sesoutboundsendingstatehandlerservice.SesOutboundSendingStateHandlerService(emailingDomainTenantStatusService);
        return {
            service,
    ...
    describe.each([
        ...
        it.each([
            {
                detailType: 'Sending Status Disabled',
                expected: _emailingdomaintenantstatustype.EmailingDomainTenantStatus.PAUSED
            },
            {
                detailType: 'Sending Status Enabled',
                expected: _emailingdomaintenantstatustype.EmailingDomainTenantStatus.ACTIVE
            }
        ])('mirrors "$detailType" to tenantStatus=$expected', async ({ detailType, expected })=>{
            ...
            expect(emailingDomainTenantStatusService.setTenantStatusForWorkspace).toHaveBeenCalledWith('ws1', expected);
        });
    });
    ...
    it('does not update any workspace when the ARN does not carry the twenty-managed prefix', async ()=>{
        ...
        expect(emailingDomainTenantStatusService.setTenantStatusForWorkspace).not.toHaveBeenCalled();
    });
});

//# sourceMappingURL=ses-outbound-sending-state-handler.service.spec.js.map