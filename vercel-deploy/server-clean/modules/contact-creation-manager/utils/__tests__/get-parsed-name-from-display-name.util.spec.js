...
describe('getParsedNameFromDisplayName', ()=>{
    describe('common shapes', ()=>{
        ...
        test.each(testCases)('$title', ({ context: { displayName, expected } })=>{
            expect((0, _getparsednamefromdisplaynameutil.getParsedNameFromDisplayName)(displayName)).toEqual(expected);
        });
    });
    describe('comma-inverted form "Last, First"', ()=>{
        const testCases = [
            ...
            {
                title: 'should strip surrounding RFC 5322 quoted-string quotes before swapping',
                context: {
                    displayName: '"Doe, John"',
                    expected: {
                        firstName: 'John',
    ...
    describe('email-derived display names', ()=>{
        const testCases = [
            ...
            {
                title: 'should de-duplicate "First.Last Last" forms synthesized by some clients',
                context: {
            ...
            {
                title: 'should expand "First.Middle Last" forms into a multi-word last name',
                context: {
    ...
    describe('mail-server ":GROUP" tag suffix', ()=>{
        const testCases = [
            ...
            {
                title: 'should strip the tag from the first token in a space-separated form',
                context: {
    ...
    describe('fallback cases (empty result)', ()=>{
        const testCases = [
            {
                title: 'should return empty names so the caller can fall back to handle parsing when the display name is empty',
                context: {
                    displayName: '',
                    expected: {
                        firstName: '',
            ...
            {
                title: 'should return empty names when the display name is only whitespace',
                context: {
                    displayName: '   ',
                    expected: {
                        firstName: '',
            ...
            {
                title: 'should return empty names when forwarders inject an email address into the display-name slot',
                context: {
...
//# sourceMappingURL=get-parsed-name-from-display-name.util.spec.js.map