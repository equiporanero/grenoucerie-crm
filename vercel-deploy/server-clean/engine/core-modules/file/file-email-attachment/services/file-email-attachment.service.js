...
Object.defineProperty(exports, "FileEmailAttachmentService", {
    enumerable: true,
    get: function() {
        return FileEmailAttachmentService;
    }
});
...
let FileEmailAttachmentService = class FileEmailAttachmentService {
    async uploadFile({ file, filename, workspaceId }) {
        ...
        const savedFile = await this.fileStorageService.writeFile({
            sourceFile: file,
            resourcePath: name,
            fileFolder: _types.FileFolder.EmailAttachment,
    ...
    async deleteFiles({ fileIds, workspaceId }) {
        for (const fileId of fileIds){
            try {
                ...
                this.logger.warn(`Failed to delete email attachment file ${fileId}: ${error}`);
            }
        }
    }
    constructor(fileStorageService, applicationService, fileUrlService){
        ...
        this.logger = new _common.Logger(FileEmailAttachmentService.name);
    }
};
FileEmailAttachmentService = _ts_decorate([
...
], FileEmailAttachmentService);

//# sourceMappingURL=file-email-attachment.service.js.map