...
const _parseapplicationloglines = require("./parse-application-log-lines");
describe('parseApplicationLogLines', ()=>{
    it('should return an empty array for empty string', ()=>{
        expect((0, _parseapplicationloglines.parseApplicationLogLines)('')).toEqual([]);
    });
    it('should return an empty array for undefined-ish input', ()=>{
        expect((0, _parseapplicationloglines.parseApplicationLogLines)(undefined)).toEqual([]);
    });
    it('should parse a structured INFO log line', ()=>{
        const raw = '2024-06-15T10:30:00.123Z INFO Hello world';
        expect((0, _parseapplicationloglines.parseApplicationLogLines)(raw)).toEqual([
            {
    ...
    it('should default unstructured lines to INFO with current timestamp', ()=>{
        ...
        const result = (0, _parseapplicationloglines.parseApplicationLogLines)(raw);
        expect(result).toHaveLength(1);
        expect(result[0].level).toBe('INFO');
        expect(result[0].message).toBe('some plain text without timestamp or level');
...
//# sourceMappingURL=parse-application-log-lines.spec.js.map