class u extends Error {
  constructor(t, n) {
    let i = Array.isArray(t) ? t.map((a) => a?.message || "").join(`
`) : "";
    i || (i = "GraphQL error"), super(i), this.errors = [], this.errors = t, this.data = n;
  }
}
function A(e, t) {
  let n = t.map((i) => i.request);
  n.length === 1 && (n = n[0]), e.fetcher(n).then((i) => {
    if (t.length === 1 && !Array.isArray(i)) {
      if (i.errors && i.errors.length) {
        t[0].reject(
          new u(i.errors, i.data)
        );
        return;
      }
      t[0].resolve(i);
      return;
    } else if (i.length !== t.length)
      throw new Error("response length did not match query length");
    for (let a = 0; a < t.length; a++)
      i[a].errors && i[a].errors.length ? t[a].reject(
        new u(i[a].errors, i[a].data)
      ) : t[a].resolve(i[a]);
  });
}
function y(e, t) {
  const n = e._queue, i = t.maxBatchSize || 0;
  if (e._queue = [], i > 0 && i < n.length)
    for (let a = 0; a < n.length / i; a++)
      A(
        e,
        n.slice(a * i, (a + 1) * i)
      );
  else
    A(e, n);
}
class I {
  constructor(t, {
    batchInterval: n = 6,
    shouldBatch: i = !0,
    maxBatchSize: a = 0
  } = {}) {
    this.fetcher = t, this._options = {
      batchInterval: n,
      shouldBatch: i,
      maxBatchSize: a
    }, this._queue = [];
  }
  /**
   * Fetch will send a graphql request and return the parsed json.
   * @param {string}      query          - the graphql query.
   * @param {Variables}   variables      - any variables you wish to inject as key/value pairs.
   * @param {[string]}    operationName  - the graphql operationName.
   * @param {Options}     overrides      - the client options overrides.
   *
   * @return {promise} resolves to parsed json of server response
   *
   * @example
   * client.fetch(`
   *    query getHuman($id: ID!) {
   *      human(id: $id) {
   *        name
   *        height
   *      }
   *    }
   * `, { id: "1001" }, 'getHuman')
   *    .then(human => {
   *      // do something with human
   *      console.log(human);
   *    });
   */
  fetch(t, n, i, a = {}) {
    const p = {
      query: t
    }, o = Object.assign({}, this._options, a);
    return n && (p.variables = n), i && (p.operationName = i), new Promise((r, s) => {
      this._queue.push({
        request: p,
        resolve: r,
        reject: s
      }), this._queue.length === 1 && (o.shouldBatch ? setTimeout(
        () => y(this, o),
        o.batchInterval
      ) : y(this, o));
    });
  }
  /**
   * Fetch will send a graphql request and return the parsed json.
   * @param {string}      query          - the graphql query.
   * @param {Variables}   variables      - any variables you wish to inject as key/value pairs.
   * @param {[string]}    operationName  - the graphql operationName.
   * @param {Options}     overrides      - the client options overrides.
   *
   * @return {Promise<Array<Result>>} resolves to parsed json of server response
   *
   * @example
   * client.forceFetch(`
   *    query getHuman($id: ID!) {
   *      human(id: $id) {
   *        name
   *        height
   *      }
   *    }
   * `, { id: "1001" }, 'getHuman')
   *    .then(human => {
   *      // do something with human
   *      console.log(human);
   *    });
   */
  forceFetch(t, n, i, a = {}) {
    const p = {
      query: t
    }, o = Object.assign({}, this._options, a, {
      shouldBatch: !1
    });
    return n && (p.variables = n), i && (p.operationName = i), new Promise((r, s) => {
      const _ = new I(this.fetcher, this._options);
      _._queue = [
        {
          request: p,
          resolve: r,
          reject: s
        }
      ], y(_, o);
    });
  }
}
const R = {
  maxBatchSize: 10,
  batchInterval: 40
}, h = ({
  url: e,
  headers: t = {},
  fetcher: n,
  fetch: i,
  batch: a = !1,
  ...p
}) => {
  if (!e && !n)
    throw new Error("url or fetcher is required");
  if (n || (n = async (l) => {
    let r = typeof t == "function" ? await t() : t;
    if (r = r || {}, typeof fetch > "u" && !i)
      throw new Error(
        "Global `fetch` function is not available, pass a fetch polyfill to Genql `createClient`"
      );
    const _ = await (i || fetch)(e, {
      headers: {
        "Content-Type": "application/json",
        ...r
      },
      method: "POST",
      body: JSON.stringify(l),
      ...p
    });
    if (!_.ok)
      throw new Error(`${_.statusText}: ${await _.text()}`);
    return await _.json();
  }), !a)
    return async (l) => {
      const r = await n(l);
      if (Array.isArray(r))
        return r.map((s) => {
          if (s?.errors?.length)
            throw new u(s.errors || [], s.data);
          return s.data;
        });
      if (r?.errors?.length)
        throw new u(r.errors || [], r.data);
      return r.data;
    };
  const o = new I(
    async (l) => await n(l),
    a === !0 ? R : a
  );
  return async ({ query: l, variables: r }) => {
    const s = await o.fetch(l, r);
    if (s?.data)
      return s.data;
    throw new Error(
      "Genql batch fetcher returned unexpected result " + JSON.stringify(s)
    );
  };
}, m = (e, t, n) => {
  if (typeof e == "object" && "__args" in e) {
    const i = e.__args;
    let a = { ...e };
    delete a.__args;
    const p = Object.keys(i);
    if (p.length === 0)
      return m(a, t, n);
    const o = S(t.root, n);
    return `(${p.map((r) => {
      t.varCounter++;
      const s = `v${t.varCounter}`, _ = o.args && o.args[r];
      if (!_)
        throw new Error(
          `no typing defined for argument \`${r}\` in path \`${n.join(
            "."
          )}\``
        );
      return t.variables[s] = {
        value: i[r],
        typing: _
      }, `${r}:$${s}`;
    })})${m(a, t, n)}`;
  } else if (typeof e == "object" && Object.keys(e).length > 0) {
    const i = e, a = Object.keys(i).filter((s) => !!i[s]);
    if (a.length === 0)
      throw new Error(
        `field selection should not be empty: ${n.join(".")}`
      );
    const p = n.length > 0 ? S(t.root, n).type : t.root, o = p.scalar;
    let l;
    if (a.includes("__scalar")) {
      const s = new Set(
        Object.keys(i).filter((_) => !i[_])
      );
      o?.length && (t.fragmentCounter++, l = `f${t.fragmentCounter}`, t.fragments.push(
        `fragment ${l} on ${p.name}{${o.filter((_) => !s.has(_)).join(",")}}`
      ));
    }
    return `{${a.filter((s) => !["__scalar", "__name"].includes(s)).map((s) => {
      const _ = m(i[s], t, [...n, s]);
      if (s.startsWith("on_")) {
        t.fragmentCounter++;
        const d = `f${t.fragmentCounter}`, c = s.match(/^on_(.+)/);
        if (!c || !c[1])
          throw new Error("match failed");
        return t.fragments.push(
          `fragment ${d} on ${c[1]}${_}`
        ), `...${d}`;
      } else
        return `${s}${_}`;
    }).concat(l ? [`...${l}`] : []).join(",")}}`;
  } else
    return "";
}, T = (e, t, n) => {
  const i = {
    root: t,
    varCounter: 0,
    variables: {},
    fragmentCounter: 0,
    fragments: []
  }, a = m(n, i, []), p = Object.keys(i.variables), o = p.length > 0 ? `(${p.map((r) => {
    const s = i.variables[r].typing[1];
    return `$${r}:${s}`;
  })})` : "", l = n?.__name || "";
  return {
    query: [
      `${e} ${l}${o}${a}`,
      ...i.fragments
    ].join(","),
    variables: Object.keys(i.variables).reduce(
      (r, s) => (r[s] = i.variables[s].value, r),
      {}
    ),
    ...l ? { operationName: l.toString() } : {}
  };
}, S = (e, t) => {
  let n;
  if (!e) throw new Error("root type is not provided");
  if (t.length === 0) throw new Error("path is empty");
  return t.forEach((i) => {
    const a = n ? n.type : e;
    if (!a.fields)
      throw new Error(`type \`${a.name}\` does not have fields`);
    const p = Object.keys(a.fields).filter((l) => l.startsWith("on_")).reduce(
      (l, r) => {
        const s = a.fields && a.fields[r];
        return s && l.push(s.type), l;
      },
      [a]
    );
    let o = null;
    if (p.forEach((l) => {
      const r = l.fields && l.fields[i];
      r && (o = r);
    }), !o)
      throw new Error(
        `type \`${a.name}\` does not have a field \`${i}\``
      );
    n = o;
  }), n;
}, w = ({
  queryRoot: e,
  mutationRoot: t,
  subscriptionRoot: n,
  ...i
}) => {
  const a = h(i), p = {};
  return e && (p.query = (o) => {
    if (!e) throw new Error("queryRoot argument is missing");
    return a(
      T("query", e, o)
    );
  }), t && (p.mutation = (o) => {
    if (!t)
      throw new Error("mutationRoot argument is missing");
    return a(
      T("mutation", t, o)
    );
  }), p;
}, f = (e) => {
  const t = Object.assign(
    {},
    ...Object.keys(e.types).map((a, p) => ({ [p]: a }))
  );
  let n = Object.assign(
    {},
    ...Object.keys(e.types || {}).map(
      (a) => {
        const o = e.types[a] || {};
        return {
          [a]: {
            name: a,
            // type scalar properties
            scalar: Object.keys(o).filter((l) => {
              const [r] = o[l] || [];
              return r && e.scalars.includes(r);
            }),
            // fields with corresponding `type` and `args`
            fields: Object.assign(
              {},
              ...Object.keys(o).map(
                (l) => {
                  const [r, s] = o[l] || [];
                  return r == null ? {} : {
                    [l]: {
                      // replace index with type name
                      type: t[r],
                      args: Object.assign(
                        {},
                        ...Object.keys(s || {}).map(
                          (_) => {
                            if (!s || !s[_])
                              return;
                            const [
                              d,
                              c
                            ] = s[_];
                            return {
                              [_]: [
                                t[d],
                                c || t[d]
                              ]
                            };
                          }
                        )
                      )
                    }
                  };
                }
              )
            )
          }
        };
      }
    )
  );
  return P(n);
}, P = (e) => (Object.keys(e).forEach((t) => {
  const n = e[t];
  if (!n.fields)
    return;
  const i = n.fields;
  Object.keys(i).forEach((a) => {
    const p = i[a];
    if (p.args) {
      const l = p.args;
      Object.keys(l).forEach((r) => {
        const s = l[r];
        if (s) {
          const [_] = s;
          typeof _ == "string" && (e[_] || (e[_] = { name: _ }), s[0] = e[_]);
        }
      });
    }
    const o = p.type;
    typeof o == "string" && (e[o] || (e[o] = { name: o }), p.type = e[o]);
  });
}), e), F = {
  scalars: [
    1,
    3,
    4,
    6,
    8,
    11,
    12,
    14,
    15,
    18,
    21,
    22,
    23,
    24,
    36,
    37,
    44,
    47,
    49,
    60,
    62,
    64,
    67,
    70,
    71,
    72,
    73,
    74,
    76,
    79,
    80,
    85,
    88,
    93,
    96,
    97,
    99,
    103,
    104,
    110,
    124,
    132,
    133,
    140,
    141,
    142,
    144,
    152,
    163,
    166,
    168,
    172,
    174,
    179,
    180,
    187,
    190,
    193,
    206,
    231,
    294,
    303,
    304,
    305,
    306,
    308,
    309,
    310,
    311,
    312,
    313,
    314,
    317,
    319,
    328,
    335,
    342,
    378,
    460,
    472
  ],
  types: {
    BillingProductDTO: {
      name: [
        1
      ],
      description: [
        1
      ],
      images: [
        1
      ],
      metadata: [
        139
      ],
      on_BillingLicensedProduct: [
        148
      ],
      on_BillingMeteredProduct: [
        149
      ],
      __typename: [
        1
      ]
    },
    String: {},
    ApiKey: {
      id: [
        3
      ],
      name: [
        1
      ],
      expiresAt: [
        4
      ],
      revokedAt: [
        4
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      role: [
        29
      ],
      __typename: [
        1
      ]
    },
    UUID: {},
    DateTime: {},
    ApplicationRegistrationVariable: {
      id: [
        3
      ],
      key: [
        1
      ],
      description: [
        1
      ],
      isSecret: [
        6
      ],
      isRequired: [
        6
      ],
      isFilled: [
        6
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    Boolean: {},
    ApplicationRegistration: {
      id: [
        3
      ],
      universalIdentifier: [
        1
      ],
      name: [
        1
      ],
      oAuthClientId: [
        1
      ],
      oAuthRedirectUris: [
        1
      ],
      oAuthScopes: [
        1
      ],
      ownerWorkspaceId: [
        3
      ],
      sourceType: [
        8
      ],
      sourcePackage: [
        1
      ],
      latestAvailableVersion: [
        1
      ],
      isListed: [
        6
      ],
      isFeatured: [
        6
      ],
      isPreInstalled: [
        6
      ],
      logoUrl: [
        1
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      isConfigured: [
        6
      ],
      __typename: [
        1
      ]
    },
    ApplicationRegistrationSourceType: {},
    TwoFactorAuthenticationMethodSummary: {
      twoFactorAuthenticationMethodId: [
        3
      ],
      status: [
        1
      ],
      strategy: [
        1
      ],
      __typename: [
        1
      ]
    },
    RowLevelPermissionPredicateGroup: {
      id: [
        1
      ],
      parentRowLevelPermissionPredicateGroupId: [
        1
      ],
      logicalOperator: [
        12
      ],
      positionInRowLevelPermissionPredicateGroup: [
        11
      ],
      roleId: [
        1
      ],
      objectMetadataId: [
        1
      ],
      __typename: [
        1
      ]
    },
    Float: {},
    RowLevelPermissionPredicateGroupLogicalOperator: {},
    RowLevelPermissionPredicate: {
      id: [
        1
      ],
      fieldMetadataId: [
        1
      ],
      objectMetadataId: [
        1
      ],
      operand: [
        14
      ],
      subFieldName: [
        1
      ],
      workspaceMemberFieldMetadataId: [
        1
      ],
      workspaceMemberSubFieldName: [
        1
      ],
      rowLevelPermissionPredicateGroupId: [
        1
      ],
      positionInRowLevelPermissionPredicateGroup: [
        11
      ],
      roleId: [
        1
      ],
      value: [
        15
      ],
      __typename: [
        1
      ]
    },
    RowLevelPermissionPredicateOperand: {},
    JSON: {},
    ObjectPermission: {
      objectMetadataId: [
        3
      ],
      canReadObjectRecords: [
        6
      ],
      canUpdateObjectRecords: [
        6
      ],
      canSoftDeleteObjectRecords: [
        6
      ],
      canDestroyObjectRecords: [
        6
      ],
      restrictedFields: [
        15
      ],
      rowLevelPermissionPredicates: [
        13
      ],
      rowLevelPermissionPredicateGroups: [
        10
      ],
      __typename: [
        1
      ]
    },
    UserWorkspace: {
      id: [
        3
      ],
      user: [
        78
      ],
      userId: [
        3
      ],
      locale: [
        1
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      permissionFlags: [
        18
      ],
      objectPermissions: [
        16
      ],
      objectsPermissions: [
        16
      ],
      twoFactorAuthenticationMethodSummary: [
        9
      ],
      __typename: [
        1
      ]
    },
    PermissionFlagType: {},
    FullName: {
      firstName: [
        1
      ],
      lastName: [
        1
      ],
      __typename: [
        1
      ]
    },
    WorkspaceMember: {
      id: [
        3
      ],
      name: [
        19
      ],
      userEmail: [
        1
      ],
      colorScheme: [
        1
      ],
      avatarUrl: [
        1
      ],
      locale: [
        1
      ],
      calendarStartDay: [
        21
      ],
      timeZone: [
        1
      ],
      dateFormat: [
        22
      ],
      timeFormat: [
        23
      ],
      roles: [
        29
      ],
      userWorkspaceId: [
        3
      ],
      numberFormat: [
        24
      ],
      __typename: [
        1
      ]
    },
    Int: {},
    WorkspaceMemberDateFormatEnum: {},
    WorkspaceMemberTimeFormatEnum: {},
    WorkspaceMemberNumberFormatEnum: {},
    Agent: {
      id: [
        3
      ],
      name: [
        1
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      description: [
        1
      ],
      prompt: [
        1
      ],
      modelId: [
        1
      ],
      responseFormat: [
        15
      ],
      roleId: [
        3
      ],
      isCustom: [
        6
      ],
      applicationId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      modelConfiguration: [
        15
      ],
      evaluationInputs: [
        1
      ],
      __typename: [
        1
      ]
    },
    FieldPermission: {
      id: [
        3
      ],
      objectMetadataId: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      roleId: [
        3
      ],
      canReadFieldValue: [
        6
      ],
      canUpdateFieldValue: [
        6
      ],
      __typename: [
        1
      ]
    },
    RolePermissionFlag: {
      id: [
        3
      ],
      roleId: [
        3
      ],
      flag: [
        1
      ],
      __typename: [
        1
      ]
    },
    ApiKeyForRole: {
      id: [
        3
      ],
      name: [
        1
      ],
      expiresAt: [
        4
      ],
      revokedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    Role: {
      id: [
        3
      ],
      universalIdentifier: [
        3
      ],
      label: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      isEditable: [
        6
      ],
      canBeAssignedToUsers: [
        6
      ],
      canBeAssignedToAgents: [
        6
      ],
      canBeAssignedToApiKeys: [
        6
      ],
      workspaceMembers: [
        20
      ],
      agents: [
        25
      ],
      apiKeys: [
        28
      ],
      canUpdateAllSettings: [
        6
      ],
      canAccessAllTools: [
        6
      ],
      canReadAllObjectRecords: [
        6
      ],
      canUpdateAllObjectRecords: [
        6
      ],
      canSoftDeleteAllObjectRecords: [
        6
      ],
      canDestroyAllObjectRecords: [
        6
      ],
      permissionFlags: [
        27
      ],
      objectPermissions: [
        16
      ],
      fieldPermissions: [
        26
      ],
      rowLevelPermissionPredicates: [
        13
      ],
      rowLevelPermissionPredicateGroups: [
        10
      ],
      __typename: [
        1
      ]
    },
    ApplicationRegistrationSummary: {
      id: [
        3
      ],
      latestAvailableVersion: [
        1
      ],
      sourceType: [
        8
      ],
      logoUrl: [
        1
      ],
      __typename: [
        1
      ]
    },
    ApplicationVariable: {
      id: [
        3
      ],
      key: [
        1
      ],
      value: [
        1
      ],
      description: [
        1
      ],
      isSecret: [
        6
      ],
      __typename: [
        1
      ]
    },
    AuthToken: {
      token: [
        1
      ],
      expiresAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    ApplicationTokenPair: {
      applicationAccessToken: [
        32
      ],
      applicationRefreshToken: [
        32
      ],
      __typename: [
        1
      ]
    },
    FrontComponent: {
      id: [
        3
      ],
      name: [
        1
      ],
      description: [
        1
      ],
      sourceComponentPath: [
        1
      ],
      builtComponentPath: [
        1
      ],
      componentName: [
        1
      ],
      builtComponentChecksum: [
        1
      ],
      universalIdentifier: [
        3
      ],
      applicationId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      isHeadless: [
        6
      ],
      usesSdkClient: [
        6
      ],
      applicationTokenPair: [
        33
      ],
      applicationVariables: [
        15
      ],
      __typename: [
        1
      ]
    },
    CommandMenuItem: {
      id: [
        3
      ],
      workflowVersionId: [
        3
      ],
      frontComponentId: [
        3
      ],
      frontComponent: [
        34
      ],
      engineComponentKey: [
        36
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      shortLabel: [
        1
      ],
      position: [
        11
      ],
      isPinned: [
        6
      ],
      availabilityType: [
        37
      ],
      payload: [
        38
      ],
      hotKeys: [
        1
      ],
      conditionalAvailabilityExpression: [
        1
      ],
      availabilityObjectMetadataId: [
        3
      ],
      pageLayoutId: [
        3
      ],
      universalIdentifier: [
        3
      ],
      applicationId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    EngineComponentKey: {},
    CommandMenuItemAvailabilityType: {},
    CommandMenuItemPayload: {
      on_PathCommandMenuItemPayload: [
        39
      ],
      on_ObjectMetadataCommandMenuItemPayload: [
        40
      ],
      __typename: [
        1
      ]
    },
    PathCommandMenuItemPayload: {
      path: [
        1
      ],
      __typename: [
        1
      ]
    },
    ObjectMetadataCommandMenuItemPayload: {
      objectMetadataItemId: [
        3
      ],
      __typename: [
        1
      ]
    },
    LogicFunction: {
      id: [
        3
      ],
      name: [
        1
      ],
      description: [
        1
      ],
      runtime: [
        1
      ],
      timeoutSeconds: [
        11
      ],
      sourceHandlerPath: [
        1
      ],
      handlerName: [
        1
      ],
      cronTriggerSettings: [
        15
      ],
      databaseEventTriggerSettings: [
        15
      ],
      httpRouteTriggerSettings: [
        15
      ],
      toolTriggerSettings: [
        15
      ],
      workflowActionTriggerSettings: [
        15
      ],
      applicationId: [
        3
      ],
      universalIdentifier: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    StandardOverrides: {
      label: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      translations: [
        15
      ],
      __typename: [
        1
      ]
    },
    Field: {
      id: [
        3
      ],
      universalIdentifier: [
        1
      ],
      type: [
        44
      ],
      name: [
        1
      ],
      label: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      standardOverrides: [
        42
      ],
      isCustom: [
        6
      ],
      isActive: [
        6
      ],
      isSystem: [
        6
      ],
      isUIReadOnly: [
        6
      ],
      isNullable: [
        6
      ],
      isUnique: [
        6
      ],
      defaultValue: [
        15
      ],
      options: [
        15
      ],
      settings: [
        15
      ],
      objectMetadataId: [
        3
      ],
      isLabelSyncedWithName: [
        6
      ],
      morphId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      applicationId: [
        3
      ],
      relation: [
        205
      ],
      morphRelations: [
        205
      ],
      object: [
        55
      ],
      __typename: [
        1
      ]
    },
    FieldMetadataType: {},
    IndexField: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      order: [
        11
      ],
      subFieldName: [
        1
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    Index: {
      id: [
        3
      ],
      name: [
        1
      ],
      isCustom: [
        6
      ],
      isUnique: [
        6
      ],
      indexWhereClause: [
        1
      ],
      indexType: [
        47
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      indexFieldMetadataList: [
        45
      ],
      objectMetadata: [
        213,
        {
          paging: [
            48,
            "CursorPaging!"
          ],
          filter: [
            50,
            "ObjectFilter!"
          ]
        }
      ],
      indexFieldMetadatas: [
        211,
        {
          paging: [
            48,
            "CursorPaging!"
          ],
          filter: [
            53,
            "IndexFieldFilter!"
          ]
        }
      ],
      __typename: [
        1
      ]
    },
    IndexType: {},
    CursorPaging: {
      before: [
        49
      ],
      after: [
        49
      ],
      first: [
        21
      ],
      last: [
        21
      ],
      __typename: [
        1
      ]
    },
    ConnectionCursor: {},
    ObjectFilter: {
      and: [
        50
      ],
      or: [
        50
      ],
      id: [
        51
      ],
      isCustom: [
        52
      ],
      isRemote: [
        52
      ],
      isActive: [
        52
      ],
      isSystem: [
        52
      ],
      isUIReadOnly: [
        52
      ],
      isSearchable: [
        52
      ],
      __typename: [
        1
      ]
    },
    UUIDFilterComparison: {
      is: [
        6
      ],
      isNot: [
        6
      ],
      eq: [
        3
      ],
      neq: [
        3
      ],
      gt: [
        3
      ],
      gte: [
        3
      ],
      lt: [
        3
      ],
      lte: [
        3
      ],
      like: [
        3
      ],
      notLike: [
        3
      ],
      iLike: [
        3
      ],
      notILike: [
        3
      ],
      in: [
        3
      ],
      notIn: [
        3
      ],
      __typename: [
        1
      ]
    },
    BooleanFieldComparison: {
      is: [
        6
      ],
      isNot: [
        6
      ],
      __typename: [
        1
      ]
    },
    IndexFieldFilter: {
      and: [
        53
      ],
      or: [
        53
      ],
      id: [
        51
      ],
      fieldMetadataId: [
        51
      ],
      __typename: [
        1
      ]
    },
    ObjectStandardOverrides: {
      labelSingular: [
        1
      ],
      labelPlural: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      color: [
        1
      ],
      translations: [
        15
      ],
      __typename: [
        1
      ]
    },
    Object: {
      id: [
        3
      ],
      universalIdentifier: [
        1
      ],
      nameSingular: [
        1
      ],
      namePlural: [
        1
      ],
      labelSingular: [
        1
      ],
      labelPlural: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      standardOverrides: [
        54
      ],
      shortcut: [
        1
      ],
      color: [
        1
      ],
      isCustom: [
        6
      ],
      isRemote: [
        6
      ],
      isActive: [
        6
      ],
      isSystem: [
        6
      ],
      isUIReadOnly: [
        6
      ],
      isSearchable: [
        6
      ],
      applicationId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      labelIdentifierFieldMetadataId: [
        3
      ],
      imageIdentifierFieldMetadataId: [
        3
      ],
      isLabelSyncedWithName: [
        6
      ],
      duplicateCriteria: [
        1
      ],
      fieldsList: [
        43
      ],
      indexMetadataList: [
        46
      ],
      fields: [
        218,
        {
          paging: [
            48,
            "CursorPaging!"
          ],
          filter: [
            56,
            "FieldFilter!"
          ]
        }
      ],
      indexMetadatas: [
        216,
        {
          paging: [
            48,
            "CursorPaging!"
          ],
          filter: [
            57,
            "IndexFilter!"
          ]
        }
      ],
      __typename: [
        1
      ]
    },
    FieldFilter: {
      and: [
        56
      ],
      or: [
        56
      ],
      id: [
        51
      ],
      isCustom: [
        52
      ],
      isActive: [
        52
      ],
      isSystem: [
        52
      ],
      isUIReadOnly: [
        52
      ],
      objectMetadataId: [
        51
      ],
      __typename: [
        1
      ]
    },
    IndexFilter: {
      and: [
        57
      ],
      or: [
        57
      ],
      id: [
        51
      ],
      isCustom: [
        52
      ],
      __typename: [
        1
      ]
    },
    Application: {
      id: [
        3
      ],
      name: [
        1
      ],
      description: [
        1
      ],
      logo: [
        1
      ],
      version: [
        1
      ],
      universalIdentifier: [
        1
      ],
      packageJsonChecksum: [
        1
      ],
      packageJsonFileId: [
        3
      ],
      yarnLockChecksum: [
        1
      ],
      yarnLockFileId: [
        3
      ],
      availablePackages: [
        15
      ],
      applicationRegistrationId: [
        3
      ],
      canBeUninstalled: [
        6
      ],
      defaultRoleId: [
        1
      ],
      settingsCustomTabFrontComponentId: [
        3
      ],
      defaultLogicFunctionRole: [
        29
      ],
      agents: [
        25
      ],
      frontComponents: [
        34
      ],
      commandMenuItems: [
        35
      ],
      logicFunctions: [
        41
      ],
      objects: [
        55
      ],
      applicationVariables: [
        31
      ],
      applicationRegistration: [
        30
      ],
      __typename: [
        1
      ]
    },
    ViewField: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      isVisible: [
        6
      ],
      size: [
        11
      ],
      position: [
        11
      ],
      aggregateOperation: [
        60
      ],
      viewId: [
        3
      ],
      viewFieldGroupId: [
        3
      ],
      workspaceId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      isActive: [
        6
      ],
      deletedAt: [
        4
      ],
      isOverridden: [
        6
      ],
      __typename: [
        1
      ]
    },
    AggregateOperations: {},
    ViewFilterGroup: {
      id: [
        3
      ],
      parentViewFilterGroupId: [
        3
      ],
      logicalOperator: [
        62
      ],
      positionInViewFilterGroup: [
        11
      ],
      viewId: [
        3
      ],
      workspaceId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    ViewFilterGroupLogicalOperator: {},
    ViewFilter: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      operand: [
        64
      ],
      value: [
        15
      ],
      viewFilterGroupId: [
        3
      ],
      positionInViewFilterGroup: [
        11
      ],
      subFieldName: [
        1
      ],
      relationTargetFieldMetadataId: [
        3
      ],
      viewId: [
        3
      ],
      workspaceId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    ViewFilterOperand: {},
    ViewGroup: {
      id: [
        3
      ],
      isVisible: [
        6
      ],
      fieldValue: [
        1
      ],
      position: [
        11
      ],
      viewId: [
        3
      ],
      workspaceId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    ViewSort: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      direction: [
        67
      ],
      subFieldName: [
        1
      ],
      viewId: [
        3
      ],
      workspaceId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    ViewSortDirection: {},
    ViewFieldGroup: {
      id: [
        3
      ],
      name: [
        1
      ],
      position: [
        11
      ],
      isVisible: [
        6
      ],
      viewId: [
        3
      ],
      workspaceId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      isActive: [
        6
      ],
      deletedAt: [
        4
      ],
      viewFields: [
        59
      ],
      isOverridden: [
        6
      ],
      __typename: [
        1
      ]
    },
    View: {
      id: [
        3
      ],
      name: [
        1
      ],
      objectMetadataId: [
        3
      ],
      type: [
        70
      ],
      key: [
        71
      ],
      icon: [
        1
      ],
      position: [
        11
      ],
      isCompact: [
        6
      ],
      isCustom: [
        6
      ],
      openRecordIn: [
        72
      ],
      kanbanAggregateOperation: [
        60
      ],
      kanbanAggregateOperationFieldMetadataId: [
        3
      ],
      mainGroupByFieldMetadataId: [
        3
      ],
      shouldHideEmptyGroups: [
        6
      ],
      calendarFieldMetadataId: [
        3
      ],
      workspaceId: [
        3
      ],
      anyFieldFilterValue: [
        1
      ],
      calendarLayout: [
        73
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      viewFields: [
        59
      ],
      viewFilters: [
        63
      ],
      viewFilterGroups: [
        61
      ],
      viewSorts: [
        66
      ],
      viewGroups: [
        65
      ],
      viewFieldGroups: [
        68
      ],
      visibility: [
        74
      ],
      createdByUserWorkspaceId: [
        3
      ],
      __typename: [
        1
      ]
    },
    ViewType: {},
    ViewKey: {},
    ViewOpenRecordIn: {},
    ViewCalendarLayout: {},
    ViewVisibility: {},
    Workspace: {
      id: [
        3
      ],
      displayName: [
        1
      ],
      logo: [
        1
      ],
      logoFileId: [
        3
      ],
      inviteHash: [
        1
      ],
      deletedAt: [
        4
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      allowImpersonation: [
        6
      ],
      isPublicInviteLinkEnabled: [
        6
      ],
      trashRetentionDays: [
        11
      ],
      eventLogRetentionDays: [
        11
      ],
      workspaceMembersCount: [
        11
      ],
      activationStatus: [
        76
      ],
      views: [
        69
      ],
      viewFields: [
        59
      ],
      viewFilters: [
        63
      ],
      viewFilterGroups: [
        61
      ],
      viewGroups: [
        65
      ],
      viewSorts: [
        66
      ],
      metadataVersion: [
        11
      ],
      databaseSchema: [
        1
      ],
      subdomain: [
        1
      ],
      customDomain: [
        1
      ],
      isGoogleAuthEnabled: [
        6
      ],
      isGoogleAuthBypassEnabled: [
        6
      ],
      isTwoFactorAuthenticationEnforced: [
        6
      ],
      isPasswordAuthEnabled: [
        6
      ],
      isPasswordAuthBypassEnabled: [
        6
      ],
      isMicrosoftAuthEnabled: [
        6
      ],
      isMicrosoftAuthBypassEnabled: [
        6
      ],
      isCustomDomainEnabled: [
        6
      ],
      isInternalMessagesImportEnabled: [
        6
      ],
      editableProfileFields: [
        1
      ],
      defaultRole: [
        29
      ],
      fastModel: [
        1
      ],
      smartModel: [
        1
      ],
      aiAdditionalInstructions: [
        1
      ],
      enabledAiModelIds: [
        1
      ],
      useRecommendedModels: [
        6
      ],
      routerModel: [
        1
      ],
      workspaceCustomApplication: [
        58
      ],
      featureFlags: [
        173
      ],
      billingSubscriptions: [
        151
      ],
      installedApplications: [
        58
      ],
      currentBillingSubscription: [
        151
      ],
      billingEntitlements: [
        230
      ],
      hasValidEnterpriseKey: [
        6
      ],
      hasValidSignedEnterpriseKey: [
        6
      ],
      hasValidEnterpriseValidityToken: [
        6
      ],
      workspaceUrls: [
        175
      ],
      workspaceCustomApplicationId: [
        1
      ],
      __typename: [
        1
      ]
    },
    WorkspaceActivationStatus: {},
    AppToken: {
      id: [
        3
      ],
      type: [
        1
      ],
      expiresAt: [
        4
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    User: {
      id: [
        3
      ],
      firstName: [
        1
      ],
      lastName: [
        1
      ],
      email: [
        1
      ],
      isEmailVerified: [
        6
      ],
      disabled: [
        6
      ],
      canImpersonate: [
        6
      ],
      canAccessFullAdminPanel: [
        6
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      locale: [
        1
      ],
      workspaceMember: [
        20
      ],
      userWorkspaces: [
        17
      ],
      onboardingStatus: [
        79
      ],
      currentWorkspace: [
        75
      ],
      currentUserWorkspace: [
        17
      ],
      userVars: [
        80
      ],
      workspaceMembers: [
        20
      ],
      deletedWorkspaceMembers: [
        229
      ],
      hasPassword: [
        6
      ],
      supportUserHash: [
        1
      ],
      workspaces: [
        17
      ],
      availableWorkspaces: [
        228
      ],
      __typename: [
        1
      ]
    },
    OnboardingStatus: {},
    JSONObject: {},
    RatioAggregateConfig: {
      fieldMetadataId: [
        3
      ],
      optionValue: [
        1
      ],
      __typename: [
        1
      ]
    },
    RichTextBody: {
      blocknote: [
        1
      ],
      markdown: [
        1
      ],
      __typename: [
        1
      ]
    },
    GridPosition: {
      row: [
        11
      ],
      column: [
        11
      ],
      rowSpan: [
        11
      ],
      columnSpan: [
        11
      ],
      __typename: [
        1
      ]
    },
    PageLayoutWidget: {
      id: [
        3
      ],
      applicationId: [
        3
      ],
      pageLayoutTabId: [
        3
      ],
      title: [
        1
      ],
      type: [
        85
      ],
      objectMetadataId: [
        3
      ],
      gridPosition: [
        83
      ],
      position: [
        86
      ],
      configuration: [
        91
      ],
      conditionalDisplay: [
        15
      ],
      conditionalAvailabilityExpression: [
        1
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      isActive: [
        6
      ],
      deletedAt: [
        4
      ],
      isOverridden: [
        6
      ],
      __typename: [
        1
      ]
    },
    WidgetType: {},
    PageLayoutWidgetPosition: {
      on_PageLayoutWidgetGridPosition: [
        87
      ],
      on_PageLayoutWidgetVerticalListPosition: [
        89
      ],
      on_PageLayoutWidgetCanvasPosition: [
        90
      ],
      __typename: [
        1
      ]
    },
    PageLayoutWidgetGridPosition: {
      layoutMode: [
        88
      ],
      row: [
        21
      ],
      column: [
        21
      ],
      rowSpan: [
        21
      ],
      columnSpan: [
        21
      ],
      __typename: [
        1
      ]
    },
    PageLayoutTabLayoutMode: {},
    PageLayoutWidgetVerticalListPosition: {
      layoutMode: [
        88
      ],
      index: [
        21
      ],
      __typename: [
        1
      ]
    },
    PageLayoutWidgetCanvasPosition: {
      layoutMode: [
        88
      ],
      __typename: [
        1
      ]
    },
    WidgetConfiguration: {
      on_AggregateChartConfiguration: [
        92
      ],
      on_StandaloneRichTextConfiguration: [
        94
      ],
      on_PieChartConfiguration: [
        95
      ],
      on_LineChartConfiguration: [
        98
      ],
      on_IframeConfiguration: [
        100
      ],
      on_GaugeChartConfiguration: [
        101
      ],
      on_BarChartConfiguration: [
        102
      ],
      on_CalendarConfiguration: [
        105
      ],
      on_FrontComponentConfiguration: [
        106
      ],
      on_EmailsConfiguration: [
        107
      ],
      on_EmailThreadConfiguration: [
        108
      ],
      on_FieldConfiguration: [
        109
      ],
      on_FieldRichTextConfiguration: [
        111
      ],
      on_FieldsConfiguration: [
        112
      ],
      on_FilesConfiguration: [
        113
      ],
      on_NotesConfiguration: [
        114
      ],
      on_TasksConfiguration: [
        115
      ],
      on_TimelineConfiguration: [
        116
      ],
      on_ViewConfiguration: [
        117
      ],
      on_RecordTableConfiguration: [
        118
      ],
      on_WorkflowConfiguration: [
        119
      ],
      on_WorkflowRunConfiguration: [
        120
      ],
      on_WorkflowVersionConfiguration: [
        121
      ],
      __typename: [
        1
      ]
    },
    AggregateChartConfiguration: {
      configurationType: [
        93
      ],
      aggregateFieldMetadataId: [
        3
      ],
      aggregateOperation: [
        60
      ],
      label: [
        1
      ],
      displayDataLabel: [
        6
      ],
      format: [
        1
      ],
      description: [
        1
      ],
      filter: [
        15
      ],
      timezone: [
        1
      ],
      firstDayOfTheWeek: [
        21
      ],
      prefix: [
        1
      ],
      suffix: [
        1
      ],
      ratioAggregateConfig: [
        81
      ],
      __typename: [
        1
      ]
    },
    WidgetConfigurationType: {},
    StandaloneRichTextConfiguration: {
      configurationType: [
        93
      ],
      body: [
        82
      ],
      __typename: [
        1
      ]
    },
    PieChartConfiguration: {
      configurationType: [
        93
      ],
      aggregateFieldMetadataId: [
        3
      ],
      aggregateOperation: [
        60
      ],
      groupByFieldMetadataId: [
        3
      ],
      groupBySubFieldName: [
        1
      ],
      dateGranularity: [
        96
      ],
      orderBy: [
        97
      ],
      manualSortOrder: [
        1
      ],
      displayDataLabel: [
        6
      ],
      showCenterMetric: [
        6
      ],
      displayLegend: [
        6
      ],
      hideEmptyCategory: [
        6
      ],
      splitMultiValueFields: [
        6
      ],
      description: [
        1
      ],
      color: [
        1
      ],
      filter: [
        15
      ],
      timezone: [
        1
      ],
      firstDayOfTheWeek: [
        21
      ],
      __typename: [
        1
      ]
    },
    ObjectRecordGroupByDateGranularity: {},
    GraphOrderBy: {},
    LineChartConfiguration: {
      configurationType: [
        93
      ],
      aggregateFieldMetadataId: [
        3
      ],
      aggregateOperation: [
        60
      ],
      primaryAxisGroupByFieldMetadataId: [
        3
      ],
      primaryAxisGroupBySubFieldName: [
        1
      ],
      primaryAxisDateGranularity: [
        96
      ],
      primaryAxisOrderBy: [
        97
      ],
      primaryAxisManualSortOrder: [
        1
      ],
      secondaryAxisGroupByFieldMetadataId: [
        3
      ],
      secondaryAxisGroupBySubFieldName: [
        1
      ],
      secondaryAxisGroupByDateGranularity: [
        96
      ],
      secondaryAxisOrderBy: [
        97
      ],
      secondaryAxisManualSortOrder: [
        1
      ],
      omitNullValues: [
        6
      ],
      splitMultiValueFields: [
        6
      ],
      axisNameDisplay: [
        99
      ],
      displayDataLabel: [
        6
      ],
      displayLegend: [
        6
      ],
      rangeMin: [
        11
      ],
      rangeMax: [
        11
      ],
      description: [
        1
      ],
      color: [
        1
      ],
      filter: [
        15
      ],
      isStacked: [
        6
      ],
      isCumulative: [
        6
      ],
      timezone: [
        1
      ],
      firstDayOfTheWeek: [
        21
      ],
      __typename: [
        1
      ]
    },
    AxisNameDisplay: {},
    IframeConfiguration: {
      configurationType: [
        93
      ],
      url: [
        1
      ],
      __typename: [
        1
      ]
    },
    GaugeChartConfiguration: {
      configurationType: [
        93
      ],
      aggregateFieldMetadataId: [
        3
      ],
      aggregateOperation: [
        60
      ],
      displayDataLabel: [
        6
      ],
      color: [
        1
      ],
      description: [
        1
      ],
      filter: [
        15
      ],
      timezone: [
        1
      ],
      firstDayOfTheWeek: [
        21
      ],
      __typename: [
        1
      ]
    },
    BarChartConfiguration: {
      configurationType: [
        93
      ],
      aggregateFieldMetadataId: [
        3
      ],
      aggregateOperation: [
        60
      ],
      primaryAxisGroupByFieldMetadataId: [
        3
      ],
      primaryAxisGroupBySubFieldName: [
        1
      ],
      primaryAxisDateGranularity: [
        96
      ],
      primaryAxisOrderBy: [
        97
      ],
      primaryAxisManualSortOrder: [
        1
      ],
      secondaryAxisGroupByFieldMetadataId: [
        3
      ],
      secondaryAxisGroupBySubFieldName: [
        1
      ],
      secondaryAxisGroupByDateGranularity: [
        96
      ],
      secondaryAxisOrderBy: [
        97
      ],
      secondaryAxisManualSortOrder: [
        1
      ],
      omitNullValues: [
        6
      ],
      splitMultiValueFields: [
        6
      ],
      axisNameDisplay: [
        99
      ],
      displayDataLabel: [
        6
      ],
      displayLegend: [
        6
      ],
      rangeMin: [
        11
      ],
      rangeMax: [
        11
      ],
      description: [
        1
      ],
      color: [
        1
      ],
      filter: [
        15
      ],
      groupMode: [
        103
      ],
      layout: [
        104
      ],
      isCumulative: [
        6
      ],
      timezone: [
        1
      ],
      firstDayOfTheWeek: [
        21
      ],
      __typename: [
        1
      ]
    },
    BarChartGroupMode: {},
    BarChartLayout: {},
    CalendarConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    FrontComponentConfiguration: {
      configurationType: [
        93
      ],
      frontComponentId: [
        3
      ],
      __typename: [
        1
      ]
    },
    EmailsConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    EmailThreadConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    FieldConfiguration: {
      configurationType: [
        93
      ],
      fieldMetadataId: [
        1
      ],
      fieldDisplayMode: [
        110
      ],
      viewId: [
        1
      ],
      __typename: [
        1
      ]
    },
    FieldDisplayMode: {},
    FieldRichTextConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    FieldsConfiguration: {
      configurationType: [
        93
      ],
      viewId: [
        1
      ],
      newFieldDefaultVisibility: [
        6
      ],
      shouldAllowUserToSeeHiddenFields: [
        6
      ],
      __typename: [
        1
      ]
    },
    FilesConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    NotesConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    TasksConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    TimelineConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    ViewConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    RecordTableConfiguration: {
      configurationType: [
        93
      ],
      viewId: [
        1
      ],
      __typename: [
        1
      ]
    },
    WorkflowConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    WorkflowRunConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    WorkflowVersionConfiguration: {
      configurationType: [
        93
      ],
      __typename: [
        1
      ]
    },
    PageLayoutTab: {
      id: [
        3
      ],
      applicationId: [
        3
      ],
      title: [
        1
      ],
      position: [
        11
      ],
      pageLayoutId: [
        3
      ],
      widgets: [
        84
      ],
      icon: [
        1
      ],
      layoutMode: [
        88
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      isActive: [
        6
      ],
      deletedAt: [
        4
      ],
      isOverridden: [
        6
      ],
      __typename: [
        1
      ]
    },
    PageLayout: {
      id: [
        3
      ],
      name: [
        1
      ],
      type: [
        124
      ],
      objectMetadataId: [
        3
      ],
      tabs: [
        122
      ],
      defaultTabToFocusOnMobileAndSidePanelId: [
        3
      ],
      universalIdentifier: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    PageLayoutType: {},
    ApplicationConnectionProviderOAuthConfig: {
      scopes: [
        1
      ],
      isClientCredentialsConfigured: [
        6
      ],
      __typename: [
        1
      ]
    },
    ApplicationConnectionProvider: {
      id: [
        3
      ],
      applicationId: [
        1
      ],
      type: [
        1
      ],
      name: [
        1
      ],
      displayName: [
        1
      ],
      oauth: [
        125
      ],
      __typename: [
        1
      ]
    },
    EnterpriseLicenseInfoDTO: {
      isValid: [
        6
      ],
      licensee: [
        1
      ],
      expiresAt: [
        4
      ],
      subscriptionId: [
        1
      ],
      __typename: [
        1
      ]
    },
    EnterpriseSubscriptionStatusDTO: {
      status: [
        1
      ],
      licensee: [
        1
      ],
      expiresAt: [
        4
      ],
      cancelAt: [
        4
      ],
      currentPeriodEnd: [
        4
      ],
      isCancellationScheduled: [
        6
      ],
      __typename: [
        1
      ]
    },
    Analytics: {
      success: [
        6
      ],
      __typename: [
        1
      ]
    },
    VerificationRecord: {
      type: [
        1
      ],
      key: [
        1
      ],
      value: [
        1
      ],
      priority: [
        11
      ],
      __typename: [
        1
      ]
    },
    EmailingDomain: {
      id: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      domain: [
        1
      ],
      driver: [
        132
      ],
      status: [
        133
      ],
      verificationRecords: [
        130
      ],
      verifiedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    EmailingDomainDriver: {},
    EmailingDomainStatus: {},
    SendEmailViaDomainOutput: {
      messageId: [
        1
      ],
      __typename: [
        1
      ]
    },
    ApprovedAccessDomain: {
      id: [
        3
      ],
      domain: [
        1
      ],
      isValidated: [
        6
      ],
      createdAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    FileWithSignedUrl: {
      id: [
        3
      ],
      path: [
        1
      ],
      size: [
        11
      ],
      createdAt: [
        4
      ],
      url: [
        1
      ],
      __typename: [
        1
      ]
    },
    BillingSubscriptionSchedulePhaseItem: {
      price: [
        1
      ],
      quantity: [
        11
      ],
      __typename: [
        1
      ]
    },
    BillingSubscriptionSchedulePhase: {
      start_date: [
        11
      ],
      end_date: [
        11
      ],
      items: [
        137
      ],
      __typename: [
        1
      ]
    },
    BillingProductMetadata: {
      planKey: [
        140
      ],
      priceUsageBased: [
        141
      ],
      productKey: [
        142
      ],
      __typename: [
        1
      ]
    },
    BillingPlanKey: {},
    BillingUsageType: {},
    BillingProductKey: {},
    BillingPriceLicensed: {
      recurringInterval: [
        144
      ],
      unitAmount: [
        11
      ],
      stripePriceId: [
        1
      ],
      priceUsageType: [
        141
      ],
      creditAmount: [
        11
      ],
      __typename: [
        1
      ]
    },
    SubscriptionInterval: {},
    BillingPriceTier: {
      upTo: [
        11
      ],
      flatAmount: [
        11
      ],
      unitAmount: [
        11
      ],
      __typename: [
        1
      ]
    },
    BillingPriceMetered: {
      tiers: [
        145
      ],
      recurringInterval: [
        144
      ],
      stripePriceId: [
        1
      ],
      priceUsageType: [
        141
      ],
      __typename: [
        1
      ]
    },
    BillingProduct: {
      name: [
        1
      ],
      description: [
        1
      ],
      images: [
        1
      ],
      metadata: [
        139
      ],
      __typename: [
        1
      ]
    },
    BillingLicensedProduct: {
      name: [
        1
      ],
      description: [
        1
      ],
      images: [
        1
      ],
      metadata: [
        139
      ],
      prices: [
        143
      ],
      __typename: [
        1
      ]
    },
    BillingMeteredProduct: {
      name: [
        1
      ],
      description: [
        1
      ],
      images: [
        1
      ],
      metadata: [
        139
      ],
      prices: [
        146
      ],
      __typename: [
        1
      ]
    },
    BillingSubscriptionItem: {
      id: [
        3
      ],
      hasReachedCurrentPeriodCap: [
        6
      ],
      quantity: [
        11
      ],
      stripePriceId: [
        1
      ],
      billingProduct: [
        0
      ],
      __typename: [
        1
      ]
    },
    BillingSubscription: {
      id: [
        3
      ],
      status: [
        152
      ],
      interval: [
        144
      ],
      billingSubscriptionItems: [
        150
      ],
      currentPeriodEnd: [
        4
      ],
      metadata: [
        15
      ],
      phases: [
        138
      ],
      __typename: [
        1
      ]
    },
    SubscriptionStatus: {},
    BillingEndTrialPeriod: {
      status: [
        152
      ],
      hasPaymentMethod: [
        6
      ],
      billingPortalUrl: [
        1
      ],
      __typename: [
        1
      ]
    },
    BillingResourceCreditUsage: {
      productKey: [
        142
      ],
      periodStart: [
        4
      ],
      periodEnd: [
        4
      ],
      usedCredits: [
        11
      ],
      grantedCredits: [
        11
      ],
      rolloverCredits: [
        11
      ],
      totalGrantedCredits: [
        11
      ],
      unitPriceCents: [
        11
      ],
      __typename: [
        1
      ]
    },
    BillingPlan: {
      planKey: [
        140
      ],
      baseProducts: [
        148
      ],
      resourceCreditProducts: [
        148
      ],
      meteredProducts: [
        149
      ],
      __typename: [
        1
      ]
    },
    BillingSession: {
      url: [
        1
      ],
      __typename: [
        1
      ]
    },
    BillingUpdate: {
      currentBillingSubscription: [
        151
      ],
      billingSubscriptions: [
        151
      ],
      __typename: [
        1
      ]
    },
    OnboardingStepSuccess: {
      success: [
        6
      ],
      __typename: [
        1
      ]
    },
    WorkspaceInvitation: {
      id: [
        3
      ],
      email: [
        1
      ],
      roleId: [
        3
      ],
      expiresAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    SendInvitations: {
      success: [
        6
      ],
      errors: [
        1
      ],
      result: [
        159
      ],
      __typename: [
        1
      ]
    },
    RecordIdentifier: {
      id: [
        3
      ],
      labelIdentifier: [
        1
      ],
      imageIdentifier: [
        1
      ],
      __typename: [
        1
      ]
    },
    NavigationMenuItem: {
      id: [
        3
      ],
      userWorkspaceId: [
        3
      ],
      targetRecordId: [
        3
      ],
      targetObjectMetadataId: [
        3
      ],
      viewId: [
        3
      ],
      type: [
        163
      ],
      name: [
        1
      ],
      link: [
        1
      ],
      icon: [
        1
      ],
      color: [
        1
      ],
      folderId: [
        3
      ],
      pageLayoutId: [
        3
      ],
      position: [
        11
      ],
      applicationId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      targetRecordIdentifier: [
        161
      ],
      __typename: [
        1
      ]
    },
    NavigationMenuItemType: {},
    ObjectRecordEventProperties: {
      updatedFields: [
        1
      ],
      before: [
        15
      ],
      after: [
        15
      ],
      diff: [
        15
      ],
      __typename: [
        1
      ]
    },
    MetadataEvent: {
      type: [
        166
      ],
      metadataName: [
        1
      ],
      recordId: [
        1
      ],
      properties: [
        164
      ],
      updatedCollectionHash: [
        1
      ],
      __typename: [
        1
      ]
    },
    MetadataEventAction: {},
    ObjectRecordEvent: {
      action: [
        168
      ],
      objectNameSingular: [
        1
      ],
      recordId: [
        1
      ],
      userId: [
        1
      ],
      workspaceMemberId: [
        1
      ],
      properties: [
        164
      ],
      __typename: [
        1
      ]
    },
    DatabaseEventAction: {},
    ObjectRecordEventWithQueryIds: {
      queryIds: [
        1
      ],
      objectRecordEvent: [
        167
      ],
      __typename: [
        1
      ]
    },
    EventSubscription: {
      eventStreamId: [
        1
      ],
      objectRecordEventsWithQueryIds: [
        169
      ],
      metadataEvents: [
        165
      ],
      __typename: [
        1
      ]
    },
    LogicFunctionExecutionResult: {
      data: [
        15
      ],
      logs: [
        1
      ],
      duration: [
        11
      ],
      status: [
        172
      ],
      error: [
        15
      ],
      __typename: [
        1
      ]
    },
    LogicFunctionExecutionStatus: {},
    FeatureFlag: {
      key: [
        174
      ],
      value: [
        6
      ],
      __typename: [
        1
      ]
    },
    FeatureFlagKey: {},
    WorkspaceUrls: {
      customUrl: [
        1
      ],
      subdomainUrl: [
        1
      ],
      __typename: [
        1
      ]
    },
    ApplicationRegistrationVariableDTO: {
      id: [
        3
      ],
      key: [
        1
      ],
      value: [
        1
      ],
      description: [
        1
      ],
      isSecret: [
        6
      ],
      isRequired: [
        6
      ],
      isFilled: [
        6
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    BillingTrialPeriod: {
      duration: [
        11
      ],
      isCreditCardRequired: [
        6
      ],
      __typename: [
        1
      ]
    },
    SSOIdentityProvider: {
      id: [
        3
      ],
      name: [
        1
      ],
      type: [
        179
      ],
      status: [
        180
      ],
      issuer: [
        1
      ],
      __typename: [
        1
      ]
    },
    IdentityProviderType: {},
    SSOIdentityProviderStatus: {},
    AuthProviders: {
      sso: [
        178
      ],
      google: [
        6
      ],
      magicLink: [
        6
      ],
      password: [
        6
      ],
      microsoft: [
        6
      ],
      __typename: [
        1
      ]
    },
    AuthBypassProviders: {
      google: [
        6
      ],
      password: [
        6
      ],
      microsoft: [
        6
      ],
      __typename: [
        1
      ]
    },
    PublicWorkspaceData: {
      id: [
        3
      ],
      authProviders: [
        181
      ],
      authBypassProviders: [
        182
      ],
      logo: [
        1
      ],
      displayName: [
        1
      ],
      workspaceUrls: [
        175
      ],
      __typename: [
        1
      ]
    },
    PublicWorkspaceDataSummary: {
      id: [
        3
      ],
      logo: [
        1
      ],
      displayName: [
        1
      ],
      __typename: [
        1
      ]
    },
    NativeModelCapabilities: {
      webSearch: [
        6
      ],
      twitterSearch: [
        6
      ],
      __typename: [
        1
      ]
    },
    ClientAiModelConfig: {
      modelId: [
        1
      ],
      label: [
        1
      ],
      modelFamily: [
        187
      ],
      modelFamilyLabel: [
        1
      ],
      sdkPackage: [
        1
      ],
      inputCostPerMillionTokens: [
        11
      ],
      outputCostPerMillionTokens: [
        11
      ],
      nativeCapabilities: [
        185
      ],
      isDeprecated: [
        6
      ],
      isRecommended: [
        6
      ],
      providerName: [
        1
      ],
      providerLabel: [
        1
      ],
      contextWindowTokens: [
        11
      ],
      maxOutputTokens: [
        11
      ],
      dataResidency: [
        1
      ],
      __typename: [
        1
      ]
    },
    ModelFamily: {},
    Billing: {
      isBillingEnabled: [
        6
      ],
      billingUrl: [
        1
      ],
      trialPeriods: [
        177
      ],
      __typename: [
        1
      ]
    },
    Support: {
      supportDriver: [
        190
      ],
      supportFrontChatId: [
        1
      ],
      __typename: [
        1
      ]
    },
    SupportDriver: {},
    Sentry: {
      environment: [
        1
      ],
      release: [
        1
      ],
      dsn: [
        1
      ],
      __typename: [
        1
      ]
    },
    Captcha: {
      provider: [
        193
      ],
      siteKey: [
        1
      ],
      __typename: [
        1
      ]
    },
    CaptchaDriverType: {},
    ApiConfig: {
      mutationMaximumAffectedRecords: [
        11
      ],
      __typename: [
        1
      ]
    },
    PublicFeatureFlagMetadata: {
      label: [
        1
      ],
      description: [
        1
      ],
      imagePath: [
        1
      ],
      __typename: [
        1
      ]
    },
    PublicFeatureFlag: {
      key: [
        174
      ],
      metadata: [
        195
      ],
      __typename: [
        1
      ]
    },
    ClientConfigMaintenanceMode: {
      startAt: [
        4
      ],
      endAt: [
        4
      ],
      link: [
        1
      ],
      __typename: [
        1
      ]
    },
    ClientConfig: {
      appVersion: [
        1
      ],
      authProviders: [
        181
      ],
      billing: [
        188
      ],
      aiModels: [
        186
      ],
      signInPrefilled: [
        6
      ],
      isMultiWorkspaceEnabled: [
        6
      ],
      isEmailVerificationRequired: [
        6
      ],
      defaultSubdomain: [
        1
      ],
      frontDomain: [
        1
      ],
      analyticsEnabled: [
        6
      ],
      support: [
        189
      ],
      isAttachmentPreviewEnabled: [
        6
      ],
      sentry: [
        191
      ],
      captcha: [
        192
      ],
      api: [
        194
      ],
      canManageFeatureFlags: [
        6
      ],
      publicFeatureFlags: [
        196
      ],
      isMicrosoftMessagingEnabled: [
        6
      ],
      isMicrosoftCalendarEnabled: [
        6
      ],
      isGoogleMessagingEnabled: [
        6
      ],
      isGoogleCalendarEnabled: [
        6
      ],
      isConfigVariablesInDbEnabled: [
        6
      ],
      isImapSmtpCaldavEnabled: [
        6
      ],
      isEmailGroupEnabled: [
        6
      ],
      allowRequestsToTwentyIcons: [
        6
      ],
      calendarBookingPageId: [
        1
      ],
      isCloudflareIntegrationEnabled: [
        6
      ],
      isClickHouseConfigured: [
        6
      ],
      isWorkspaceSchemaDDLLocked: [
        6
      ],
      maintenance: [
        197
      ],
      __typename: [
        1
      ]
    },
    UsageBreakdownItem: {
      key: [
        1
      ],
      label: [
        1
      ],
      creditsUsed: [
        11
      ],
      __typename: [
        1
      ]
    },
    VersionDistributionEntry: {
      version: [
        1
      ],
      count: [
        21
      ],
      __typename: [
        1
      ]
    },
    ApplicationRegistrationStats: {
      activeInstalls: [
        21
      ],
      mostInstalledVersion: [
        1
      ],
      versionDistribution: [
        200
      ],
      __typename: [
        1
      ]
    },
    CreateApplicationRegistration: {
      applicationRegistration: [
        7
      ],
      clientSecret: [
        1
      ],
      __typename: [
        1
      ]
    },
    PublicApplicationRegistration: {
      id: [
        3
      ],
      name: [
        1
      ],
      logoUrl: [
        1
      ],
      websiteUrl: [
        1
      ],
      oAuthScopes: [
        1
      ],
      __typename: [
        1
      ]
    },
    RotateClientSecret: {
      clientSecret: [
        1
      ],
      __typename: [
        1
      ]
    },
    Relation: {
      type: [
        206
      ],
      sourceObjectMetadata: [
        55
      ],
      targetObjectMetadata: [
        55
      ],
      sourceFieldMetadata: [
        43
      ],
      targetFieldMetadata: [
        43
      ],
      __typename: [
        1
      ]
    },
    RelationType: {},
    IndexEdge: {
      node: [
        46
      ],
      cursor: [
        49
      ],
      __typename: [
        1
      ]
    },
    PageInfo: {
      hasNextPage: [
        6
      ],
      hasPreviousPage: [
        6
      ],
      startCursor: [
        49
      ],
      endCursor: [
        49
      ],
      __typename: [
        1
      ]
    },
    IndexConnection: {
      pageInfo: [
        208
      ],
      edges: [
        207
      ],
      __typename: [
        1
      ]
    },
    IndexFieldEdge: {
      node: [
        45
      ],
      cursor: [
        49
      ],
      __typename: [
        1
      ]
    },
    IndexIndexFieldMetadatasConnection: {
      pageInfo: [
        208
      ],
      edges: [
        210
      ],
      __typename: [
        1
      ]
    },
    ObjectEdge: {
      node: [
        55
      ],
      cursor: [
        49
      ],
      __typename: [
        1
      ]
    },
    IndexObjectMetadataConnection: {
      pageInfo: [
        208
      ],
      edges: [
        212
      ],
      __typename: [
        1
      ]
    },
    ObjectRecordCount: {
      objectNamePlural: [
        1
      ],
      totalCount: [
        21
      ],
      __typename: [
        1
      ]
    },
    ObjectConnection: {
      pageInfo: [
        208
      ],
      edges: [
        212
      ],
      __typename: [
        1
      ]
    },
    ObjectIndexMetadatasConnection: {
      pageInfo: [
        208
      ],
      edges: [
        207
      ],
      __typename: [
        1
      ]
    },
    FieldEdge: {
      node: [
        43
      ],
      cursor: [
        49
      ],
      __typename: [
        1
      ]
    },
    ObjectFieldsConnection: {
      pageInfo: [
        208
      ],
      edges: [
        217
      ],
      __typename: [
        1
      ]
    },
    FieldConnection: {
      pageInfo: [
        208
      ],
      edges: [
        217
      ],
      __typename: [
        1
      ]
    },
    ResendEmailVerificationToken: {
      success: [
        6
      ],
      __typename: [
        1
      ]
    },
    DeleteSso: {
      identityProviderId: [
        3
      ],
      __typename: [
        1
      ]
    },
    EditSso: {
      id: [
        3
      ],
      type: [
        179
      ],
      issuer: [
        1
      ],
      name: [
        1
      ],
      status: [
        180
      ],
      __typename: [
        1
      ]
    },
    WorkspaceNameAndId: {
      displayName: [
        1
      ],
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    FindAvailableSSOIDP: {
      type: [
        179
      ],
      id: [
        3
      ],
      issuer: [
        1
      ],
      name: [
        1
      ],
      status: [
        180
      ],
      workspace: [
        223
      ],
      __typename: [
        1
      ]
    },
    SetupSso: {
      id: [
        3
      ],
      type: [
        179
      ],
      issuer: [
        1
      ],
      name: [
        1
      ],
      status: [
        180
      ],
      __typename: [
        1
      ]
    },
    SSOConnection: {
      type: [
        179
      ],
      id: [
        3
      ],
      issuer: [
        1
      ],
      name: [
        1
      ],
      status: [
        180
      ],
      __typename: [
        1
      ]
    },
    AvailableWorkspace: {
      id: [
        3
      ],
      displayName: [
        1
      ],
      loginToken: [
        1
      ],
      personalInviteToken: [
        1
      ],
      inviteHash: [
        1
      ],
      workspaceUrls: [
        175
      ],
      logo: [
        1
      ],
      sso: [
        226
      ],
      __typename: [
        1
      ]
    },
    AvailableWorkspaces: {
      availableWorkspacesForSignIn: [
        227
      ],
      availableWorkspacesForSignUp: [
        227
      ],
      __typename: [
        1
      ]
    },
    DeletedWorkspaceMember: {
      id: [
        3
      ],
      name: [
        19
      ],
      userEmail: [
        1
      ],
      avatarUrl: [
        1
      ],
      userWorkspaceId: [
        3
      ],
      __typename: [
        1
      ]
    },
    BillingEntitlement: {
      key: [
        231
      ],
      value: [
        6
      ],
      __typename: [
        1
      ]
    },
    BillingEntitlementKey: {},
    DomainRecord: {
      validationType: [
        1
      ],
      type: [
        1
      ],
      status: [
        1
      ],
      key: [
        1
      ],
      value: [
        1
      ],
      __typename: [
        1
      ]
    },
    DomainValidRecords: {
      id: [
        3
      ],
      domain: [
        1
      ],
      records: [
        232
      ],
      __typename: [
        1
      ]
    },
    UpsertRowLevelPermissionPredicatesResult: {
      predicates: [
        13
      ],
      predicateGroups: [
        10
      ],
      __typename: [
        1
      ]
    },
    LogicFunctionLogs: {
      logs: [
        1
      ],
      __typename: [
        1
      ]
    },
    DeleteTwoFactorAuthenticationMethod: {
      success: [
        6
      ],
      __typename: [
        1
      ]
    },
    InitiateTwoFactorAuthenticationProvisioning: {
      uri: [
        1
      ],
      __typename: [
        1
      ]
    },
    VerifyTwoFactorAuthenticationMethod: {
      success: [
        6
      ],
      __typename: [
        1
      ]
    },
    AuthorizeApp: {
      redirectUrl: [
        1
      ],
      __typename: [
        1
      ]
    },
    AuthTokenPair: {
      accessOrWorkspaceAgnosticToken: [
        32
      ],
      refreshToken: [
        32
      ],
      __typename: [
        1
      ]
    },
    AvailableWorkspacesAndAccessTokens: {
      tokens: [
        240
      ],
      availableWorkspaces: [
        228
      ],
      __typename: [
        1
      ]
    },
    EmailPasswordResetLink: {
      success: [
        6
      ],
      __typename: [
        1
      ]
    },
    GetAuthorizationUrlForSSO: {
      authorizationURL: [
        1
      ],
      type: [
        1
      ],
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    InvalidatePassword: {
      success: [
        6
      ],
      __typename: [
        1
      ]
    },
    WorkspaceUrlsAndId: {
      workspaceUrls: [
        175
      ],
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    SignUp: {
      loginToken: [
        32
      ],
      workspace: [
        245
      ],
      __typename: [
        1
      ]
    },
    TransientToken: {
      transientToken: [
        32
      ],
      __typename: [
        1
      ]
    },
    ValidatePasswordResetToken: {
      id: [
        3
      ],
      email: [
        1
      ],
      hasPassword: [
        6
      ],
      __typename: [
        1
      ]
    },
    VerifyEmailAndGetLoginToken: {
      loginToken: [
        32
      ],
      workspaceUrls: [
        175
      ],
      __typename: [
        1
      ]
    },
    ApiKeyToken: {
      token: [
        1
      ],
      __typename: [
        1
      ]
    },
    AuthTokens: {
      tokens: [
        240
      ],
      __typename: [
        1
      ]
    },
    LoginToken: {
      loginToken: [
        32
      ],
      __typename: [
        1
      ]
    },
    CheckUserExist: {
      exists: [
        6
      ],
      availableWorkspacesCount: [
        11
      ],
      isEmailVerified: [
        6
      ],
      __typename: [
        1
      ]
    },
    WorkspaceInviteHashValid: {
      isValid: [
        6
      ],
      __typename: [
        1
      ]
    },
    Impersonate: {
      loginToken: [
        32
      ],
      workspace: [
        245
      ],
      __typename: [
        1
      ]
    },
    UsageTimeSeries: {
      date: [
        1
      ],
      creditsUsed: [
        11
      ],
      __typename: [
        1
      ]
    },
    UsageUserDaily: {
      userWorkspaceId: [
        1
      ],
      dailyUsage: [
        256
      ],
      __typename: [
        1
      ]
    },
    UsageAnalytics: {
      usageByUser: [
        199
      ],
      usageByOperationType: [
        199
      ],
      usageByModel: [
        199
      ],
      timeSeries: [
        256
      ],
      periodStart: [
        4
      ],
      periodEnd: [
        4
      ],
      userDailyUsage: [
        257
      ],
      __typename: [
        1
      ]
    },
    DevelopmentApplication: {
      id: [
        1
      ],
      universalIdentifier: [
        1
      ],
      __typename: [
        1
      ]
    },
    WorkspaceMigration: {
      applicationUniversalIdentifier: [
        1
      ],
      actions: [
        15
      ],
      __typename: [
        1
      ]
    },
    File: {
      id: [
        3
      ],
      path: [
        1
      ],
      size: [
        11
      ],
      createdAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    MarketplaceApp: {
      id: [
        1
      ],
      name: [
        1
      ],
      description: [
        1
      ],
      author: [
        1
      ],
      category: [
        1
      ],
      logo: [
        1
      ],
      sourcePackage: [
        1
      ],
      isFeatured: [
        6
      ],
      __typename: [
        1
      ]
    },
    MarketplaceAppDetail: {
      universalIdentifier: [
        1
      ],
      id: [
        1
      ],
      name: [
        1
      ],
      sourceType: [
        8
      ],
      sourcePackage: [
        1
      ],
      latestAvailableVersion: [
        1
      ],
      isListed: [
        6
      ],
      isFeatured: [
        6
      ],
      manifest: [
        15
      ],
      __typename: [
        1
      ]
    },
    PublicDomain: {
      id: [
        3
      ],
      domain: [
        1
      ],
      isValidated: [
        6
      ],
      applicationId: [
        3
      ],
      createdAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    AutocompleteResult: {
      text: [
        1
      ],
      placeId: [
        1
      ],
      __typename: [
        1
      ]
    },
    Location: {
      lat: [
        11
      ],
      lng: [
        11
      ],
      __typename: [
        1
      ]
    },
    PlaceDetailsResult: {
      street: [
        1
      ],
      state: [
        1
      ],
      postcode: [
        1
      ],
      city: [
        1
      ],
      country: [
        1
      ],
      location: [
        266
      ],
      __typename: [
        1
      ]
    },
    PublicConnectionParametersOutput: {
      host: [
        1
      ],
      port: [
        11
      ],
      username: [
        1
      ],
      secure: [
        6
      ],
      __typename: [
        1
      ]
    },
    PublicImapSmtpCaldavConnectionParameters: {
      IMAP: [
        268
      ],
      SMTP: [
        268
      ],
      CALDAV: [
        268
      ],
      __typename: [
        1
      ]
    },
    ConnectedAccountPublicDTO: {
      id: [
        3
      ],
      handle: [
        1
      ],
      provider: [
        1
      ],
      lastCredentialsRefreshedAt: [
        4
      ],
      authFailedAt: [
        4
      ],
      handleAliases: [
        1
      ],
      scopes: [
        1
      ],
      lastSignedInAt: [
        4
      ],
      userWorkspaceId: [
        3
      ],
      connectionProviderId: [
        3
      ],
      applicationId: [
        3
      ],
      name: [
        1
      ],
      visibility: [
        1
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      connectionParameters: [
        269
      ],
      __typename: [
        1
      ]
    },
    ImapSmtpCaldavPublicConnectionParams: {
      host: [
        1
      ],
      port: [
        11
      ],
      username: [
        1
      ],
      secure: [
        6
      ],
      __typename: [
        1
      ]
    },
    ImapSmtpCaldavPublicConnectionParameters: {
      IMAP: [
        271
      ],
      SMTP: [
        271
      ],
      CALDAV: [
        271
      ],
      __typename: [
        1
      ]
    },
    ConnectedImapSmtpCaldavAccount: {
      id: [
        3
      ],
      handle: [
        1
      ],
      provider: [
        1
      ],
      userWorkspaceId: [
        3
      ],
      connectionParameters: [
        272
      ],
      __typename: [
        1
      ]
    },
    ImapSmtpCaldavConnectionSuccess: {
      success: [
        6
      ],
      connectedAccountId: [
        1
      ],
      __typename: [
        1
      ]
    },
    Webhook: {
      id: [
        3
      ],
      targetUrl: [
        1
      ],
      operations: [
        1
      ],
      description: [
        1
      ],
      secret: [
        1
      ],
      applicationId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    ToolIndexEntry: {
      name: [
        1
      ],
      description: [
        1
      ],
      category: [
        1
      ],
      objectName: [
        1
      ],
      icon: [
        1
      ],
      inputSchema: [
        15
      ],
      __typename: [
        1
      ]
    },
    AgentMessagePart: {
      id: [
        3
      ],
      messageId: [
        3
      ],
      orderIndex: [
        21
      ],
      type: [
        1
      ],
      textContent: [
        1
      ],
      reasoningContent: [
        1
      ],
      toolName: [
        1
      ],
      toolCallId: [
        1
      ],
      toolInput: [
        15
      ],
      toolOutput: [
        15
      ],
      state: [
        1
      ],
      providerExecuted: [
        6
      ],
      errorMessage: [
        1
      ],
      errorDetails: [
        15
      ],
      sourceUrlSourceId: [
        1
      ],
      sourceUrlUrl: [
        1
      ],
      sourceUrlTitle: [
        1
      ],
      sourceDocumentSourceId: [
        1
      ],
      sourceDocumentMediaType: [
        1
      ],
      sourceDocumentTitle: [
        1
      ],
      sourceDocumentFilename: [
        1
      ],
      fileMediaType: [
        1
      ],
      fileFilename: [
        1
      ],
      fileId: [
        3
      ],
      fileUrl: [
        1
      ],
      providerMetadata: [
        15
      ],
      createdAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    ChannelSyncSuccess: {
      success: [
        6
      ],
      __typename: [
        1
      ]
    },
    BarChartSeries: {
      key: [
        1
      ],
      label: [
        1
      ],
      __typename: [
        1
      ]
    },
    BarChartData: {
      data: [
        15
      ],
      indexBy: [
        1
      ],
      keys: [
        1
      ],
      series: [
        279
      ],
      xAxisLabel: [
        1
      ],
      yAxisLabel: [
        1
      ],
      showLegend: [
        6
      ],
      showDataLabels: [
        6
      ],
      layout: [
        104
      ],
      groupMode: [
        103
      ],
      hasTooManyGroups: [
        6
      ],
      formattedToRawLookup: [
        15
      ],
      __typename: [
        1
      ]
    },
    LineChartDataPoint: {
      x: [
        1
      ],
      y: [
        11
      ],
      __typename: [
        1
      ]
    },
    LineChartSeries: {
      id: [
        1
      ],
      label: [
        1
      ],
      data: [
        281
      ],
      __typename: [
        1
      ]
    },
    LineChartData: {
      series: [
        282
      ],
      xAxisLabel: [
        1
      ],
      yAxisLabel: [
        1
      ],
      showLegend: [
        6
      ],
      showDataLabels: [
        6
      ],
      hasTooManyGroups: [
        6
      ],
      formattedToRawLookup: [
        15
      ],
      __typename: [
        1
      ]
    },
    PieChartDataItem: {
      id: [
        1
      ],
      value: [
        11
      ],
      __typename: [
        1
      ]
    },
    PieChartData: {
      data: [
        284
      ],
      showLegend: [
        6
      ],
      showDataLabels: [
        6
      ],
      showCenterMetric: [
        6
      ],
      hasTooManyGroups: [
        6
      ],
      formattedToRawLookup: [
        15
      ],
      __typename: [
        1
      ]
    },
    DuplicatedDashboard: {
      id: [
        3
      ],
      title: [
        1
      ],
      pageLayoutId: [
        3
      ],
      position: [
        11
      ],
      createdAt: [
        1
      ],
      updatedAt: [
        1
      ],
      __typename: [
        1
      ]
    },
    SendEmailOutput: {
      success: [
        6
      ],
      error: [
        1
      ],
      __typename: [
        1
      ]
    },
    EventLogRecord: {
      event: [
        1
      ],
      timestamp: [
        4
      ],
      userId: [
        1
      ],
      properties: [
        15
      ],
      recordId: [
        1
      ],
      objectMetadataId: [
        1
      ],
      isCustom: [
        6
      ],
      __typename: [
        1
      ]
    },
    EventLogPageInfo: {
      endCursor: [
        1
      ],
      hasNextPage: [
        6
      ],
      __typename: [
        1
      ]
    },
    EventLogQueryResult: {
      records: [
        288
      ],
      totalCount: [
        21
      ],
      pageInfo: [
        289
      ],
      __typename: [
        1
      ]
    },
    Skill: {
      id: [
        3
      ],
      name: [
        1
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      description: [
        1
      ],
      content: [
        1
      ],
      isCustom: [
        6
      ],
      isActive: [
        6
      ],
      applicationId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    AgentMessage: {
      id: [
        3
      ],
      threadId: [
        3
      ],
      turnId: [
        3
      ],
      agentId: [
        3
      ],
      role: [
        1
      ],
      status: [
        1
      ],
      parts: [
        277
      ],
      processedAt: [
        4
      ],
      createdAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    AgentChatThread: {
      id: [
        294
      ],
      title: [
        1
      ],
      totalInputTokens: [
        21
      ],
      totalOutputTokens: [
        21
      ],
      contextWindowTokens: [
        21
      ],
      conversationSize: [
        21
      ],
      totalInputCredits: [
        11
      ],
      totalOutputCredits: [
        11
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      deletedAt: [
        4
      ],
      lastMessageAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    ID: {},
    AiSystemPromptSection: {
      title: [
        1
      ],
      content: [
        1
      ],
      estimatedTokenCount: [
        21
      ],
      __typename: [
        1
      ]
    },
    AiSystemPromptPreview: {
      sections: [
        295
      ],
      estimatedTokenCount: [
        21
      ],
      __typename: [
        1
      ]
    },
    ChatStreamCatchupChunks: {
      chunks: [
        15
      ],
      maxSeq: [
        21
      ],
      __typename: [
        1
      ]
    },
    SendChatMessageResult: {
      messageId: [
        1
      ],
      queued: [
        6
      ],
      streamId: [
        1
      ],
      __typename: [
        1
      ]
    },
    AgentChatEvent: {
      threadId: [
        1
      ],
      event: [
        15
      ],
      __typename: [
        1
      ]
    },
    AgentTurnEvaluation: {
      id: [
        3
      ],
      turnId: [
        3
      ],
      score: [
        21
      ],
      comment: [
        1
      ],
      createdAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    AgentTurn: {
      id: [
        3
      ],
      threadId: [
        3
      ],
      agentId: [
        3
      ],
      evaluations: [
        300
      ],
      messages: [
        292
      ],
      createdAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    CalendarChannel: {
      id: [
        3
      ],
      handle: [
        1
      ],
      syncStatus: [
        303
      ],
      syncStage: [
        304
      ],
      visibility: [
        305
      ],
      isContactAutoCreationEnabled: [
        6
      ],
      contactAutoCreationPolicy: [
        306
      ],
      isSyncEnabled: [
        6
      ],
      syncedAt: [
        4
      ],
      syncStageStartedAt: [
        4
      ],
      throttleFailureCount: [
        11
      ],
      connectedAccountId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    CalendarChannelSyncStatus: {},
    CalendarChannelSyncStage: {},
    CalendarChannelVisibility: {},
    CalendarChannelContactAutoCreationPolicy: {},
    MessageChannel: {
      id: [
        3
      ],
      visibility: [
        308
      ],
      handle: [
        1
      ],
      type: [
        309
      ],
      isContactAutoCreationEnabled: [
        6
      ],
      contactAutoCreationPolicy: [
        310
      ],
      messageFolderImportPolicy: [
        311
      ],
      excludeNonProfessionalEmails: [
        6
      ],
      excludeGroupEmails: [
        6
      ],
      pendingGroupEmailsAction: [
        312
      ],
      isSyncEnabled: [
        6
      ],
      syncedAt: [
        4
      ],
      syncStatus: [
        313
      ],
      syncStage: [
        314
      ],
      syncStageStartedAt: [
        4
      ],
      throttleFailureCount: [
        11
      ],
      throttleRetryAfter: [
        4
      ],
      connectedAccountId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      connectedAccount: [
        270
      ],
      __typename: [
        1
      ]
    },
    MessageChannelVisibility: {},
    MessageChannelType: {},
    MessageChannelContactAutoCreationPolicy: {},
    MessageFolderImportPolicy: {},
    MessageChannelPendingGroupEmailsAction: {},
    MessageChannelSyncStatus: {},
    MessageChannelSyncStage: {},
    CreateEmailGroupChannelOutput: {
      messageChannel: [
        307
      ],
      forwardingAddress: [
        1
      ],
      __typename: [
        1
      ]
    },
    MessageFolder: {
      id: [
        3
      ],
      name: [
        1
      ],
      isSentFolder: [
        6
      ],
      isSynced: [
        6
      ],
      parentFolderId: [
        1
      ],
      externalId: [
        1
      ],
      pendingSyncAction: [
        317
      ],
      messageChannelId: [
        3
      ],
      createdAt: [
        4
      ],
      updatedAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    MessageFolderPendingSyncAction: {},
    CollectionHash: {
      collectionName: [
        319
      ],
      hash: [
        1
      ],
      __typename: [
        1
      ]
    },
    AllMetadataName: {},
    MinimalObjectMetadata: {
      id: [
        3
      ],
      nameSingular: [
        1
      ],
      namePlural: [
        1
      ],
      labelSingular: [
        1
      ],
      labelPlural: [
        1
      ],
      icon: [
        1
      ],
      color: [
        1
      ],
      isCustom: [
        6
      ],
      isActive: [
        6
      ],
      isSystem: [
        6
      ],
      isRemote: [
        6
      ],
      __typename: [
        1
      ]
    },
    MinimalView: {
      id: [
        3
      ],
      type: [
        70
      ],
      key: [
        71
      ],
      objectMetadataId: [
        3
      ],
      __typename: [
        1
      ]
    },
    MinimalMetadata: {
      objectMetadataItems: [
        320
      ],
      views: [
        321
      ],
      collectionHashes: [
        318
      ],
      __typename: [
        1
      ]
    },
    Query: {
      navigationMenuItems: [
        162
      ],
      navigationMenuItem: [
        162,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      enterprisePortalSession: [
        1,
        {
          returnUrlPath: [
            1
          ]
        }
      ],
      enterpriseCheckoutSession: [
        1,
        {
          billingInterval: [
            1
          ]
        }
      ],
      enterpriseSubscriptionStatus: [
        128
      ],
      getViewFilterGroups: [
        61,
        {
          viewId: [
            1
          ]
        }
      ],
      getViewFilterGroup: [
        61,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      getViewFilters: [
        63,
        {
          viewId: [
            1
          ]
        }
      ],
      getViewFilter: [
        63,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      getViews: [
        69,
        {
          objectMetadataId: [
            1
          ],
          viewTypes: [
            70,
            "[ViewType!]"
          ]
        }
      ],
      getView: [
        69,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      getViewSorts: [
        66,
        {
          viewId: [
            1
          ]
        }
      ],
      getViewSort: [
        66,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      getViewFields: [
        59,
        {
          viewId: [
            1,
            "String!"
          ]
        }
      ],
      getViewField: [
        59,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      getViewFieldGroups: [
        68,
        {
          viewId: [
            1,
            "String!"
          ]
        }
      ],
      getViewFieldGroup: [
        68,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      apiKeys: [
        2
      ],
      apiKey: [
        2,
        {
          input: [
            324,
            "GetApiKeyInput!"
          ]
        }
      ],
      billingPortalSession: [
        156,
        {
          returnUrlPath: [
            1
          ]
        }
      ],
      listPlans: [
        155
      ],
      getResourceCreditUsage: [
        154
      ],
      findWorkspaceInvitations: [
        159
      ],
      getApprovedAccessDomains: [
        135
      ],
      getPageLayoutTabs: [
        122,
        {
          pageLayoutId: [
            1,
            "String!"
          ]
        }
      ],
      getPageLayoutTab: [
        122,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      getPageLayouts: [
        123,
        {
          objectMetadataId: [
            1
          ],
          pageLayoutType: [
            124
          ]
        }
      ],
      getPageLayout: [
        123,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      getEmailingDomains: [
        131
      ],
      applicationConnectionProviders: [
        126,
        {
          applicationId: [
            3,
            "UUID!"
          ]
        }
      ],
      getPageLayoutWidgets: [
        84,
        {
          pageLayoutTabId: [
            1,
            "String!"
          ]
        }
      ],
      getPageLayoutWidget: [
        84,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      findOneLogicFunction: [
        41,
        {
          input: [
            325,
            "LogicFunctionIdInput!"
          ]
        }
      ],
      findManyLogicFunctions: [
        41
      ],
      getAvailablePackages: [
        15,
        {
          input: [
            325,
            "LogicFunctionIdInput!"
          ]
        }
      ],
      getLogicFunctionSourceCode: [
        1,
        {
          input: [
            325,
            "LogicFunctionIdInput!"
          ]
        }
      ],
      commandMenuItems: [
        35
      ],
      commandMenuItem: [
        35,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      frontComponents: [
        34
      ],
      frontComponent: [
        34,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      objectRecordCounts: [
        214
      ],
      object: [
        55,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      objects: [
        215,
        {
          paging: [
            48,
            "CursorPaging!"
          ],
          filter: [
            50,
            "ObjectFilter!"
          ]
        }
      ],
      index: [
        46,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      indexMetadatas: [
        209,
        {
          paging: [
            48,
            "CursorPaging!"
          ],
          filter: [
            57,
            "IndexFilter!"
          ]
        }
      ],
      findManyAgents: [
        25
      ],
      findOneAgent: [
        25,
        {
          input: [
            326,
            "AgentIdInput!"
          ]
        }
      ],
      getRoles: [
        29
      ],
      getToolIndex: [
        276
      ],
      getToolInputSchema: [
        15,
        {
          toolName: [
            1,
            "String!"
          ]
        }
      ],
      webhooks: [
        275
      ],
      webhook: [
        275,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      field: [
        43,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      fields: [
        219,
        {
          paging: [
            48,
            "CursorPaging!"
          ],
          filter: [
            56,
            "FieldFilter!"
          ]
        }
      ],
      getViewGroups: [
        65,
        {
          viewId: [
            1
          ]
        }
      ],
      getViewGroup: [
        65,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      myMessageFolders: [
        316,
        {
          messageChannelId: [
            3
          ]
        }
      ],
      myMessageChannels: [
        307,
        {
          connectedAccountId: [
            3
          ]
        }
      ],
      myConnectedAccounts: [
        270
      ],
      myCalendarChannels: [
        302,
        {
          connectedAccountId: [
            3
          ]
        }
      ],
      minimalMetadata: [
        322
      ],
      chatThreads: [
        293
      ],
      chatThread: [
        293,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      chatMessages: [
        292,
        {
          threadId: [
            3,
            "UUID!"
          ]
        }
      ],
      chatStreamCatchupChunks: [
        297,
        {
          threadId: [
            3,
            "UUID!"
          ]
        }
      ],
      getAiSystemPromptPreview: [
        296
      ],
      skills: [
        291
      ],
      skill: [
        291,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      agentTurns: [
        301,
        {
          agentId: [
            3,
            "UUID!"
          ]
        }
      ],
      checkUserExists: [
        253,
        {
          email: [
            1,
            "String!"
          ],
          captchaToken: [
            1
          ]
        }
      ],
      checkWorkspaceInviteHashIsValid: [
        254,
        {
          inviteHash: [
            1,
            "String!"
          ]
        }
      ],
      findWorkspaceFromInviteHash: [
        75,
        {
          inviteHash: [
            1,
            "String!"
          ]
        }
      ],
      validatePasswordResetToken: [
        248,
        {
          passwordResetToken: [
            1,
            "String!"
          ]
        }
      ],
      findApplicationRegistrationByClientId: [
        203,
        {
          clientId: [
            1,
            "String!"
          ]
        }
      ],
      findApplicationRegistrationByUniversalIdentifier: [
        7,
        {
          universalIdentifier: [
            1,
            "String!"
          ]
        }
      ],
      findManyApplicationRegistrations: [
        7
      ],
      findOneApplicationRegistration: [
        7,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      findApplicationRegistrationStats: [
        201,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      findApplicationRegistrationVariables: [
        176,
        {
          applicationRegistrationId: [
            1,
            "String!"
          ]
        }
      ],
      applicationRegistrationTarballUrl: [
        1,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      currentUser: [
        78
      ],
      currentWorkspace: [
        75
      ],
      getPublicWorkspaceDataByDomain: [
        183,
        {
          origin: [
            1
          ]
        }
      ],
      getPublicWorkspaceDataById: [
        184,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      findManyApplications: [
        58
      ],
      findOneApplication: [
        58,
        {
          id: [
            3
          ],
          universalIdentifier: [
            3
          ]
        }
      ],
      getSSOIdentityProviders: [
        224
      ],
      eventLogs: [
        290,
        {
          input: [
            327,
            "EventLogQueryInput!"
          ]
        }
      ],
      pieChartData: [
        285,
        {
          input: [
            331,
            "PieChartDataInput!"
          ]
        }
      ],
      lineChartData: [
        283,
        {
          input: [
            332,
            "LineChartDataInput!"
          ]
        }
      ],
      barChartData: [
        280,
        {
          input: [
            333,
            "BarChartDataInput!"
          ]
        }
      ],
      getConnectedImapSmtpCaldavAccount: [
        273,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      getAutoCompleteAddress: [
        265,
        {
          address: [
            1,
            "String!"
          ],
          token: [
            1,
            "String!"
          ],
          country: [
            1
          ],
          isFieldCity: [
            6
          ]
        }
      ],
      getAddressDetails: [
        267,
        {
          placeId: [
            1,
            "String!"
          ],
          token: [
            1,
            "String!"
          ]
        }
      ],
      getUsageAnalytics: [
        258,
        {
          input: [
            334
          ]
        }
      ],
      findManyPublicDomains: [
        264
      ],
      findManyMarketplaceApps: [
        262
      ],
      findMarketplaceAppDetail: [
        263,
        {
          universalIdentifier: [
            1,
            "String!"
          ]
        }
      ],
      __typename: [
        1
      ]
    },
    GetApiKeyInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    LogicFunctionIdInput: {
      id: [
        294
      ],
      __typename: [
        1
      ]
    },
    AgentIdInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    EventLogQueryInput: {
      table: [
        328
      ],
      filters: [
        329
      ],
      first: [
        21
      ],
      after: [
        1
      ],
      __typename: [
        1
      ]
    },
    EventLogTable: {},
    EventLogFiltersInput: {
      eventType: [
        1
      ],
      userWorkspaceId: [
        1
      ],
      dateRange: [
        330
      ],
      recordId: [
        1
      ],
      objectMetadataId: [
        1
      ],
      __typename: [
        1
      ]
    },
    EventLogDateRangeInput: {
      start: [
        4
      ],
      end: [
        4
      ],
      __typename: [
        1
      ]
    },
    PieChartDataInput: {
      objectMetadataId: [
        3
      ],
      configuration: [
        15
      ],
      __typename: [
        1
      ]
    },
    LineChartDataInput: {
      objectMetadataId: [
        3
      ],
      configuration: [
        15
      ],
      __typename: [
        1
      ]
    },
    BarChartDataInput: {
      objectMetadataId: [
        3
      ],
      configuration: [
        15
      ],
      __typename: [
        1
      ]
    },
    UsageAnalyticsInput: {
      periodStart: [
        4
      ],
      periodEnd: [
        4
      ],
      userWorkspaceId: [
        1
      ],
      operationTypes: [
        335
      ],
      __typename: [
        1
      ]
    },
    UsageOperationType: {},
    Mutation: {
      addQueryToEventStream: [
        6,
        {
          input: [
            337,
            "AddQuerySubscriptionInput!"
          ]
        }
      ],
      removeQueryFromEventStream: [
        6,
        {
          input: [
            338,
            "RemoveQueryFromEventStreamInput!"
          ]
        }
      ],
      createManyNavigationMenuItems: [
        162,
        {
          inputs: [
            339,
            "[CreateNavigationMenuItemInput!]!"
          ]
        }
      ],
      createNavigationMenuItem: [
        162,
        {
          input: [
            339,
            "CreateNavigationMenuItemInput!"
          ]
        }
      ],
      updateManyNavigationMenuItems: [
        162,
        {
          inputs: [
            340,
            "[UpdateOneNavigationMenuItemInput!]!"
          ]
        }
      ],
      updateNavigationMenuItem: [
        162,
        {
          input: [
            340,
            "UpdateOneNavigationMenuItemInput!"
          ]
        }
      ],
      deleteManyNavigationMenuItems: [
        162,
        {
          ids: [
            3,
            "[UUID!]!"
          ]
        }
      ],
      deleteNavigationMenuItem: [
        162,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      uploadEmailAttachmentFile: [
        136,
        {
          file: [
            342,
            "Upload!"
          ]
        }
      ],
      refreshEnterpriseValidityToken: [
        6
      ],
      setEnterpriseKey: [
        127,
        {
          enterpriseKey: [
            1,
            "String!"
          ]
        }
      ],
      uploadAiChatFile: [
        136,
        {
          file: [
            342,
            "Upload!"
          ]
        }
      ],
      uploadWorkflowFile: [
        136,
        {
          file: [
            342,
            "Upload!"
          ]
        }
      ],
      uploadWorkspaceLogo: [
        136,
        {
          file: [
            342,
            "Upload!"
          ]
        }
      ],
      uploadWorkspaceMemberProfilePicture: [
        136,
        {
          file: [
            342,
            "Upload!"
          ]
        }
      ],
      uploadFilesFieldFile: [
        136,
        {
          file: [
            342,
            "Upload!"
          ],
          fieldMetadataId: [
            1,
            "String!"
          ]
        }
      ],
      uploadFilesFieldFileByUniversalIdentifier: [
        136,
        {
          file: [
            342,
            "Upload!"
          ],
          fieldMetadataUniversalIdentifier: [
            1,
            "String!"
          ]
        }
      ],
      createViewFilterGroup: [
        61,
        {
          input: [
            343,
            "CreateViewFilterGroupInput!"
          ]
        }
      ],
      updateViewFilterGroup: [
        61,
        {
          id: [
            1,
            "String!"
          ],
          input: [
            344,
            "UpdateViewFilterGroupInput!"
          ]
        }
      ],
      deleteViewFilterGroup: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      destroyViewFilterGroup: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      createViewFilter: [
        63,
        {
          input: [
            345,
            "CreateViewFilterInput!"
          ]
        }
      ],
      updateViewFilter: [
        63,
        {
          input: [
            346,
            "UpdateViewFilterInput!"
          ]
        }
      ],
      deleteViewFilter: [
        63,
        {
          input: [
            348,
            "DeleteViewFilterInput!"
          ]
        }
      ],
      destroyViewFilter: [
        63,
        {
          input: [
            349,
            "DestroyViewFilterInput!"
          ]
        }
      ],
      createView: [
        69,
        {
          input: [
            350,
            "CreateViewInput!"
          ]
        }
      ],
      updateView: [
        69,
        {
          id: [
            1,
            "String!"
          ],
          input: [
            351,
            "UpdateViewInput!"
          ]
        }
      ],
      deleteView: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      destroyView: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      upsertViewWidget: [
        69,
        {
          input: [
            352,
            "UpsertViewWidgetInput!"
          ]
        }
      ],
      createViewSort: [
        66,
        {
          input: [
            357,
            "CreateViewSortInput!"
          ]
        }
      ],
      updateViewSort: [
        66,
        {
          input: [
            358,
            "UpdateViewSortInput!"
          ]
        }
      ],
      deleteViewSort: [
        6,
        {
          input: [
            360,
            "DeleteViewSortInput!"
          ]
        }
      ],
      destroyViewSort: [
        6,
        {
          input: [
            361,
            "DestroyViewSortInput!"
          ]
        }
      ],
      updateViewField: [
        59,
        {
          input: [
            362,
            "UpdateViewFieldInput!"
          ]
        }
      ],
      createViewField: [
        59,
        {
          input: [
            364,
            "CreateViewFieldInput!"
          ]
        }
      ],
      createManyViewFields: [
        59,
        {
          inputs: [
            364,
            "[CreateViewFieldInput!]!"
          ]
        }
      ],
      deleteViewField: [
        59,
        {
          input: [
            365,
            "DeleteViewFieldInput!"
          ]
        }
      ],
      destroyViewField: [
        59,
        {
          input: [
            366,
            "DestroyViewFieldInput!"
          ]
        }
      ],
      updateViewFieldGroup: [
        68,
        {
          input: [
            367,
            "UpdateViewFieldGroupInput!"
          ]
        }
      ],
      createViewFieldGroup: [
        68,
        {
          input: [
            369,
            "CreateViewFieldGroupInput!"
          ]
        }
      ],
      createManyViewFieldGroups: [
        68,
        {
          inputs: [
            369,
            "[CreateViewFieldGroupInput!]!"
          ]
        }
      ],
      deleteViewFieldGroup: [
        68,
        {
          input: [
            370,
            "DeleteViewFieldGroupInput!"
          ]
        }
      ],
      destroyViewFieldGroup: [
        68,
        {
          input: [
            371,
            "DestroyViewFieldGroupInput!"
          ]
        }
      ],
      upsertFieldsWidget: [
        69,
        {
          input: [
            372,
            "UpsertFieldsWidgetInput!"
          ]
        }
      ],
      createApiKey: [
        2,
        {
          input: [
            375,
            "CreateApiKeyInput!"
          ]
        }
      ],
      updateApiKey: [
        2,
        {
          input: [
            376,
            "UpdateApiKeyInput!"
          ]
        }
      ],
      revokeApiKey: [
        2,
        {
          input: [
            377,
            "RevokeApiKeyInput!"
          ]
        }
      ],
      assignRoleToApiKey: [
        6,
        {
          apiKeyId: [
            3,
            "UUID!"
          ],
          roleId: [
            3,
            "UUID!"
          ]
        }
      ],
      createObjectEvent: [
        129,
        {
          event: [
            1,
            "String!"
          ],
          recordId: [
            3,
            "UUID!"
          ],
          objectMetadataId: [
            3,
            "UUID!"
          ],
          properties: [
            15
          ]
        }
      ],
      trackAnalytics: [
        129,
        {
          type: [
            378,
            "AnalyticsType!"
          ],
          name: [
            1
          ],
          event: [
            1
          ],
          properties: [
            15
          ]
        }
      ],
      skipSyncEmailOnboardingStep: [
        158
      ],
      skipBookOnboardingStep: [
        158
      ],
      checkoutSession: [
        156,
        {
          recurringInterval: [
            144,
            "SubscriptionInterval!"
          ],
          plan: [
            140,
            "BillingPlanKey!"
          ],
          requirePaymentMethod: [
            6,
            "Boolean!"
          ],
          successUrlPath: [
            1
          ]
        }
      ],
      switchSubscriptionInterval: [
        157
      ],
      switchBillingPlan: [
        157
      ],
      cancelSwitchBillingPlan: [
        157
      ],
      cancelSwitchBillingInterval: [
        157
      ],
      setResourceCreditSubscriptionPrice: [
        157,
        {
          priceId: [
            1,
            "String!"
          ]
        }
      ],
      endSubscriptionTrialPeriod: [
        153
      ],
      cancelSwitchResourceCreditPrice: [
        157
      ],
      deleteWorkspaceInvitation: [
        1,
        {
          appTokenId: [
            1,
            "String!"
          ]
        }
      ],
      resendWorkspaceInvitation: [
        160,
        {
          appTokenId: [
            1,
            "String!"
          ]
        }
      ],
      sendInvitations: [
        160,
        {
          emails: [
            1,
            "[String!]!"
          ],
          roleId: [
            3
          ]
        }
      ],
      createApprovedAccessDomain: [
        135,
        {
          input: [
            379,
            "CreateApprovedAccessDomainInput!"
          ]
        }
      ],
      deleteApprovedAccessDomain: [
        6,
        {
          input: [
            380,
            "DeleteApprovedAccessDomainInput!"
          ]
        }
      ],
      validateApprovedAccessDomain: [
        135,
        {
          input: [
            381,
            "ValidateApprovedAccessDomainInput!"
          ]
        }
      ],
      createPageLayoutTab: [
        122,
        {
          input: [
            382,
            "CreatePageLayoutTabInput!"
          ]
        }
      ],
      updatePageLayoutTab: [
        122,
        {
          id: [
            1,
            "String!"
          ],
          input: [
            383,
            "UpdatePageLayoutTabInput!"
          ]
        }
      ],
      destroyPageLayoutTab: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      createPageLayout: [
        123,
        {
          input: [
            384,
            "CreatePageLayoutInput!"
          ]
        }
      ],
      updatePageLayout: [
        123,
        {
          id: [
            1,
            "String!"
          ],
          input: [
            385,
            "UpdatePageLayoutInput!"
          ]
        }
      ],
      destroyPageLayout: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      updatePageLayoutWithTabsAndWidgets: [
        123,
        {
          id: [
            1,
            "String!"
          ],
          input: [
            386,
            "UpdatePageLayoutWithTabsInput!"
          ]
        }
      ],
      resetPageLayoutToDefault: [
        123,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      resetPageLayoutWidgetToDefault: [
        84,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      resetPageLayoutTabToDefault: [
        122,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      createEmailingDomain: [
        131,
        {
          domain: [
            1,
            "String!"
          ],
          driver: [
            132,
            "EmailingDomainDriver!"
          ]
        }
      ],
      deleteEmailingDomain: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      verifyEmailingDomain: [
        131,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      sendEmailViaEmailingDomain: [
        134,
        {
          input: [
            390,
            "SendEmailViaDomainInput!"
          ]
        }
      ],
      updateOneApplicationVariable: [
        6,
        {
          key: [
            1,
            "String!"
          ],
          value: [
            1,
            "String!"
          ],
          applicationId: [
            3,
            "UUID!"
          ]
        }
      ],
      createPageLayoutWidget: [
        84,
        {
          input: [
            391,
            "CreatePageLayoutWidgetInput!"
          ]
        }
      ],
      updatePageLayoutWidget: [
        84,
        {
          id: [
            1,
            "String!"
          ],
          input: [
            392,
            "UpdatePageLayoutWidgetInput!"
          ]
        }
      ],
      destroyPageLayoutWidget: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      deleteOneLogicFunction: [
        41,
        {
          input: [
            325,
            "LogicFunctionIdInput!"
          ]
        }
      ],
      createOneLogicFunction: [
        41,
        {
          input: [
            393,
            "CreateLogicFunctionFromSourceInput!"
          ]
        }
      ],
      executeOneLogicFunction: [
        171,
        {
          input: [
            394,
            "ExecuteOneLogicFunctionInput!"
          ]
        }
      ],
      updateOneLogicFunction: [
        6,
        {
          input: [
            395,
            "UpdateLogicFunctionFromSourceInput!"
          ]
        }
      ],
      createCommandMenuItem: [
        35,
        {
          input: [
            397,
            "CreateCommandMenuItemInput!"
          ]
        }
      ],
      updateCommandMenuItem: [
        35,
        {
          input: [
            398,
            "UpdateCommandMenuItemInput!"
          ]
        }
      ],
      deleteCommandMenuItem: [
        35,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      createFrontComponent: [
        34,
        {
          input: [
            399,
            "CreateFrontComponentInput!"
          ]
        }
      ],
      updateFrontComponent: [
        34,
        {
          input: [
            400,
            "UpdateFrontComponentInput!"
          ]
        }
      ],
      deleteFrontComponent: [
        34,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      createOneObject: [
        55,
        {
          input: [
            402,
            "CreateOneObjectInput!"
          ]
        }
      ],
      deleteOneObject: [
        55,
        {
          input: [
            404,
            "DeleteOneObjectInput!"
          ]
        }
      ],
      updateOneObject: [
        55,
        {
          input: [
            405,
            "UpdateOneObjectInput!"
          ]
        }
      ],
      createOneIndex: [
        46,
        {
          input: [
            407,
            "CreateOneIndexInput!"
          ]
        }
      ],
      deleteOneIndex: [
        46,
        {
          input: [
            410,
            "DeleteOneIndexInput!"
          ]
        }
      ],
      createOneAgent: [
        25,
        {
          input: [
            411,
            "CreateAgentInput!"
          ]
        }
      ],
      updateOneAgent: [
        25,
        {
          input: [
            412,
            "UpdateAgentInput!"
          ]
        }
      ],
      deleteOneAgent: [
        25,
        {
          input: [
            326,
            "AgentIdInput!"
          ]
        }
      ],
      updateWorkspaceMemberRole: [
        20,
        {
          workspaceMemberId: [
            3,
            "UUID!"
          ],
          roleId: [
            3,
            "UUID!"
          ]
        }
      ],
      createOneRole: [
        29,
        {
          createRoleInput: [
            413,
            "CreateRoleInput!"
          ]
        }
      ],
      updateOneRole: [
        29,
        {
          updateRoleInput: [
            414,
            "UpdateRoleInput!"
          ]
        }
      ],
      deleteOneRole: [
        1,
        {
          roleId: [
            3,
            "UUID!"
          ]
        }
      ],
      upsertObjectPermissions: [
        16,
        {
          upsertObjectPermissionsInput: [
            416,
            "UpsertObjectPermissionsInput!"
          ]
        }
      ],
      upsertPermissionFlags: [
        27,
        {
          upsertPermissionFlagsInput: [
            418,
            "UpsertPermissionFlagsInput!"
          ]
        }
      ],
      upsertFieldPermissions: [
        26,
        {
          upsertFieldPermissionsInput: [
            419,
            "UpsertFieldPermissionsInput!"
          ]
        }
      ],
      upsertRowLevelPermissionPredicates: [
        234,
        {
          input: [
            421,
            "UpsertRowLevelPermissionPredicatesInput!"
          ]
        }
      ],
      assignRoleToAgent: [
        6,
        {
          agentId: [
            3,
            "UUID!"
          ],
          roleId: [
            3,
            "UUID!"
          ]
        }
      ],
      removeRoleFromAgent: [
        6,
        {
          agentId: [
            3,
            "UUID!"
          ]
        }
      ],
      createWebhook: [
        275,
        {
          input: [
            424,
            "CreateWebhookInput!"
          ]
        }
      ],
      updateWebhook: [
        275,
        {
          input: [
            425,
            "UpdateWebhookInput!"
          ]
        }
      ],
      deleteWebhook: [
        275,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      createOneField: [
        43,
        {
          input: [
            427,
            "CreateOneFieldMetadataInput!"
          ]
        }
      ],
      updateOneField: [
        43,
        {
          input: [
            429,
            "UpdateOneFieldMetadataInput!"
          ]
        }
      ],
      deleteOneField: [
        43,
        {
          input: [
            431,
            "DeleteOneFieldInput!"
          ]
        }
      ],
      createViewGroup: [
        65,
        {
          input: [
            432,
            "CreateViewGroupInput!"
          ]
        }
      ],
      createManyViewGroups: [
        65,
        {
          inputs: [
            432,
            "[CreateViewGroupInput!]!"
          ]
        }
      ],
      updateViewGroup: [
        65,
        {
          input: [
            433,
            "UpdateViewGroupInput!"
          ]
        }
      ],
      updateManyViewGroups: [
        65,
        {
          inputs: [
            433,
            "[UpdateViewGroupInput!]!"
          ]
        }
      ],
      deleteViewGroup: [
        65,
        {
          input: [
            435,
            "DeleteViewGroupInput!"
          ]
        }
      ],
      destroyViewGroup: [
        65,
        {
          input: [
            436,
            "DestroyViewGroupInput!"
          ]
        }
      ],
      updateMessageFolder: [
        316,
        {
          input: [
            437,
            "UpdateMessageFolderInput!"
          ]
        }
      ],
      updateMessageFolders: [
        316,
        {
          input: [
            439,
            "UpdateMessageFoldersInput!"
          ]
        }
      ],
      updateMessageChannel: [
        307,
        {
          input: [
            440,
            "UpdateMessageChannelInput!"
          ]
        }
      ],
      createEmailGroupChannel: [
        315,
        {
          input: [
            442,
            "CreateEmailGroupChannelInput!"
          ]
        }
      ],
      deleteEmailGroupChannel: [
        307,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      deleteConnectedAccount: [
        270,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      updateCalendarChannel: [
        302,
        {
          input: [
            443,
            "UpdateCalendarChannelInput!"
          ]
        }
      ],
      createChatThread: [
        293
      ],
      sendChatMessage: [
        298,
        {
          threadId: [
            3,
            "UUID!"
          ],
          text: [
            1,
            "String!"
          ],
          messageId: [
            3,
            "UUID!"
          ],
          browsingContext: [
            15
          ],
          modelId: [
            1
          ],
          fileAttachments: [
            445,
            "[FileAttachmentInput!]"
          ]
        }
      ],
      stopAgentChatStream: [
        6,
        {
          threadId: [
            3,
            "UUID!"
          ]
        }
      ],
      renameChatThread: [
        293,
        {
          id: [
            3,
            "UUID!"
          ],
          title: [
            1,
            "String!"
          ]
        }
      ],
      archiveChatThread: [
        293,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      unarchiveChatThread: [
        293,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      deleteChatThread: [
        6,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      deleteQueuedChatMessage: [
        6,
        {
          messageId: [
            3,
            "UUID!"
          ]
        }
      ],
      createSkill: [
        291,
        {
          input: [
            446,
            "CreateSkillInput!"
          ]
        }
      ],
      updateSkill: [
        291,
        {
          input: [
            447,
            "UpdateSkillInput!"
          ]
        }
      ],
      deleteSkill: [
        291,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      activateSkill: [
        291,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      deactivateSkill: [
        291,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      evaluateAgentTurn: [
        300,
        {
          turnId: [
            3,
            "UUID!"
          ]
        }
      ],
      runEvaluationInput: [
        301,
        {
          agentId: [
            3,
            "UUID!"
          ],
          input: [
            1,
            "String!"
          ]
        }
      ],
      getAuthorizationUrlForSSO: [
        243,
        {
          input: [
            448,
            "GetAuthorizationUrlForSSOInput!"
          ]
        }
      ],
      getLoginTokenFromCredentials: [
        252,
        {
          email: [
            1,
            "String!"
          ],
          password: [
            1,
            "String!"
          ],
          captchaToken: [
            1
          ],
          locale: [
            1
          ],
          verifyEmailRedirectPath: [
            1
          ],
          origin: [
            1,
            "String!"
          ]
        }
      ],
      signIn: [
        241,
        {
          email: [
            1,
            "String!"
          ],
          password: [
            1,
            "String!"
          ],
          captchaToken: [
            1
          ],
          locale: [
            1
          ],
          verifyEmailRedirectPath: [
            1
          ]
        }
      ],
      verifyEmailAndGetLoginToken: [
        249,
        {
          emailVerificationToken: [
            1,
            "String!"
          ],
          email: [
            1,
            "String!"
          ],
          captchaToken: [
            1
          ],
          origin: [
            1,
            "String!"
          ]
        }
      ],
      verifyEmailAndGetWorkspaceAgnosticToken: [
        241,
        {
          emailVerificationToken: [
            1,
            "String!"
          ],
          email: [
            1,
            "String!"
          ],
          captchaToken: [
            1
          ]
        }
      ],
      getAuthTokensFromOTP: [
        251,
        {
          otp: [
            1,
            "String!"
          ],
          loginToken: [
            1,
            "String!"
          ],
          captchaToken: [
            1
          ],
          origin: [
            1,
            "String!"
          ]
        }
      ],
      signUp: [
        241,
        {
          email: [
            1,
            "String!"
          ],
          password: [
            1,
            "String!"
          ],
          captchaToken: [
            1
          ],
          locale: [
            1
          ],
          verifyEmailRedirectPath: [
            1
          ]
        }
      ],
      signUpInWorkspace: [
        246,
        {
          email: [
            1,
            "String!"
          ],
          password: [
            1,
            "String!"
          ],
          workspaceId: [
            3
          ],
          workspaceInviteHash: [
            1
          ],
          workspacePersonalInviteToken: [
            1
          ],
          captchaToken: [
            1
          ],
          locale: [
            1
          ],
          verifyEmailRedirectPath: [
            1
          ]
        }
      ],
      signUpInNewWorkspace: [
        246
      ],
      generateTransientToken: [
        247
      ],
      getAuthTokensFromLoginToken: [
        251,
        {
          loginToken: [
            1,
            "String!"
          ],
          origin: [
            1,
            "String!"
          ]
        }
      ],
      authorizeApp: [
        239,
        {
          clientId: [
            1,
            "String!"
          ],
          codeChallenge: [
            1
          ],
          redirectUrl: [
            1,
            "String!"
          ],
          state: [
            1
          ],
          scope: [
            1
          ]
        }
      ],
      renewToken: [
        251,
        {
          appToken: [
            1,
            "String!"
          ]
        }
      ],
      generateApiKeyToken: [
        250,
        {
          apiKeyId: [
            3,
            "UUID!"
          ],
          expiresAt: [
            1,
            "String!"
          ]
        }
      ],
      emailPasswordResetLink: [
        242,
        {
          email: [
            1,
            "String!"
          ],
          workspaceId: [
            3
          ]
        }
      ],
      updatePasswordViaResetToken: [
        244,
        {
          passwordResetToken: [
            1,
            "String!"
          ],
          newPassword: [
            1,
            "String!"
          ]
        }
      ],
      createApplicationRegistration: [
        202,
        {
          input: [
            449,
            "CreateApplicationRegistrationInput!"
          ]
        }
      ],
      updateApplicationRegistration: [
        7,
        {
          input: [
            450,
            "UpdateApplicationRegistrationInput!"
          ]
        }
      ],
      deleteApplicationRegistration: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      rotateApplicationRegistrationClientSecret: [
        204,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      createApplicationRegistrationVariable: [
        5,
        {
          input: [
            452,
            "CreateApplicationRegistrationVariableInput!"
          ]
        }
      ],
      updateApplicationRegistrationVariable: [
        5,
        {
          input: [
            453,
            "UpdateApplicationRegistrationVariableInput!"
          ]
        }
      ],
      deleteApplicationRegistrationVariable: [
        6,
        {
          id: [
            1,
            "String!"
          ]
        }
      ],
      uploadAppTarball: [
        7,
        {
          file: [
            342,
            "Upload!"
          ],
          universalIdentifier: [
            1
          ]
        }
      ],
      transferApplicationRegistrationOwnership: [
        7,
        {
          applicationRegistrationId: [
            1,
            "String!"
          ],
          targetWorkspaceSubdomain: [
            1,
            "String!"
          ]
        }
      ],
      initiateOTPProvisioning: [
        237,
        {
          loginToken: [
            1,
            "String!"
          ],
          origin: [
            1,
            "String!"
          ]
        }
      ],
      initiateOTPProvisioningForAuthenticatedUser: [
        237
      ],
      deleteTwoFactorAuthenticationMethod: [
        236,
        {
          twoFactorAuthenticationMethodId: [
            3,
            "UUID!"
          ]
        }
      ],
      verifyTwoFactorAuthenticationMethodForAuthenticatedUser: [
        238,
        {
          otp: [
            1,
            "String!"
          ]
        }
      ],
      deleteUser: [
        78
      ],
      deleteUserFromWorkspace: [
        17,
        {
          workspaceMemberIdToDelete: [
            1,
            "String!"
          ]
        }
      ],
      updateWorkspaceMemberSettings: [
        6,
        {
          input: [
            455,
            "UpdateWorkspaceMemberSettingsInput!"
          ]
        }
      ],
      updateUserEmail: [
        6,
        {
          newEmail: [
            1,
            "String!"
          ],
          verifyEmailRedirectPath: [
            1
          ]
        }
      ],
      resendEmailVerificationToken: [
        220,
        {
          email: [
            1,
            "String!"
          ],
          origin: [
            1,
            "String!"
          ]
        }
      ],
      activateWorkspace: [
        75,
        {
          data: [
            456,
            "ActivateWorkspaceInput!"
          ]
        }
      ],
      updateWorkspace: [
        75,
        {
          data: [
            457,
            "UpdateWorkspaceInput!"
          ]
        }
      ],
      deleteCurrentWorkspace: [
        75
      ],
      checkCustomDomainValidRecords: [
        233
      ],
      runWorkspaceMigration: [
        6,
        {
          workspaceMigration: [
            458,
            "WorkspaceMigrationInput!"
          ]
        }
      ],
      uninstallApplication: [
        6,
        {
          universalIdentifier: [
            1,
            "String!"
          ]
        }
      ],
      createOIDCIdentityProvider: [
        225,
        {
          input: [
            461,
            "SetupOIDCSsoInput!"
          ]
        }
      ],
      createSAMLIdentityProvider: [
        225,
        {
          input: [
            462,
            "SetupSAMLSsoInput!"
          ]
        }
      ],
      deleteSSOIdentityProvider: [
        221,
        {
          input: [
            463,
            "DeleteSsoInput!"
          ]
        }
      ],
      editSSOIdentityProvider: [
        222,
        {
          input: [
            464,
            "EditSsoInput!"
          ]
        }
      ],
      duplicateDashboard: [
        286,
        {
          id: [
            3,
            "UUID!"
          ]
        }
      ],
      impersonate: [
        255,
        {
          userId: [
            3,
            "UUID!"
          ],
          workspaceId: [
            3,
            "UUID!"
          ]
        }
      ],
      sendEmail: [
        287,
        {
          input: [
            465,
            "SendEmailInput!"
          ]
        }
      ],
      startChannelSync: [
        278,
        {
          connectedAccountId: [
            3,
            "UUID!"
          ]
        }
      ],
      saveImapSmtpCaldavAccount: [
        274,
        {
          handle: [
            1,
            "String!"
          ],
          connectionParameters: [
            467,
            "EmailAccountConnectionParameters!"
          ],
          id: [
            3
          ]
        }
      ],
      updateLabPublicFeatureFlag: [
        173,
        {
          input: [
            469,
            "UpdateLabPublicFeatureFlagInput!"
          ]
        }
      ],
      createPublicDomain: [
        264,
        {
          domain: [
            1,
            "String!"
          ],
          applicationId: [
            1
          ]
        }
      ],
      updatePublicDomain: [
        264,
        {
          domain: [
            1,
            "String!"
          ],
          applicationId: [
            1
          ]
        }
      ],
      deletePublicDomain: [
        6,
        {
          domain: [
            1,
            "String!"
          ]
        }
      ],
      checkPublicDomainValidRecords: [
        233,
        {
          domain: [
            1,
            "String!"
          ]
        }
      ],
      createOneAppToken: [
        77,
        {
          input: [
            470,
            "CreateOneAppTokenInput!"
          ]
        }
      ],
      installMarketplaceApp: [
        6,
        {
          universalIdentifier: [
            1,
            "String!"
          ],
          version: [
            1
          ]
        }
      ],
      installApplication: [
        58,
        {
          universalIdentifier: [
            1,
            "String!"
          ],
          version: [
            1
          ]
        }
      ],
      syncMarketplaceCatalog: [
        6
      ],
      createDevelopmentApplication: [
        259,
        {
          universalIdentifier: [
            1,
            "String!"
          ],
          name: [
            1,
            "String!"
          ]
        }
      ],
      generateApplicationToken: [
        33,
        {
          applicationId: [
            3,
            "UUID!"
          ]
        }
      ],
      syncApplication: [
        260,
        {
          manifest: [
            15,
            "JSON!"
          ]
        }
      ],
      uploadApplicationFile: [
        261,
        {
          file: [
            342,
            "Upload!"
          ],
          applicationUniversalIdentifier: [
            1,
            "String!"
          ],
          fileFolder: [
            472,
            "FileFolder!"
          ],
          filePath: [
            1,
            "String!"
          ]
        }
      ],
      upgradeApplication: [
        6,
        {
          appRegistrationId: [
            1,
            "String!"
          ],
          targetVersion: [
            1,
            "String!"
          ]
        }
      ],
      renewApplicationToken: [
        33,
        {
          applicationRefreshToken: [
            1,
            "String!"
          ]
        }
      ],
      __typename: [
        1
      ]
    },
    AddQuerySubscriptionInput: {
      eventStreamId: [
        1
      ],
      queryId: [
        1
      ],
      operationSignature: [
        15
      ],
      __typename: [
        1
      ]
    },
    RemoveQueryFromEventStreamInput: {
      eventStreamId: [
        1
      ],
      queryId: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreateNavigationMenuItemInput: {
      id: [
        3
      ],
      userWorkspaceId: [
        3
      ],
      targetRecordId: [
        3
      ],
      targetObjectMetadataId: [
        3
      ],
      viewId: [
        3
      ],
      type: [
        163
      ],
      name: [
        1
      ],
      link: [
        1
      ],
      icon: [
        1
      ],
      color: [
        1
      ],
      folderId: [
        3
      ],
      pageLayoutId: [
        3
      ],
      position: [
        11
      ],
      __typename: [
        1
      ]
    },
    UpdateOneNavigationMenuItemInput: {
      id: [
        3
      ],
      update: [
        341
      ],
      __typename: [
        1
      ]
    },
    UpdateNavigationMenuItemInput: {
      folderId: [
        3
      ],
      position: [
        11
      ],
      name: [
        1
      ],
      link: [
        1
      ],
      icon: [
        1
      ],
      color: [
        1
      ],
      pageLayoutId: [
        3
      ],
      __typename: [
        1
      ]
    },
    Upload: {},
    CreateViewFilterGroupInput: {
      id: [
        3
      ],
      parentViewFilterGroupId: [
        3
      ],
      logicalOperator: [
        62
      ],
      positionInViewFilterGroup: [
        11
      ],
      viewId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateViewFilterGroupInput: {
      id: [
        3
      ],
      parentViewFilterGroupId: [
        3
      ],
      logicalOperator: [
        62
      ],
      positionInViewFilterGroup: [
        11
      ],
      viewId: [
        3
      ],
      __typename: [
        1
      ]
    },
    CreateViewFilterInput: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      operand: [
        64
      ],
      value: [
        15
      ],
      viewFilterGroupId: [
        3
      ],
      positionInViewFilterGroup: [
        11
      ],
      subFieldName: [
        1
      ],
      relationTargetFieldMetadataId: [
        3
      ],
      viewId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateViewFilterInput: {
      id: [
        3
      ],
      update: [
        347
      ],
      __typename: [
        1
      ]
    },
    UpdateViewFilterInputUpdates: {
      fieldMetadataId: [
        3
      ],
      operand: [
        64
      ],
      value: [
        15
      ],
      viewFilterGroupId: [
        3
      ],
      positionInViewFilterGroup: [
        11
      ],
      subFieldName: [
        1
      ],
      relationTargetFieldMetadataId: [
        3
      ],
      __typename: [
        1
      ]
    },
    DeleteViewFilterInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    DestroyViewFilterInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    CreateViewInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      objectMetadataId: [
        3
      ],
      type: [
        70
      ],
      key: [
        71
      ],
      icon: [
        1
      ],
      position: [
        11
      ],
      isCompact: [
        6
      ],
      shouldHideEmptyGroups: [
        6
      ],
      openRecordIn: [
        72
      ],
      kanbanAggregateOperation: [
        60
      ],
      kanbanAggregateOperationFieldMetadataId: [
        3
      ],
      anyFieldFilterValue: [
        1
      ],
      calendarLayout: [
        73
      ],
      calendarFieldMetadataId: [
        3
      ],
      mainGroupByFieldMetadataId: [
        3
      ],
      visibility: [
        74
      ],
      __typename: [
        1
      ]
    },
    UpdateViewInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      type: [
        70
      ],
      icon: [
        1
      ],
      position: [
        11
      ],
      isCompact: [
        6
      ],
      openRecordIn: [
        72
      ],
      kanbanAggregateOperation: [
        60
      ],
      kanbanAggregateOperationFieldMetadataId: [
        3
      ],
      anyFieldFilterValue: [
        1
      ],
      calendarLayout: [
        73
      ],
      calendarFieldMetadataId: [
        3
      ],
      visibility: [
        74
      ],
      mainGroupByFieldMetadataId: [
        3
      ],
      shouldHideEmptyGroups: [
        6
      ],
      __typename: [
        1
      ]
    },
    UpsertViewWidgetInput: {
      widgetId: [
        3
      ],
      viewFields: [
        353
      ],
      viewFilters: [
        354
      ],
      viewFilterGroups: [
        355
      ],
      viewSorts: [
        356
      ],
      __typename: [
        1
      ]
    },
    UpsertViewWidgetViewFieldInput: {
      viewFieldId: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      isVisible: [
        6
      ],
      position: [
        11
      ],
      size: [
        11
      ],
      __typename: [
        1
      ]
    },
    UpsertViewWidgetViewFilterInput: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      operand: [
        64
      ],
      value: [
        15
      ],
      viewFilterGroupId: [
        3
      ],
      positionInViewFilterGroup: [
        11
      ],
      subFieldName: [
        1
      ],
      relationTargetFieldMetadataId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpsertViewWidgetViewFilterGroupInput: {
      id: [
        3
      ],
      parentViewFilterGroupId: [
        3
      ],
      logicalOperator: [
        62
      ],
      positionInViewFilterGroup: [
        11
      ],
      __typename: [
        1
      ]
    },
    UpsertViewWidgetViewSortInput: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      direction: [
        67
      ],
      __typename: [
        1
      ]
    },
    CreateViewSortInput: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      direction: [
        67
      ],
      subFieldName: [
        1
      ],
      viewId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateViewSortInput: {
      id: [
        3
      ],
      update: [
        359
      ],
      __typename: [
        1
      ]
    },
    UpdateViewSortInputUpdates: {
      direction: [
        67
      ],
      subFieldName: [
        1
      ],
      __typename: [
        1
      ]
    },
    DeleteViewSortInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    DestroyViewSortInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateViewFieldInput: {
      id: [
        3
      ],
      update: [
        363
      ],
      __typename: [
        1
      ]
    },
    UpdateViewFieldInputUpdates: {
      isVisible: [
        6
      ],
      size: [
        11
      ],
      position: [
        11
      ],
      aggregateOperation: [
        60
      ],
      viewFieldGroupId: [
        3
      ],
      __typename: [
        1
      ]
    },
    CreateViewFieldInput: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      viewId: [
        3
      ],
      isVisible: [
        6
      ],
      size: [
        11
      ],
      position: [
        11
      ],
      aggregateOperation: [
        60
      ],
      viewFieldGroupId: [
        3
      ],
      __typename: [
        1
      ]
    },
    DeleteViewFieldInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    DestroyViewFieldInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateViewFieldGroupInput: {
      id: [
        3
      ],
      update: [
        368
      ],
      __typename: [
        1
      ]
    },
    UpdateViewFieldGroupInputUpdates: {
      name: [
        1
      ],
      position: [
        11
      ],
      isVisible: [
        6
      ],
      deletedAt: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreateViewFieldGroupInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      viewId: [
        3
      ],
      position: [
        11
      ],
      isVisible: [
        6
      ],
      __typename: [
        1
      ]
    },
    DeleteViewFieldGroupInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    DestroyViewFieldGroupInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpsertFieldsWidgetInput: {
      widgetId: [
        3
      ],
      groups: [
        373
      ],
      fields: [
        374
      ],
      __typename: [
        1
      ]
    },
    UpsertFieldsWidgetGroupInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      position: [
        11
      ],
      isVisible: [
        6
      ],
      fields: [
        374
      ],
      __typename: [
        1
      ]
    },
    UpsertFieldsWidgetFieldInput: {
      viewFieldId: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      isVisible: [
        6
      ],
      position: [
        11
      ],
      __typename: [
        1
      ]
    },
    CreateApiKeyInput: {
      name: [
        1
      ],
      expiresAt: [
        1
      ],
      revokedAt: [
        1
      ],
      roleId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateApiKeyInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      expiresAt: [
        1
      ],
      revokedAt: [
        1
      ],
      __typename: [
        1
      ]
    },
    RevokeApiKeyInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    AnalyticsType: {},
    CreateApprovedAccessDomainInput: {
      domain: [
        1
      ],
      email: [
        1
      ],
      __typename: [
        1
      ]
    },
    DeleteApprovedAccessDomainInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    ValidateApprovedAccessDomainInput: {
      validationToken: [
        1
      ],
      approvedAccessDomainId: [
        3
      ],
      __typename: [
        1
      ]
    },
    CreatePageLayoutTabInput: {
      title: [
        1
      ],
      position: [
        11
      ],
      pageLayoutId: [
        3
      ],
      layoutMode: [
        88
      ],
      __typename: [
        1
      ]
    },
    UpdatePageLayoutTabInput: {
      title: [
        1
      ],
      position: [
        11
      ],
      icon: [
        1
      ],
      layoutMode: [
        88
      ],
      __typename: [
        1
      ]
    },
    CreatePageLayoutInput: {
      name: [
        1
      ],
      type: [
        124
      ],
      objectMetadataId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdatePageLayoutInput: {
      name: [
        1
      ],
      type: [
        124
      ],
      objectMetadataId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdatePageLayoutWithTabsInput: {
      name: [
        1
      ],
      type: [
        124
      ],
      objectMetadataId: [
        3
      ],
      tabs: [
        387
      ],
      __typename: [
        1
      ]
    },
    UpdatePageLayoutTabWithWidgetsInput: {
      id: [
        3
      ],
      title: [
        1
      ],
      position: [
        11
      ],
      icon: [
        1
      ],
      layoutMode: [
        88
      ],
      widgets: [
        388
      ],
      __typename: [
        1
      ]
    },
    UpdatePageLayoutWidgetWithIdInput: {
      id: [
        3
      ],
      pageLayoutTabId: [
        3
      ],
      title: [
        1
      ],
      type: [
        85
      ],
      objectMetadataId: [
        3
      ],
      gridPosition: [
        389
      ],
      position: [
        15
      ],
      configuration: [
        15
      ],
      conditionalDisplay: [
        15
      ],
      conditionalAvailabilityExpression: [
        1
      ],
      __typename: [
        1
      ]
    },
    GridPositionInput: {
      row: [
        11
      ],
      column: [
        11
      ],
      rowSpan: [
        11
      ],
      columnSpan: [
        11
      ],
      __typename: [
        1
      ]
    },
    SendEmailViaDomainInput: {
      emailingDomainId: [
        1
      ],
      to: [
        1
      ],
      cc: [
        1
      ],
      bcc: [
        1
      ],
      subject: [
        1
      ],
      text: [
        1
      ],
      html: [
        1
      ],
      from: [
        1
      ],
      replyTo: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreatePageLayoutWidgetInput: {
      pageLayoutTabId: [
        3
      ],
      title: [
        1
      ],
      type: [
        85
      ],
      objectMetadataId: [
        3
      ],
      gridPosition: [
        389
      ],
      position: [
        15
      ],
      configuration: [
        15
      ],
      __typename: [
        1
      ]
    },
    UpdatePageLayoutWidgetInput: {
      pageLayoutTabId: [
        3
      ],
      title: [
        1
      ],
      type: [
        85
      ],
      objectMetadataId: [
        3
      ],
      gridPosition: [
        389
      ],
      position: [
        15
      ],
      configuration: [
        15
      ],
      conditionalDisplay: [
        15
      ],
      conditionalAvailabilityExpression: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreateLogicFunctionFromSourceInput: {
      id: [
        3
      ],
      universalIdentifier: [
        3
      ],
      name: [
        1
      ],
      description: [
        1
      ],
      timeoutSeconds: [
        11
      ],
      source: [
        15
      ],
      cronTriggerSettings: [
        15
      ],
      databaseEventTriggerSettings: [
        15
      ],
      httpRouteTriggerSettings: [
        15
      ],
      toolTriggerSettings: [
        15
      ],
      workflowActionTriggerSettings: [
        15
      ],
      __typename: [
        1
      ]
    },
    ExecuteOneLogicFunctionInput: {
      id: [
        3
      ],
      payload: [
        15
      ],
      __typename: [
        1
      ]
    },
    UpdateLogicFunctionFromSourceInput: {
      id: [
        3
      ],
      update: [
        396
      ],
      __typename: [
        1
      ]
    },
    UpdateLogicFunctionFromSourceInputUpdates: {
      name: [
        1
      ],
      description: [
        1
      ],
      timeoutSeconds: [
        11
      ],
      sourceHandlerCode: [
        1
      ],
      handlerName: [
        1
      ],
      sourceHandlerPath: [
        1
      ],
      cronTriggerSettings: [
        15
      ],
      databaseEventTriggerSettings: [
        15
      ],
      httpRouteTriggerSettings: [
        15
      ],
      toolTriggerSettings: [
        15
      ],
      workflowActionTriggerSettings: [
        15
      ],
      __typename: [
        1
      ]
    },
    CreateCommandMenuItemInput: {
      workflowVersionId: [
        3
      ],
      frontComponentId: [
        3
      ],
      engineComponentKey: [
        36
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      shortLabel: [
        1
      ],
      position: [
        11
      ],
      isPinned: [
        6
      ],
      availabilityType: [
        37
      ],
      hotKeys: [
        1
      ],
      conditionalAvailabilityExpression: [
        1
      ],
      availabilityObjectMetadataId: [
        3
      ],
      payload: [
        15
      ],
      pageLayoutId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateCommandMenuItemInput: {
      id: [
        3
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      shortLabel: [
        1
      ],
      position: [
        11
      ],
      isPinned: [
        6
      ],
      availabilityType: [
        37
      ],
      availabilityObjectMetadataId: [
        3
      ],
      engineComponentKey: [
        36
      ],
      hotKeys: [
        1
      ],
      pageLayoutId: [
        3
      ],
      __typename: [
        1
      ]
    },
    CreateFrontComponentInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      description: [
        1
      ],
      sourceComponentPath: [
        1
      ],
      builtComponentPath: [
        1
      ],
      componentName: [
        1
      ],
      builtComponentChecksum: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpdateFrontComponentInput: {
      id: [
        3
      ],
      update: [
        401
      ],
      __typename: [
        1
      ]
    },
    UpdateFrontComponentInputUpdates: {
      name: [
        1
      ],
      description: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreateOneObjectInput: {
      object: [
        403
      ],
      __typename: [
        1
      ]
    },
    CreateObjectInput: {
      nameSingular: [
        1
      ],
      namePlural: [
        1
      ],
      labelSingular: [
        1
      ],
      labelPlural: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      shortcut: [
        1
      ],
      color: [
        1
      ],
      skipNameField: [
        6
      ],
      isRemote: [
        6
      ],
      primaryKeyColumnType: [
        1
      ],
      primaryKeyFieldMetadataSettings: [
        15
      ],
      isLabelSyncedWithName: [
        6
      ],
      __typename: [
        1
      ]
    },
    DeleteOneObjectInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateOneObjectInput: {
      update: [
        406
      ],
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateObjectPayload: {
      labelSingular: [
        1
      ],
      labelPlural: [
        1
      ],
      nameSingular: [
        1
      ],
      namePlural: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      shortcut: [
        1
      ],
      color: [
        1
      ],
      isActive: [
        6
      ],
      labelIdentifierFieldMetadataId: [
        3
      ],
      imageIdentifierFieldMetadataId: [
        3
      ],
      isLabelSyncedWithName: [
        6
      ],
      isSearchable: [
        6
      ],
      __typename: [
        1
      ]
    },
    CreateOneIndexInput: {
      index: [
        408
      ],
      __typename: [
        1
      ]
    },
    CreateIndexInput: {
      objectMetadataId: [
        3
      ],
      fields: [
        409
      ],
      indexType: [
        47
      ],
      __typename: [
        1
      ]
    },
    CreateIndexFieldInput: {
      fieldMetadataId: [
        3
      ],
      subFieldName: [
        1
      ],
      __typename: [
        1
      ]
    },
    DeleteOneIndexInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    CreateAgentInput: {
      name: [
        1
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      description: [
        1
      ],
      prompt: [
        1
      ],
      modelId: [
        1
      ],
      roleId: [
        3
      ],
      responseFormat: [
        15
      ],
      modelConfiguration: [
        15
      ],
      evaluationInputs: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpdateAgentInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      description: [
        1
      ],
      prompt: [
        1
      ],
      modelId: [
        1
      ],
      roleId: [
        3
      ],
      responseFormat: [
        15
      ],
      modelConfiguration: [
        15
      ],
      evaluationInputs: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreateRoleInput: {
      id: [
        1
      ],
      label: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      canUpdateAllSettings: [
        6
      ],
      canAccessAllTools: [
        6
      ],
      canReadAllObjectRecords: [
        6
      ],
      canUpdateAllObjectRecords: [
        6
      ],
      canSoftDeleteAllObjectRecords: [
        6
      ],
      canDestroyAllObjectRecords: [
        6
      ],
      canBeAssignedToUsers: [
        6
      ],
      canBeAssignedToAgents: [
        6
      ],
      canBeAssignedToApiKeys: [
        6
      ],
      __typename: [
        1
      ]
    },
    UpdateRoleInput: {
      update: [
        415
      ],
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateRolePayload: {
      label: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      canUpdateAllSettings: [
        6
      ],
      canAccessAllTools: [
        6
      ],
      canReadAllObjectRecords: [
        6
      ],
      canUpdateAllObjectRecords: [
        6
      ],
      canSoftDeleteAllObjectRecords: [
        6
      ],
      canDestroyAllObjectRecords: [
        6
      ],
      canBeAssignedToUsers: [
        6
      ],
      canBeAssignedToAgents: [
        6
      ],
      canBeAssignedToApiKeys: [
        6
      ],
      __typename: [
        1
      ]
    },
    UpsertObjectPermissionsInput: {
      roleId: [
        3
      ],
      objectPermissions: [
        417
      ],
      __typename: [
        1
      ]
    },
    ObjectPermissionInput: {
      objectMetadataId: [
        3
      ],
      canReadObjectRecords: [
        6
      ],
      canUpdateObjectRecords: [
        6
      ],
      canSoftDeleteObjectRecords: [
        6
      ],
      canDestroyObjectRecords: [
        6
      ],
      __typename: [
        1
      ]
    },
    UpsertPermissionFlagsInput: {
      roleId: [
        3
      ],
      permissionFlagKeys: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpsertFieldPermissionsInput: {
      roleId: [
        3
      ],
      fieldPermissions: [
        420
      ],
      __typename: [
        1
      ]
    },
    FieldPermissionInput: {
      objectMetadataId: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      canReadFieldValue: [
        6
      ],
      canUpdateFieldValue: [
        6
      ],
      __typename: [
        1
      ]
    },
    UpsertRowLevelPermissionPredicatesInput: {
      roleId: [
        3
      ],
      objectMetadataId: [
        3
      ],
      predicates: [
        422
      ],
      predicateGroups: [
        423
      ],
      __typename: [
        1
      ]
    },
    RowLevelPermissionPredicateInput: {
      id: [
        3
      ],
      fieldMetadataId: [
        3
      ],
      operand: [
        14
      ],
      value: [
        15
      ],
      subFieldName: [
        1
      ],
      workspaceMemberFieldMetadataId: [
        1
      ],
      workspaceMemberSubFieldName: [
        1
      ],
      rowLevelPermissionPredicateGroupId: [
        3
      ],
      positionInRowLevelPermissionPredicateGroup: [
        11
      ],
      __typename: [
        1
      ]
    },
    RowLevelPermissionPredicateGroupInput: {
      id: [
        3
      ],
      objectMetadataId: [
        3
      ],
      parentRowLevelPermissionPredicateGroupId: [
        3
      ],
      logicalOperator: [
        12
      ],
      positionInRowLevelPermissionPredicateGroup: [
        11
      ],
      __typename: [
        1
      ]
    },
    CreateWebhookInput: {
      id: [
        3
      ],
      targetUrl: [
        1
      ],
      operations: [
        1
      ],
      description: [
        1
      ],
      secret: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpdateWebhookInput: {
      id: [
        3
      ],
      update: [
        426
      ],
      __typename: [
        1
      ]
    },
    UpdateWebhookInputUpdates: {
      targetUrl: [
        1
      ],
      operations: [
        1
      ],
      description: [
        1
      ],
      secret: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreateOneFieldMetadataInput: {
      field: [
        428
      ],
      __typename: [
        1
      ]
    },
    CreateFieldInput: {
      type: [
        44
      ],
      name: [
        1
      ],
      label: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      isCustom: [
        6
      ],
      isActive: [
        6
      ],
      isSystem: [
        6
      ],
      isUIReadOnly: [
        6
      ],
      isNullable: [
        6
      ],
      isUnique: [
        6
      ],
      defaultValue: [
        15
      ],
      options: [
        15
      ],
      settings: [
        15
      ],
      objectMetadataId: [
        3
      ],
      isLabelSyncedWithName: [
        6
      ],
      isRemoteCreation: [
        6
      ],
      relationCreationPayload: [
        15
      ],
      morphRelationsCreationPayload: [
        15
      ],
      __typename: [
        1
      ]
    },
    UpdateOneFieldMetadataInput: {
      id: [
        3
      ],
      update: [
        430
      ],
      __typename: [
        1
      ]
    },
    UpdateFieldInput: {
      universalIdentifier: [
        1
      ],
      name: [
        1
      ],
      label: [
        1
      ],
      description: [
        1
      ],
      icon: [
        1
      ],
      isActive: [
        6
      ],
      isSystem: [
        6
      ],
      isUIReadOnly: [
        6
      ],
      isNullable: [
        6
      ],
      isUnique: [
        6
      ],
      defaultValue: [
        15
      ],
      options: [
        15
      ],
      settings: [
        15
      ],
      objectMetadataId: [
        3
      ],
      isLabelSyncedWithName: [
        6
      ],
      morphRelationsUpdatePayload: [
        15
      ],
      __typename: [
        1
      ]
    },
    DeleteOneFieldInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    CreateViewGroupInput: {
      id: [
        3
      ],
      isVisible: [
        6
      ],
      fieldValue: [
        1
      ],
      position: [
        11
      ],
      viewId: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateViewGroupInput: {
      id: [
        3
      ],
      update: [
        434
      ],
      __typename: [
        1
      ]
    },
    UpdateViewGroupInputUpdates: {
      fieldMetadataId: [
        3
      ],
      isVisible: [
        6
      ],
      fieldValue: [
        1
      ],
      position: [
        11
      ],
      __typename: [
        1
      ]
    },
    DeleteViewGroupInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    DestroyViewGroupInput: {
      id: [
        3
      ],
      __typename: [
        1
      ]
    },
    UpdateMessageFolderInput: {
      id: [
        3
      ],
      update: [
        438
      ],
      __typename: [
        1
      ]
    },
    UpdateMessageFolderInputUpdates: {
      isSynced: [
        6
      ],
      __typename: [
        1
      ]
    },
    UpdateMessageFoldersInput: {
      ids: [
        3
      ],
      update: [
        438
      ],
      __typename: [
        1
      ]
    },
    UpdateMessageChannelInput: {
      id: [
        3
      ],
      update: [
        441
      ],
      __typename: [
        1
      ]
    },
    UpdateMessageChannelInputUpdates: {
      visibility: [
        308
      ],
      isContactAutoCreationEnabled: [
        6
      ],
      contactAutoCreationPolicy: [
        310
      ],
      messageFolderImportPolicy: [
        311
      ],
      isSyncEnabled: [
        6
      ],
      excludeNonProfessionalEmails: [
        6
      ],
      excludeGroupEmails: [
        6
      ],
      __typename: [
        1
      ]
    },
    CreateEmailGroupChannelInput: {
      handle: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpdateCalendarChannelInput: {
      id: [
        3
      ],
      update: [
        444
      ],
      __typename: [
        1
      ]
    },
    UpdateCalendarChannelInputUpdates: {
      visibility: [
        305
      ],
      isContactAutoCreationEnabled: [
        6
      ],
      contactAutoCreationPolicy: [
        306
      ],
      isSyncEnabled: [
        6
      ],
      __typename: [
        1
      ]
    },
    FileAttachmentInput: {
      id: [
        3
      ],
      filename: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreateSkillInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      description: [
        1
      ],
      content: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpdateSkillInput: {
      id: [
        3
      ],
      name: [
        1
      ],
      label: [
        1
      ],
      icon: [
        1
      ],
      description: [
        1
      ],
      content: [
        1
      ],
      isActive: [
        6
      ],
      __typename: [
        1
      ]
    },
    GetAuthorizationUrlForSSOInput: {
      identityProviderId: [
        3
      ],
      workspaceInviteHash: [
        1
      ],
      __typename: [
        1
      ]
    },
    CreateApplicationRegistrationInput: {
      name: [
        1
      ],
      universalIdentifier: [
        1
      ],
      oAuthRedirectUris: [
        1
      ],
      oAuthScopes: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpdateApplicationRegistrationInput: {
      id: [
        1
      ],
      update: [
        451
      ],
      __typename: [
        1
      ]
    },
    UpdateApplicationRegistrationPayload: {
      name: [
        1
      ],
      oAuthRedirectUris: [
        1
      ],
      oAuthScopes: [
        1
      ],
      isListed: [
        6
      ],
      __typename: [
        1
      ]
    },
    CreateApplicationRegistrationVariableInput: {
      applicationRegistrationId: [
        1
      ],
      key: [
        1
      ],
      value: [
        1
      ],
      description: [
        1
      ],
      isSecret: [
        6
      ],
      __typename: [
        1
      ]
    },
    UpdateApplicationRegistrationVariableInput: {
      id: [
        1
      ],
      update: [
        454
      ],
      __typename: [
        1
      ]
    },
    UpdateApplicationRegistrationVariablePayload: {
      value: [
        1
      ],
      resetValue: [
        6
      ],
      description: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpdateWorkspaceMemberSettingsInput: {
      workspaceMemberId: [
        3
      ],
      update: [
        15
      ],
      __typename: [
        1
      ]
    },
    ActivateWorkspaceInput: {
      displayName: [
        1
      ],
      __typename: [
        1
      ]
    },
    UpdateWorkspaceInput: {
      subdomain: [
        1
      ],
      customDomain: [
        1
      ],
      displayName: [
        1
      ],
      logo: [
        1
      ],
      inviteHash: [
        1
      ],
      isPublicInviteLinkEnabled: [
        6
      ],
      allowImpersonation: [
        6
      ],
      isGoogleAuthEnabled: [
        6
      ],
      isMicrosoftAuthEnabled: [
        6
      ],
      isPasswordAuthEnabled: [
        6
      ],
      isGoogleAuthBypassEnabled: [
        6
      ],
      isMicrosoftAuthBypassEnabled: [
        6
      ],
      isPasswordAuthBypassEnabled: [
        6
      ],
      defaultRoleId: [
        3
      ],
      isTwoFactorAuthenticationEnforced: [
        6
      ],
      trashRetentionDays: [
        11
      ],
      eventLogRetentionDays: [
        11
      ],
      fastModel: [
        1
      ],
      smartModel: [
        1
      ],
      aiAdditionalInstructions: [
        1
      ],
      editableProfileFields: [
        1
      ],
      enabledAiModelIds: [
        1
      ],
      useRecommendedModels: [
        6
      ],
      isInternalMessagesImportEnabled: [
        6
      ],
      __typename: [
        1
      ]
    },
    WorkspaceMigrationInput: {
      actions: [
        459
      ],
      __typename: [
        1
      ]
    },
    WorkspaceMigrationDeleteActionInput: {
      type: [
        460
      ],
      metadataName: [
        319
      ],
      universalIdentifier: [
        1
      ],
      __typename: [
        1
      ]
    },
    WorkspaceMigrationActionType: {},
    SetupOIDCSsoInput: {
      name: [
        1
      ],
      issuer: [
        1
      ],
      clientID: [
        1
      ],
      clientSecret: [
        1
      ],
      __typename: [
        1
      ]
    },
    SetupSAMLSsoInput: {
      name: [
        1
      ],
      issuer: [
        1
      ],
      id: [
        3
      ],
      ssoURL: [
        1
      ],
      certificate: [
        1
      ],
      fingerprint: [
        1
      ],
      __typename: [
        1
      ]
    },
    DeleteSsoInput: {
      identityProviderId: [
        3
      ],
      __typename: [
        1
      ]
    },
    EditSsoInput: {
      id: [
        3
      ],
      status: [
        180
      ],
      __typename: [
        1
      ]
    },
    SendEmailInput: {
      connectedAccountId: [
        1
      ],
      to: [
        1
      ],
      cc: [
        1
      ],
      bcc: [
        1
      ],
      subject: [
        1
      ],
      body: [
        1
      ],
      inReplyTo: [
        1
      ],
      files: [
        466
      ],
      __typename: [
        1
      ]
    },
    SendEmailAttachmentInput: {
      id: [
        1
      ],
      name: [
        1
      ],
      __typename: [
        1
      ]
    },
    EmailAccountConnectionParameters: {
      IMAP: [
        468
      ],
      SMTP: [
        468
      ],
      CALDAV: [
        468
      ],
      __typename: [
        1
      ]
    },
    ConnectionParametersInput: {
      host: [
        1
      ],
      port: [
        11
      ],
      username: [
        1
      ],
      password: [
        1
      ],
      secure: [
        6
      ],
      __typename: [
        1
      ]
    },
    UpdateLabPublicFeatureFlagInput: {
      publicFeatureFlag: [
        1
      ],
      value: [
        6
      ],
      __typename: [
        1
      ]
    },
    CreateOneAppTokenInput: {
      appToken: [
        471
      ],
      __typename: [
        1
      ]
    },
    CreateAppTokenInput: {
      expiresAt: [
        4
      ],
      __typename: [
        1
      ]
    },
    FileFolder: {},
    Subscription: {
      onEventSubscription: [
        170,
        {
          eventStreamId: [
            1,
            "String!"
          ]
        }
      ],
      logicFunctionLogs: [
        235,
        {
          input: [
            474,
            "LogicFunctionLogsInput!"
          ]
        }
      ],
      onAgentChatEvent: [
        299,
        {
          threadId: [
            3,
            "UUID!"
          ]
        }
      ],
      __typename: [
        1
      ]
    },
    LogicFunctionLogsInput: {
      applicationId: [
        3
      ],
      applicationUniversalIdentifier: [
        3
      ],
      name: [
        1
      ],
      id: [
        3
      ],
      universalIdentifier: [
        3
      ],
      __typename: [
        1
      ]
    }
  }
}, D = ["BillingLicensedProduct", "BillingMeteredProduct"], L = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingProductDTO"');
  return D.includes(e.__typename);
}, b = ["ApiKey"], M = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApiKey"');
  return b.includes(e.__typename);
}, N = ["ApplicationRegistrationVariable"], v = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationRegistrationVariable"');
  return N.includes(e.__typename);
}, U = ["ApplicationRegistration"], k = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationRegistration"');
  return U.includes(e.__typename);
}, V = ["TwoFactorAuthenticationMethodSummary"], W = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isTwoFactorAuthenticationMethodSummary"');
  return V.includes(e.__typename);
}, G = ["RowLevelPermissionPredicateGroup"], B = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRowLevelPermissionPredicateGroup"');
  return G.includes(e.__typename);
}, x = ["RowLevelPermissionPredicate"], K = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRowLevelPermissionPredicate"');
  return x.includes(e.__typename);
}, H = ["ObjectPermission"], Y = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectPermission"');
  return H.includes(e.__typename);
}, Q = ["UserWorkspace"], q = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isUserWorkspace"');
  return Q.includes(e.__typename);
}, z = ["FullName"], X = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFullName"');
  return z.includes(e.__typename);
}, $ = ["WorkspaceMember"], J = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkspaceMember"');
  return $.includes(e.__typename);
}, Z = ["Agent"], j = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAgent"');
  return Z.includes(e.__typename);
}, ee = ["FieldPermission"], te = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFieldPermission"');
  return ee.includes(e.__typename);
}, ie = ["RolePermissionFlag"], ne = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRolePermissionFlag"');
  return ie.includes(e.__typename);
}, ae = ["ApiKeyForRole"], se = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApiKeyForRole"');
  return ae.includes(e.__typename);
}, oe = ["Role"], re = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRole"');
  return oe.includes(e.__typename);
}, pe = ["ApplicationRegistrationSummary"], le = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationRegistrationSummary"');
  return pe.includes(e.__typename);
}, _e = ["ApplicationVariable"], de = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationVariable"');
  return _e.includes(e.__typename);
}, ce = ["AuthToken"], ue = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAuthToken"');
  return ce.includes(e.__typename);
}, me = ["ApplicationTokenPair"], ye = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationTokenPair"');
  return me.includes(e.__typename);
}, ge = ["FrontComponent"], Ee = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFrontComponent"');
  return ge.includes(e.__typename);
}, Ie = ["CommandMenuItem"], Ae = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCommandMenuItem"');
  return Ie.includes(e.__typename);
}, Te = ["PathCommandMenuItemPayload", "ObjectMetadataCommandMenuItemPayload"], Se = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCommandMenuItemPayload"');
  return Te.includes(e.__typename);
}, Ce = ["PathCommandMenuItemPayload"], Oe = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPathCommandMenuItemPayload"');
  return Ce.includes(e.__typename);
}, Re = ["ObjectMetadataCommandMenuItemPayload"], he = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectMetadataCommandMenuItemPayload"');
  return Re.includes(e.__typename);
}, we = ["LogicFunction"], fe = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLogicFunction"');
  return we.includes(e.__typename);
}, Pe = ["StandardOverrides"], Fe = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isStandardOverrides"');
  return Pe.includes(e.__typename);
}, De = ["Field"], Le = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isField"');
  return De.includes(e.__typename);
}, be = ["IndexField"], Me = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isIndexField"');
  return be.includes(e.__typename);
}, Ne = ["Index"], ve = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isIndex"');
  return Ne.includes(e.__typename);
}, Ue = ["ObjectStandardOverrides"], ke = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectStandardOverrides"');
  return Ue.includes(e.__typename);
}, Ve = ["Object"], We = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObject"');
  return Ve.includes(e.__typename);
}, Ge = ["Application"], Be = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplication"');
  return Ge.includes(e.__typename);
}, xe = ["ViewField"], Ke = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isViewField"');
  return xe.includes(e.__typename);
}, He = ["ViewFilterGroup"], Ye = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isViewFilterGroup"');
  return He.includes(e.__typename);
}, Qe = ["ViewFilter"], qe = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isViewFilter"');
  return Qe.includes(e.__typename);
}, ze = ["ViewGroup"], Xe = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isViewGroup"');
  return ze.includes(e.__typename);
}, $e = ["ViewSort"], Je = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isViewSort"');
  return $e.includes(e.__typename);
}, Ze = ["ViewFieldGroup"], je = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isViewFieldGroup"');
  return Ze.includes(e.__typename);
}, et = ["View"], tt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isView"');
  return et.includes(e.__typename);
}, it = ["Workspace"], nt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkspace"');
  return it.includes(e.__typename);
}, at = ["AppToken"], st = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAppToken"');
  return at.includes(e.__typename);
}, ot = ["User"], rt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isUser"');
  return ot.includes(e.__typename);
}, pt = ["RatioAggregateConfig"], lt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRatioAggregateConfig"');
  return pt.includes(e.__typename);
}, _t = ["RichTextBody"], dt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRichTextBody"');
  return _t.includes(e.__typename);
}, ct = ["GridPosition"], ut = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isGridPosition"');
  return ct.includes(e.__typename);
}, mt = ["PageLayoutWidget"], yt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPageLayoutWidget"');
  return mt.includes(e.__typename);
}, gt = ["PageLayoutWidgetGridPosition", "PageLayoutWidgetVerticalListPosition", "PageLayoutWidgetCanvasPosition"], Et = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPageLayoutWidgetPosition"');
  return gt.includes(e.__typename);
}, It = ["PageLayoutWidgetGridPosition"], At = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPageLayoutWidgetGridPosition"');
  return It.includes(e.__typename);
}, Tt = ["PageLayoutWidgetVerticalListPosition"], St = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPageLayoutWidgetVerticalListPosition"');
  return Tt.includes(e.__typename);
}, Ct = ["PageLayoutWidgetCanvasPosition"], Ot = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPageLayoutWidgetCanvasPosition"');
  return Ct.includes(e.__typename);
}, Rt = ["AggregateChartConfiguration", "StandaloneRichTextConfiguration", "PieChartConfiguration", "LineChartConfiguration", "IframeConfiguration", "GaugeChartConfiguration", "BarChartConfiguration", "CalendarConfiguration", "FrontComponentConfiguration", "EmailsConfiguration", "EmailThreadConfiguration", "FieldConfiguration", "FieldRichTextConfiguration", "FieldsConfiguration", "FilesConfiguration", "NotesConfiguration", "TasksConfiguration", "TimelineConfiguration", "ViewConfiguration", "RecordTableConfiguration", "WorkflowConfiguration", "WorkflowRunConfiguration", "WorkflowVersionConfiguration"], ht = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWidgetConfiguration"');
  return Rt.includes(e.__typename);
}, wt = ["AggregateChartConfiguration"], ft = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAggregateChartConfiguration"');
  return wt.includes(e.__typename);
}, Pt = ["StandaloneRichTextConfiguration"], Ft = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isStandaloneRichTextConfiguration"');
  return Pt.includes(e.__typename);
}, Dt = ["PieChartConfiguration"], Lt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPieChartConfiguration"');
  return Dt.includes(e.__typename);
}, bt = ["LineChartConfiguration"], Mt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLineChartConfiguration"');
  return bt.includes(e.__typename);
}, Nt = ["IframeConfiguration"], vt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isIframeConfiguration"');
  return Nt.includes(e.__typename);
}, Ut = ["GaugeChartConfiguration"], kt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isGaugeChartConfiguration"');
  return Ut.includes(e.__typename);
}, Vt = ["BarChartConfiguration"], Wt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBarChartConfiguration"');
  return Vt.includes(e.__typename);
}, Gt = ["CalendarConfiguration"], Bt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCalendarConfiguration"');
  return Gt.includes(e.__typename);
}, xt = ["FrontComponentConfiguration"], Kt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFrontComponentConfiguration"');
  return xt.includes(e.__typename);
}, Ht = ["EmailsConfiguration"], Yt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEmailsConfiguration"');
  return Ht.includes(e.__typename);
}, Qt = ["EmailThreadConfiguration"], qt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEmailThreadConfiguration"');
  return Qt.includes(e.__typename);
}, zt = ["FieldConfiguration"], Xt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFieldConfiguration"');
  return zt.includes(e.__typename);
}, $t = ["FieldRichTextConfiguration"], Jt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFieldRichTextConfiguration"');
  return $t.includes(e.__typename);
}, Zt = ["FieldsConfiguration"], jt = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFieldsConfiguration"');
  return Zt.includes(e.__typename);
}, ei = ["FilesConfiguration"], ti = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFilesConfiguration"');
  return ei.includes(e.__typename);
}, ii = ["NotesConfiguration"], ni = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isNotesConfiguration"');
  return ii.includes(e.__typename);
}, ai = ["TasksConfiguration"], si = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isTasksConfiguration"');
  return ai.includes(e.__typename);
}, oi = ["TimelineConfiguration"], ri = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isTimelineConfiguration"');
  return oi.includes(e.__typename);
}, pi = ["ViewConfiguration"], li = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isViewConfiguration"');
  return pi.includes(e.__typename);
}, _i = ["RecordTableConfiguration"], di = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRecordTableConfiguration"');
  return _i.includes(e.__typename);
}, ci = ["WorkflowConfiguration"], ui = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkflowConfiguration"');
  return ci.includes(e.__typename);
}, mi = ["WorkflowRunConfiguration"], yi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkflowRunConfiguration"');
  return mi.includes(e.__typename);
}, gi = ["WorkflowVersionConfiguration"], Ei = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkflowVersionConfiguration"');
  return gi.includes(e.__typename);
}, Ii = ["PageLayoutTab"], Ai = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPageLayoutTab"');
  return Ii.includes(e.__typename);
}, Ti = ["PageLayout"], Si = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPageLayout"');
  return Ti.includes(e.__typename);
}, Ci = ["ApplicationConnectionProviderOAuthConfig"], Oi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationConnectionProviderOAuthConfig"');
  return Ci.includes(e.__typename);
}, Ri = ["ApplicationConnectionProvider"], hi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationConnectionProvider"');
  return Ri.includes(e.__typename);
}, wi = ["EnterpriseLicenseInfoDTO"], fi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEnterpriseLicenseInfoDTO"');
  return wi.includes(e.__typename);
}, Pi = ["EnterpriseSubscriptionStatusDTO"], Fi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEnterpriseSubscriptionStatusDTO"');
  return Pi.includes(e.__typename);
}, Di = ["Analytics"], Li = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAnalytics"');
  return Di.includes(e.__typename);
}, bi = ["VerificationRecord"], Mi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isVerificationRecord"');
  return bi.includes(e.__typename);
}, Ni = ["EmailingDomain"], vi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEmailingDomain"');
  return Ni.includes(e.__typename);
}, Ui = ["SendEmailViaDomainOutput"], ki = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSendEmailViaDomainOutput"');
  return Ui.includes(e.__typename);
}, Vi = ["ApprovedAccessDomain"], Wi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApprovedAccessDomain"');
  return Vi.includes(e.__typename);
}, Gi = ["FileWithSignedUrl"], Bi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFileWithSignedUrl"');
  return Gi.includes(e.__typename);
}, xi = ["BillingSubscriptionSchedulePhaseItem"], Ki = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingSubscriptionSchedulePhaseItem"');
  return xi.includes(e.__typename);
}, Hi = ["BillingSubscriptionSchedulePhase"], Yi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingSubscriptionSchedulePhase"');
  return Hi.includes(e.__typename);
}, Qi = ["BillingProductMetadata"], qi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingProductMetadata"');
  return Qi.includes(e.__typename);
}, zi = ["BillingPriceLicensed"], Xi = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingPriceLicensed"');
  return zi.includes(e.__typename);
}, $i = ["BillingPriceTier"], Ji = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingPriceTier"');
  return $i.includes(e.__typename);
}, Zi = ["BillingPriceMetered"], ji = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingPriceMetered"');
  return Zi.includes(e.__typename);
}, en = ["BillingProduct"], tn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingProduct"');
  return en.includes(e.__typename);
}, nn = ["BillingLicensedProduct"], an = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingLicensedProduct"');
  return nn.includes(e.__typename);
}, sn = ["BillingMeteredProduct"], on = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingMeteredProduct"');
  return sn.includes(e.__typename);
}, rn = ["BillingSubscriptionItem"], pn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingSubscriptionItem"');
  return rn.includes(e.__typename);
}, ln = ["BillingSubscription"], _n = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingSubscription"');
  return ln.includes(e.__typename);
}, dn = ["BillingEndTrialPeriod"], cn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingEndTrialPeriod"');
  return dn.includes(e.__typename);
}, un = ["BillingResourceCreditUsage"], mn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingResourceCreditUsage"');
  return un.includes(e.__typename);
}, yn = ["BillingPlan"], gn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingPlan"');
  return yn.includes(e.__typename);
}, En = ["BillingSession"], In = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingSession"');
  return En.includes(e.__typename);
}, An = ["BillingUpdate"], Tn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingUpdate"');
  return An.includes(e.__typename);
}, Sn = ["OnboardingStepSuccess"], Cn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isOnboardingStepSuccess"');
  return Sn.includes(e.__typename);
}, On = ["WorkspaceInvitation"], Rn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkspaceInvitation"');
  return On.includes(e.__typename);
}, hn = ["SendInvitations"], wn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSendInvitations"');
  return hn.includes(e.__typename);
}, fn = ["RecordIdentifier"], Pn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRecordIdentifier"');
  return fn.includes(e.__typename);
}, Fn = ["NavigationMenuItem"], Dn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isNavigationMenuItem"');
  return Fn.includes(e.__typename);
}, Ln = ["ObjectRecordEventProperties"], bn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectRecordEventProperties"');
  return Ln.includes(e.__typename);
}, Mn = ["MetadataEvent"], Nn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMetadataEvent"');
  return Mn.includes(e.__typename);
}, vn = ["ObjectRecordEvent"], Un = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectRecordEvent"');
  return vn.includes(e.__typename);
}, kn = ["ObjectRecordEventWithQueryIds"], Vn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectRecordEventWithQueryIds"');
  return kn.includes(e.__typename);
}, Wn = ["EventSubscription"], Gn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEventSubscription"');
  return Wn.includes(e.__typename);
}, Bn = ["LogicFunctionExecutionResult"], xn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLogicFunctionExecutionResult"');
  return Bn.includes(e.__typename);
}, Kn = ["FeatureFlag"], Hn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFeatureFlag"');
  return Kn.includes(e.__typename);
}, Yn = ["WorkspaceUrls"], Qn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkspaceUrls"');
  return Yn.includes(e.__typename);
}, qn = ["ApplicationRegistrationVariableDTO"], zn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationRegistrationVariableDTO"');
  return qn.includes(e.__typename);
}, Xn = ["BillingTrialPeriod"], $n = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingTrialPeriod"');
  return Xn.includes(e.__typename);
}, Jn = ["SSOIdentityProvider"], Zn = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSSOIdentityProvider"');
  return Jn.includes(e.__typename);
}, jn = ["AuthProviders"], ea = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAuthProviders"');
  return jn.includes(e.__typename);
}, ta = ["AuthBypassProviders"], ia = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAuthBypassProviders"');
  return ta.includes(e.__typename);
}, na = ["PublicWorkspaceData"], aa = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPublicWorkspaceData"');
  return na.includes(e.__typename);
}, sa = ["PublicWorkspaceDataSummary"], oa = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPublicWorkspaceDataSummary"');
  return sa.includes(e.__typename);
}, ra = ["NativeModelCapabilities"], pa = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isNativeModelCapabilities"');
  return ra.includes(e.__typename);
}, la = ["ClientAiModelConfig"], _a = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isClientAiModelConfig"');
  return la.includes(e.__typename);
}, da = ["Billing"], ca = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBilling"');
  return da.includes(e.__typename);
}, ua = ["Support"], ma = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSupport"');
  return ua.includes(e.__typename);
}, ya = ["Sentry"], ga = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSentry"');
  return ya.includes(e.__typename);
}, Ea = ["Captcha"], Ia = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCaptcha"');
  return Ea.includes(e.__typename);
}, Aa = ["ApiConfig"], Ta = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApiConfig"');
  return Aa.includes(e.__typename);
}, Sa = ["PublicFeatureFlagMetadata"], Ca = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPublicFeatureFlagMetadata"');
  return Sa.includes(e.__typename);
}, Oa = ["PublicFeatureFlag"], Ra = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPublicFeatureFlag"');
  return Oa.includes(e.__typename);
}, ha = ["ClientConfigMaintenanceMode"], wa = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isClientConfigMaintenanceMode"');
  return ha.includes(e.__typename);
}, fa = ["ClientConfig"], Pa = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isClientConfig"');
  return fa.includes(e.__typename);
}, Fa = ["UsageBreakdownItem"], Da = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isUsageBreakdownItem"');
  return Fa.includes(e.__typename);
}, La = ["VersionDistributionEntry"], ba = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isVersionDistributionEntry"');
  return La.includes(e.__typename);
}, Ma = ["ApplicationRegistrationStats"], Na = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApplicationRegistrationStats"');
  return Ma.includes(e.__typename);
}, va = ["CreateApplicationRegistration"], Ua = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCreateApplicationRegistration"');
  return va.includes(e.__typename);
}, ka = ["PublicApplicationRegistration"], Va = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPublicApplicationRegistration"');
  return ka.includes(e.__typename);
}, Wa = ["RotateClientSecret"], Ga = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRotateClientSecret"');
  return Wa.includes(e.__typename);
}, Ba = ["Relation"], xa = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isRelation"');
  return Ba.includes(e.__typename);
}, Ka = ["IndexEdge"], Ha = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isIndexEdge"');
  return Ka.includes(e.__typename);
}, Ya = ["PageInfo"], Qa = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPageInfo"');
  return Ya.includes(e.__typename);
}, qa = ["IndexConnection"], za = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isIndexConnection"');
  return qa.includes(e.__typename);
}, Xa = ["IndexFieldEdge"], $a = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isIndexFieldEdge"');
  return Xa.includes(e.__typename);
}, Ja = ["IndexIndexFieldMetadatasConnection"], Za = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isIndexIndexFieldMetadatasConnection"');
  return Ja.includes(e.__typename);
}, ja = ["ObjectEdge"], es = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectEdge"');
  return ja.includes(e.__typename);
}, ts = ["IndexObjectMetadataConnection"], is = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isIndexObjectMetadataConnection"');
  return ts.includes(e.__typename);
}, ns = ["ObjectRecordCount"], as = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectRecordCount"');
  return ns.includes(e.__typename);
}, ss = ["ObjectConnection"], os = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectConnection"');
  return ss.includes(e.__typename);
}, rs = ["ObjectIndexMetadatasConnection"], ps = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectIndexMetadatasConnection"');
  return rs.includes(e.__typename);
}, ls = ["FieldEdge"], _s = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFieldEdge"');
  return ls.includes(e.__typename);
}, ds = ["ObjectFieldsConnection"], cs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isObjectFieldsConnection"');
  return ds.includes(e.__typename);
}, us = ["FieldConnection"], ms = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFieldConnection"');
  return us.includes(e.__typename);
}, ys = ["ResendEmailVerificationToken"], gs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isResendEmailVerificationToken"');
  return ys.includes(e.__typename);
}, Es = ["DeleteSso"], Is = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isDeleteSso"');
  return Es.includes(e.__typename);
}, As = ["EditSso"], Ts = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEditSso"');
  return As.includes(e.__typename);
}, Ss = ["WorkspaceNameAndId"], Cs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkspaceNameAndId"');
  return Ss.includes(e.__typename);
}, Os = ["FindAvailableSSOIDP"], Rs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFindAvailableSSOIDP"');
  return Os.includes(e.__typename);
}, hs = ["SetupSso"], ws = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSetupSso"');
  return hs.includes(e.__typename);
}, fs = ["SSOConnection"], Ps = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSSOConnection"');
  return fs.includes(e.__typename);
}, Fs = ["AvailableWorkspace"], Ds = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAvailableWorkspace"');
  return Fs.includes(e.__typename);
}, Ls = ["AvailableWorkspaces"], bs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAvailableWorkspaces"');
  return Ls.includes(e.__typename);
}, Ms = ["DeletedWorkspaceMember"], Ns = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isDeletedWorkspaceMember"');
  return Ms.includes(e.__typename);
}, vs = ["BillingEntitlement"], Us = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBillingEntitlement"');
  return vs.includes(e.__typename);
}, ks = ["DomainRecord"], Vs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isDomainRecord"');
  return ks.includes(e.__typename);
}, Ws = ["DomainValidRecords"], Gs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isDomainValidRecords"');
  return Ws.includes(e.__typename);
}, Bs = ["UpsertRowLevelPermissionPredicatesResult"], xs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isUpsertRowLevelPermissionPredicatesResult"');
  return Bs.includes(e.__typename);
}, Ks = ["LogicFunctionLogs"], Hs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLogicFunctionLogs"');
  return Ks.includes(e.__typename);
}, Ys = ["DeleteTwoFactorAuthenticationMethod"], Qs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isDeleteTwoFactorAuthenticationMethod"');
  return Ys.includes(e.__typename);
}, qs = ["InitiateTwoFactorAuthenticationProvisioning"], zs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isInitiateTwoFactorAuthenticationProvisioning"');
  return qs.includes(e.__typename);
}, Xs = ["VerifyTwoFactorAuthenticationMethod"], $s = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isVerifyTwoFactorAuthenticationMethod"');
  return Xs.includes(e.__typename);
}, Js = ["AuthorizeApp"], Zs = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAuthorizeApp"');
  return Js.includes(e.__typename);
}, js = ["AuthTokenPair"], eo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAuthTokenPair"');
  return js.includes(e.__typename);
}, to = ["AvailableWorkspacesAndAccessTokens"], io = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAvailableWorkspacesAndAccessTokens"');
  return to.includes(e.__typename);
}, no = ["EmailPasswordResetLink"], ao = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEmailPasswordResetLink"');
  return no.includes(e.__typename);
}, so = ["GetAuthorizationUrlForSSO"], oo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isGetAuthorizationUrlForSSO"');
  return so.includes(e.__typename);
}, ro = ["InvalidatePassword"], po = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isInvalidatePassword"');
  return ro.includes(e.__typename);
}, lo = ["WorkspaceUrlsAndId"], _o = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkspaceUrlsAndId"');
  return lo.includes(e.__typename);
}, co = ["SignUp"], uo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSignUp"');
  return co.includes(e.__typename);
}, mo = ["TransientToken"], yo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isTransientToken"');
  return mo.includes(e.__typename);
}, go = ["ValidatePasswordResetToken"], Eo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isValidatePasswordResetToken"');
  return go.includes(e.__typename);
}, Io = ["VerifyEmailAndGetLoginToken"], Ao = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isVerifyEmailAndGetLoginToken"');
  return Io.includes(e.__typename);
}, To = ["ApiKeyToken"], So = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isApiKeyToken"');
  return To.includes(e.__typename);
}, Co = ["AuthTokens"], Oo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAuthTokens"');
  return Co.includes(e.__typename);
}, Ro = ["LoginToken"], ho = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLoginToken"');
  return Ro.includes(e.__typename);
}, wo = ["CheckUserExist"], fo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCheckUserExist"');
  return wo.includes(e.__typename);
}, Po = ["WorkspaceInviteHashValid"], Fo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkspaceInviteHashValid"');
  return Po.includes(e.__typename);
}, Do = ["Impersonate"], Lo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isImpersonate"');
  return Do.includes(e.__typename);
}, bo = ["UsageTimeSeries"], Mo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isUsageTimeSeries"');
  return bo.includes(e.__typename);
}, No = ["UsageUserDaily"], vo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isUsageUserDaily"');
  return No.includes(e.__typename);
}, Uo = ["UsageAnalytics"], ko = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isUsageAnalytics"');
  return Uo.includes(e.__typename);
}, Vo = ["DevelopmentApplication"], Wo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isDevelopmentApplication"');
  return Vo.includes(e.__typename);
}, Go = ["WorkspaceMigration"], Bo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWorkspaceMigration"');
  return Go.includes(e.__typename);
}, xo = ["File"], Ko = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isFile"');
  return xo.includes(e.__typename);
}, Ho = ["MarketplaceApp"], Yo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMarketplaceApp"');
  return Ho.includes(e.__typename);
}, Qo = ["MarketplaceAppDetail"], qo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMarketplaceAppDetail"');
  return Qo.includes(e.__typename);
}, zo = ["PublicDomain"], Xo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPublicDomain"');
  return zo.includes(e.__typename);
}, $o = ["AutocompleteResult"], Jo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAutocompleteResult"');
  return $o.includes(e.__typename);
}, Zo = ["Location"], jo = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLocation"');
  return Zo.includes(e.__typename);
}, er = ["PlaceDetailsResult"], tr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPlaceDetailsResult"');
  return er.includes(e.__typename);
}, ir = ["PublicConnectionParametersOutput"], nr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPublicConnectionParametersOutput"');
  return ir.includes(e.__typename);
}, ar = ["PublicImapSmtpCaldavConnectionParameters"], sr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPublicImapSmtpCaldavConnectionParameters"');
  return ar.includes(e.__typename);
}, or = ["ConnectedAccountPublicDTO"], rr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isConnectedAccountPublicDTO"');
  return or.includes(e.__typename);
}, pr = ["ImapSmtpCaldavPublicConnectionParams"], lr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isImapSmtpCaldavPublicConnectionParams"');
  return pr.includes(e.__typename);
}, _r = ["ImapSmtpCaldavPublicConnectionParameters"], dr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isImapSmtpCaldavPublicConnectionParameters"');
  return _r.includes(e.__typename);
}, cr = ["ConnectedImapSmtpCaldavAccount"], ur = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isConnectedImapSmtpCaldavAccount"');
  return cr.includes(e.__typename);
}, mr = ["ImapSmtpCaldavConnectionSuccess"], yr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isImapSmtpCaldavConnectionSuccess"');
  return mr.includes(e.__typename);
}, gr = ["Webhook"], Er = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isWebhook"');
  return gr.includes(e.__typename);
}, Ir = ["ToolIndexEntry"], Ar = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isToolIndexEntry"');
  return Ir.includes(e.__typename);
}, Tr = ["AgentMessagePart"], Sr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAgentMessagePart"');
  return Tr.includes(e.__typename);
}, Cr = ["ChannelSyncSuccess"], Or = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isChannelSyncSuccess"');
  return Cr.includes(e.__typename);
}, Rr = ["BarChartSeries"], hr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBarChartSeries"');
  return Rr.includes(e.__typename);
}, wr = ["BarChartData"], fr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isBarChartData"');
  return wr.includes(e.__typename);
}, Pr = ["LineChartDataPoint"], Fr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLineChartDataPoint"');
  return Pr.includes(e.__typename);
}, Dr = ["LineChartSeries"], Lr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLineChartSeries"');
  return Dr.includes(e.__typename);
}, br = ["LineChartData"], Mr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isLineChartData"');
  return br.includes(e.__typename);
}, Nr = ["PieChartDataItem"], vr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPieChartDataItem"');
  return Nr.includes(e.__typename);
}, Ur = ["PieChartData"], kr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isPieChartData"');
  return Ur.includes(e.__typename);
}, Vr = ["DuplicatedDashboard"], Wr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isDuplicatedDashboard"');
  return Vr.includes(e.__typename);
}, Gr = ["SendEmailOutput"], Br = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSendEmailOutput"');
  return Gr.includes(e.__typename);
}, xr = ["EventLogRecord"], Kr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEventLogRecord"');
  return xr.includes(e.__typename);
}, Hr = ["EventLogPageInfo"], Yr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEventLogPageInfo"');
  return Hr.includes(e.__typename);
}, Qr = ["EventLogQueryResult"], qr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isEventLogQueryResult"');
  return Qr.includes(e.__typename);
}, zr = ["Skill"], Xr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSkill"');
  return zr.includes(e.__typename);
}, $r = ["AgentMessage"], Jr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAgentMessage"');
  return $r.includes(e.__typename);
}, Zr = ["AgentChatThread"], jr = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAgentChatThread"');
  return Zr.includes(e.__typename);
}, ep = ["AiSystemPromptSection"], tp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAiSystemPromptSection"');
  return ep.includes(e.__typename);
}, ip = ["AiSystemPromptPreview"], np = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAiSystemPromptPreview"');
  return ip.includes(e.__typename);
}, ap = ["ChatStreamCatchupChunks"], sp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isChatStreamCatchupChunks"');
  return ap.includes(e.__typename);
}, op = ["SendChatMessageResult"], rp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSendChatMessageResult"');
  return op.includes(e.__typename);
}, pp = ["AgentChatEvent"], lp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAgentChatEvent"');
  return pp.includes(e.__typename);
}, _p = ["AgentTurnEvaluation"], dp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAgentTurnEvaluation"');
  return _p.includes(e.__typename);
}, cp = ["AgentTurn"], up = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isAgentTurn"');
  return cp.includes(e.__typename);
}, mp = ["CalendarChannel"], yp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCalendarChannel"');
  return mp.includes(e.__typename);
}, gp = ["MessageChannel"], Ep = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMessageChannel"');
  return gp.includes(e.__typename);
}, Ip = ["CreateEmailGroupChannelOutput"], Ap = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCreateEmailGroupChannelOutput"');
  return Ip.includes(e.__typename);
}, Tp = ["MessageFolder"], Sp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMessageFolder"');
  return Tp.includes(e.__typename);
}, Cp = ["CollectionHash"], Op = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isCollectionHash"');
  return Cp.includes(e.__typename);
}, Rp = ["MinimalObjectMetadata"], hp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMinimalObjectMetadata"');
  return Rp.includes(e.__typename);
}, wp = ["MinimalView"], fp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMinimalView"');
  return wp.includes(e.__typename);
}, Pp = ["MinimalMetadata"], Fp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMinimalMetadata"');
  return Pp.includes(e.__typename);
}, Dp = ["Query"], Lp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isQuery"');
  return Dp.includes(e.__typename);
}, bp = ["Mutation"], Mp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isMutation"');
  return bp.includes(e.__typename);
}, Np = ["Subscription"], vp = (e) => {
  if (!e?.__typename) throw new Error('__typename is missing in "isSubscription"');
  return Np.includes(e.__typename);
}, Up = {
  NPM: "NPM",
  TARBALL: "TARBALL",
  LOCAL: "LOCAL",
  OAUTH_ONLY: "OAUTH_ONLY"
}, kp = {
  AND: "AND",
  OR: "OR"
}, Vp = {
  IS: "IS",
  IS_NOT_NULL: "IS_NOT_NULL",
  IS_NOT: "IS_NOT",
  LESS_THAN_OR_EQUAL: "LESS_THAN_OR_EQUAL",
  GREATER_THAN_OR_EQUAL: "GREATER_THAN_OR_EQUAL",
  IS_BEFORE: "IS_BEFORE",
  IS_AFTER: "IS_AFTER",
  CONTAINS: "CONTAINS",
  DOES_NOT_CONTAIN: "DOES_NOT_CONTAIN",
  IS_EMPTY: "IS_EMPTY",
  IS_NOT_EMPTY: "IS_NOT_EMPTY",
  IS_RELATIVE: "IS_RELATIVE",
  IS_IN_PAST: "IS_IN_PAST",
  IS_IN_FUTURE: "IS_IN_FUTURE",
  IS_TODAY: "IS_TODAY",
  VECTOR_SEARCH: "VECTOR_SEARCH"
}, Wp = {
  API_KEYS_AND_WEBHOOKS: "API_KEYS_AND_WEBHOOKS",
  WORKSPACE: "WORKSPACE",
  WORKSPACE_MEMBERS: "WORKSPACE_MEMBERS",
  ROLES: "ROLES",
  DATA_MODEL: "DATA_MODEL",
  SECURITY: "SECURITY",
  WORKFLOWS: "WORKFLOWS",
  IMPERSONATE: "IMPERSONATE",
  SSO_BYPASS: "SSO_BYPASS",
  APPLICATIONS: "APPLICATIONS",
  MARKETPLACE_APPS: "MARKETPLACE_APPS",
  LAYOUTS: "LAYOUTS",
  BILLING: "BILLING",
  AI_SETTINGS: "AI_SETTINGS",
  AI: "AI",
  VIEWS: "VIEWS",
  UPLOAD_FILE: "UPLOAD_FILE",
  DOWNLOAD_FILE: "DOWNLOAD_FILE",
  SEND_EMAIL_TOOL: "SEND_EMAIL_TOOL",
  HTTP_REQUEST_TOOL: "HTTP_REQUEST_TOOL",
  CODE_INTERPRETER_TOOL: "CODE_INTERPRETER_TOOL",
  IMPORT_CSV: "IMPORT_CSV",
  EXPORT_CSV: "EXPORT_CSV",
  CONNECTED_ACCOUNTS: "CONNECTED_ACCOUNTS",
  PROFILE_INFORMATION: "PROFILE_INFORMATION"
}, Gp = {
  SYSTEM: "SYSTEM",
  MONTH_FIRST: "MONTH_FIRST",
  DAY_FIRST: "DAY_FIRST",
  YEAR_FIRST: "YEAR_FIRST"
}, Bp = {
  SYSTEM: "SYSTEM",
  HOUR_12: "HOUR_12",
  HOUR_24: "HOUR_24"
}, xp = {
  SYSTEM: "SYSTEM",
  COMMAS_AND_DOT: "COMMAS_AND_DOT",
  SPACES_AND_COMMA: "SPACES_AND_COMMA",
  DOTS_AND_COMMA: "DOTS_AND_COMMA",
  APOSTROPHE_AND_DOT: "APOSTROPHE_AND_DOT"
}, Kp = {
  NAVIGATE_TO_NEXT_RECORD: "NAVIGATE_TO_NEXT_RECORD",
  NAVIGATE_TO_PREVIOUS_RECORD: "NAVIGATE_TO_PREVIOUS_RECORD",
  CREATE_NEW_RECORD: "CREATE_NEW_RECORD",
  DELETE_RECORDS: "DELETE_RECORDS",
  RESTORE_RECORDS: "RESTORE_RECORDS",
  DESTROY_RECORDS: "DESTROY_RECORDS",
  ADD_TO_FAVORITES: "ADD_TO_FAVORITES",
  REMOVE_FROM_FAVORITES: "REMOVE_FROM_FAVORITES",
  EXPORT_NOTE_TO_PDF: "EXPORT_NOTE_TO_PDF",
  EXPORT_RECORDS: "EXPORT_RECORDS",
  UPDATE_MULTIPLE_RECORDS: "UPDATE_MULTIPLE_RECORDS",
  MERGE_MULTIPLE_RECORDS: "MERGE_MULTIPLE_RECORDS",
  IMPORT_RECORDS: "IMPORT_RECORDS",
  EXPORT_VIEW: "EXPORT_VIEW",
  SEE_DELETED_RECORDS: "SEE_DELETED_RECORDS",
  CREATE_NEW_VIEW: "CREATE_NEW_VIEW",
  HIDE_DELETED_RECORDS: "HIDE_DELETED_RECORDS",
  EDIT_RECORD_PAGE_LAYOUT: "EDIT_RECORD_PAGE_LAYOUT",
  EDIT_DASHBOARD_LAYOUT: "EDIT_DASHBOARD_LAYOUT",
  SAVE_DASHBOARD_LAYOUT: "SAVE_DASHBOARD_LAYOUT",
  CANCEL_DASHBOARD_LAYOUT: "CANCEL_DASHBOARD_LAYOUT",
  DUPLICATE_DASHBOARD: "DUPLICATE_DASHBOARD",
  ACTIVATE_WORKFLOW: "ACTIVATE_WORKFLOW",
  DEACTIVATE_WORKFLOW: "DEACTIVATE_WORKFLOW",
  DISCARD_DRAFT_WORKFLOW: "DISCARD_DRAFT_WORKFLOW",
  TEST_WORKFLOW: "TEST_WORKFLOW",
  SEE_ACTIVE_VERSION_WORKFLOW: "SEE_ACTIVE_VERSION_WORKFLOW",
  SEE_RUNS_WORKFLOW: "SEE_RUNS_WORKFLOW",
  SEE_VERSIONS_WORKFLOW: "SEE_VERSIONS_WORKFLOW",
  ADD_NODE_WORKFLOW: "ADD_NODE_WORKFLOW",
  TIDY_UP_WORKFLOW: "TIDY_UP_WORKFLOW",
  DUPLICATE_WORKFLOW: "DUPLICATE_WORKFLOW",
  SEE_VERSION_WORKFLOW_RUN: "SEE_VERSION_WORKFLOW_RUN",
  SEE_WORKFLOW_WORKFLOW_RUN: "SEE_WORKFLOW_WORKFLOW_RUN",
  STOP_WORKFLOW_RUN: "STOP_WORKFLOW_RUN",
  SEE_RUNS_WORKFLOW_VERSION: "SEE_RUNS_WORKFLOW_VERSION",
  SEE_WORKFLOW_WORKFLOW_VERSION: "SEE_WORKFLOW_WORKFLOW_VERSION",
  USE_AS_DRAFT_WORKFLOW_VERSION: "USE_AS_DRAFT_WORKFLOW_VERSION",
  SEE_VERSIONS_WORKFLOW_VERSION: "SEE_VERSIONS_WORKFLOW_VERSION",
  SEARCH_RECORDS: "SEARCH_RECORDS",
  SEARCH_RECORDS_FALLBACK: "SEARCH_RECORDS_FALLBACK",
  ASK_AI: "ASK_AI",
  VIEW_PREVIOUS_AI_CHATS: "VIEW_PREVIOUS_AI_CHATS",
  NAVIGATION: "NAVIGATION",
  TRIGGER_WORKFLOW_VERSION: "TRIGGER_WORKFLOW_VERSION",
  FRONT_COMPONENT_RENDERER: "FRONT_COMPONENT_RENDERER",
  REPLY_TO_EMAIL_THREAD: "REPLY_TO_EMAIL_THREAD",
  COMPOSE_EMAIL: "COMPOSE_EMAIL",
  GO_TO_PEOPLE: "GO_TO_PEOPLE",
  GO_TO_COMPANIES: "GO_TO_COMPANIES",
  GO_TO_DASHBOARDS: "GO_TO_DASHBOARDS",
  GO_TO_OPPORTUNITIES: "GO_TO_OPPORTUNITIES",
  GO_TO_SETTINGS: "GO_TO_SETTINGS",
  GO_TO_TASKS: "GO_TO_TASKS",
  GO_TO_NOTES: "GO_TO_NOTES",
  GO_TO_WORKFLOWS: "GO_TO_WORKFLOWS",
  GO_TO_RUNS: "GO_TO_RUNS",
  DELETE_SINGLE_RECORD: "DELETE_SINGLE_RECORD",
  DELETE_MULTIPLE_RECORDS: "DELETE_MULTIPLE_RECORDS",
  RESTORE_SINGLE_RECORD: "RESTORE_SINGLE_RECORD",
  RESTORE_MULTIPLE_RECORDS: "RESTORE_MULTIPLE_RECORDS",
  DESTROY_SINGLE_RECORD: "DESTROY_SINGLE_RECORD",
  DESTROY_MULTIPLE_RECORDS: "DESTROY_MULTIPLE_RECORDS",
  EXPORT_FROM_RECORD_INDEX: "EXPORT_FROM_RECORD_INDEX",
  EXPORT_FROM_RECORD_SHOW: "EXPORT_FROM_RECORD_SHOW",
  EXPORT_MULTIPLE_RECORDS: "EXPORT_MULTIPLE_RECORDS"
}, Hp = {
  GLOBAL: "GLOBAL",
  GLOBAL_OBJECT_CONTEXT: "GLOBAL_OBJECT_CONTEXT",
  RECORD_SELECTION: "RECORD_SELECTION",
  FALLBACK: "FALLBACK"
}, Yp = {
  ACTOR: "ACTOR",
  ADDRESS: "ADDRESS",
  ARRAY: "ARRAY",
  BOOLEAN: "BOOLEAN",
  CURRENCY: "CURRENCY",
  DATE: "DATE",
  DATE_TIME: "DATE_TIME",
  EMAILS: "EMAILS",
  FILES: "FILES",
  FULL_NAME: "FULL_NAME",
  LINKS: "LINKS",
  MORPH_RELATION: "MORPH_RELATION",
  MULTI_SELECT: "MULTI_SELECT",
  NUMBER: "NUMBER",
  NUMERIC: "NUMERIC",
  PHONES: "PHONES",
  POSITION: "POSITION",
  RATING: "RATING",
  RAW_JSON: "RAW_JSON",
  RELATION: "RELATION",
  RICH_TEXT: "RICH_TEXT",
  SELECT: "SELECT",
  TEXT: "TEXT",
  TS_VECTOR: "TS_VECTOR",
  UUID: "UUID"
}, Qp = {
  BTREE: "BTREE",
  GIN: "GIN"
}, qp = {
  MIN: "MIN",
  MAX: "MAX",
  AVG: "AVG",
  SUM: "SUM",
  COUNT: "COUNT",
  COUNT_UNIQUE_VALUES: "COUNT_UNIQUE_VALUES",
  COUNT_EMPTY: "COUNT_EMPTY",
  COUNT_NOT_EMPTY: "COUNT_NOT_EMPTY",
  COUNT_TRUE: "COUNT_TRUE",
  COUNT_FALSE: "COUNT_FALSE",
  PERCENTAGE_EMPTY: "PERCENTAGE_EMPTY",
  PERCENTAGE_NOT_EMPTY: "PERCENTAGE_NOT_EMPTY"
}, zp = {
  AND: "AND",
  OR: "OR",
  NOT: "NOT"
}, Xp = {
  IS: "IS",
  IS_NOT_NULL: "IS_NOT_NULL",
  IS_NOT: "IS_NOT",
  LESS_THAN_OR_EQUAL: "LESS_THAN_OR_EQUAL",
  GREATER_THAN_OR_EQUAL: "GREATER_THAN_OR_EQUAL",
  IS_BEFORE: "IS_BEFORE",
  IS_AFTER: "IS_AFTER",
  CONTAINS: "CONTAINS",
  DOES_NOT_CONTAIN: "DOES_NOT_CONTAIN",
  IS_EMPTY: "IS_EMPTY",
  IS_NOT_EMPTY: "IS_NOT_EMPTY",
  IS_RELATIVE: "IS_RELATIVE",
  IS_IN_PAST: "IS_IN_PAST",
  IS_IN_FUTURE: "IS_IN_FUTURE",
  IS_TODAY: "IS_TODAY",
  VECTOR_SEARCH: "VECTOR_SEARCH"
}, $p = {
  ASC: "ASC",
  DESC: "DESC"
}, Jp = {
  TABLE: "TABLE",
  KANBAN: "KANBAN",
  CALENDAR: "CALENDAR",
  FIELDS_WIDGET: "FIELDS_WIDGET",
  TABLE_WIDGET: "TABLE_WIDGET"
}, Zp = {
  INDEX: "INDEX"
}, jp = {
  SIDE_PANEL: "SIDE_PANEL",
  RECORD_PAGE: "RECORD_PAGE"
}, el = {
  DAY: "DAY",
  WEEK: "WEEK",
  MONTH: "MONTH"
}, tl = {
  WORKSPACE: "WORKSPACE",
  UNLISTED: "UNLISTED"
}, il = {
  ONGOING_CREATION: "ONGOING_CREATION",
  PENDING_CREATION: "PENDING_CREATION",
  ACTIVE: "ACTIVE",
  INACTIVE: "INACTIVE",
  SUSPENDED: "SUSPENDED"
}, nl = {
  PLAN_REQUIRED: "PLAN_REQUIRED",
  WORKSPACE_ACTIVATION: "WORKSPACE_ACTIVATION",
  PROFILE_CREATION: "PROFILE_CREATION",
  SYNC_EMAIL: "SYNC_EMAIL",
  INVITE_TEAM: "INVITE_TEAM",
  BOOK_ONBOARDING: "BOOK_ONBOARDING",
  COMPLETED: "COMPLETED"
}, al = {
  VIEW: "VIEW",
  IFRAME: "IFRAME",
  FIELD: "FIELD",
  FIELDS: "FIELDS",
  GRAPH: "GRAPH",
  STANDALONE_RICH_TEXT: "STANDALONE_RICH_TEXT",
  TIMELINE: "TIMELINE",
  TASKS: "TASKS",
  NOTES: "NOTES",
  FILES: "FILES",
  EMAILS: "EMAILS",
  CALENDAR: "CALENDAR",
  FIELD_RICH_TEXT: "FIELD_RICH_TEXT",
  WORKFLOW: "WORKFLOW",
  WORKFLOW_VERSION: "WORKFLOW_VERSION",
  WORKFLOW_RUN: "WORKFLOW_RUN",
  FRONT_COMPONENT: "FRONT_COMPONENT",
  RECORD_TABLE: "RECORD_TABLE",
  EMAIL_THREAD: "EMAIL_THREAD"
}, sl = {
  GRID: "GRID",
  VERTICAL_LIST: "VERTICAL_LIST",
  CANVAS: "CANVAS"
}, ol = {
  AGGREGATE_CHART: "AGGREGATE_CHART",
  GAUGE_CHART: "GAUGE_CHART",
  PIE_CHART: "PIE_CHART",
  BAR_CHART: "BAR_CHART",
  LINE_CHART: "LINE_CHART",
  IFRAME: "IFRAME",
  STANDALONE_RICH_TEXT: "STANDALONE_RICH_TEXT",
  VIEW: "VIEW",
  FIELD: "FIELD",
  FIELDS: "FIELDS",
  TIMELINE: "TIMELINE",
  TASKS: "TASKS",
  NOTES: "NOTES",
  FILES: "FILES",
  EMAILS: "EMAILS",
  CALENDAR: "CALENDAR",
  FIELD_RICH_TEXT: "FIELD_RICH_TEXT",
  WORKFLOW: "WORKFLOW",
  WORKFLOW_VERSION: "WORKFLOW_VERSION",
  WORKFLOW_RUN: "WORKFLOW_RUN",
  FRONT_COMPONENT: "FRONT_COMPONENT",
  RECORD_TABLE: "RECORD_TABLE",
  EMAIL_THREAD: "EMAIL_THREAD"
}, rl = {
  DAY: "DAY",
  MONTH: "MONTH",
  QUARTER: "QUARTER",
  YEAR: "YEAR",
  WEEK: "WEEK",
  DAY_OF_THE_WEEK: "DAY_OF_THE_WEEK",
  MONTH_OF_THE_YEAR: "MONTH_OF_THE_YEAR",
  QUARTER_OF_THE_YEAR: "QUARTER_OF_THE_YEAR",
  NONE: "NONE"
}, pl = {
  FIELD_ASC: "FIELD_ASC",
  FIELD_DESC: "FIELD_DESC",
  FIELD_POSITION_ASC: "FIELD_POSITION_ASC",
  FIELD_POSITION_DESC: "FIELD_POSITION_DESC",
  VALUE_ASC: "VALUE_ASC",
  VALUE_DESC: "VALUE_DESC",
  MANUAL: "MANUAL"
}, ll = {
  NONE: "NONE",
  X: "X",
  Y: "Y",
  BOTH: "BOTH"
}, _l = {
  STACKED: "STACKED",
  GROUPED: "GROUPED"
}, dl = {
  VERTICAL: "VERTICAL",
  HORIZONTAL: "HORIZONTAL"
}, cl = {
  CARD: "CARD",
  EDITOR: "EDITOR",
  FIELD: "FIELD",
  VIEW: "VIEW",
  TABLE: "TABLE"
}, ul = {
  RECORD_INDEX: "RECORD_INDEX",
  RECORD_PAGE: "RECORD_PAGE",
  DASHBOARD: "DASHBOARD",
  STANDALONE_PAGE: "STANDALONE_PAGE"
}, ml = {
  AWS_SES: "AWS_SES"
}, yl = {
  PENDING: "PENDING",
  VERIFIED: "VERIFIED",
  FAILED: "FAILED",
  TEMPORARY_FAILURE: "TEMPORARY_FAILURE"
}, gl = {
  PRO: "PRO",
  ENTERPRISE: "ENTERPRISE"
}, El = {
  METERED: "METERED",
  LICENSED: "LICENSED"
}, Il = {
  BASE_PRODUCT: "BASE_PRODUCT",
  RESOURCE_CREDIT: "RESOURCE_CREDIT"
}, Al = {
  Month: "Month",
  Year: "Year"
}, Tl = {
  Active: "Active",
  Canceled: "Canceled",
  Incomplete: "Incomplete",
  IncompleteExpired: "IncompleteExpired",
  PastDue: "PastDue",
  Paused: "Paused",
  Trialing: "Trialing",
  Unpaid: "Unpaid"
}, Sl = {
  VIEW: "VIEW",
  FOLDER: "FOLDER",
  LINK: "LINK",
  OBJECT: "OBJECT",
  RECORD: "RECORD",
  PAGE_LAYOUT: "PAGE_LAYOUT"
}, Cl = {
  CREATED: "CREATED",
  UPDATED: "UPDATED",
  DELETED: "DELETED"
}, Ol = {
  CREATED: "CREATED",
  UPDATED: "UPDATED",
  DELETED: "DELETED",
  DESTROYED: "DESTROYED",
  RESTORED: "RESTORED",
  UPSERTED: "UPSERTED"
}, Rl = {
  IDLE: "IDLE",
  SUCCESS: "SUCCESS",
  ERROR: "ERROR"
}, hl = {
  IS_UNIQUE_INDEXES_ENABLED: "IS_UNIQUE_INDEXES_ENABLED",
  IS_JSON_FILTER_ENABLED: "IS_JSON_FILTER_ENABLED",
  IS_MARKETPLACE_SETTING_TAB_VISIBLE: "IS_MARKETPLACE_SETTING_TAB_VISIBLE",
  IS_PUBLIC_DOMAIN_ENABLED: "IS_PUBLIC_DOMAIN_ENABLED",
  IS_EMAIL_GROUP_ENABLED: "IS_EMAIL_GROUP_ENABLED",
  IS_JUNCTION_RELATIONS_ENABLED: "IS_JUNCTION_RELATIONS_ENABLED",
  IS_REST_METADATA_API_NEW_FORMAT_DIRECT: "IS_REST_METADATA_API_NEW_FORMAT_DIRECT"
}, wl = {
  OIDC: "OIDC",
  SAML: "SAML"
}, fl = {
  Active: "Active",
  Inactive: "Inactive",
  Error: "Error"
}, Pl = {
  GPT: "GPT",
  CLAUDE: "CLAUDE",
  GEMINI: "GEMINI",
  MISTRAL: "MISTRAL",
  GROK: "GROK"
}, Fl = {
  NONE: "NONE",
  FRONT: "FRONT"
}, Dl = {
  GOOGLE_RECAPTCHA: "GOOGLE_RECAPTCHA",
  TURNSTILE: "TURNSTILE"
}, Ll = {
  ONE_TO_MANY: "ONE_TO_MANY",
  MANY_TO_ONE: "MANY_TO_ONE"
}, bl = {
  SSO: "SSO",
  CUSTOM_DOMAIN: "CUSTOM_DOMAIN",
  RLS: "RLS",
  AUDIT_LOGS: "AUDIT_LOGS"
}, Ml = {
  NOT_SYNCED: "NOT_SYNCED",
  ONGOING: "ONGOING",
  ACTIVE: "ACTIVE",
  FAILED_INSUFFICIENT_PERMISSIONS: "FAILED_INSUFFICIENT_PERMISSIONS",
  FAILED_UNKNOWN: "FAILED_UNKNOWN"
}, Nl = {
  PENDING_CONFIGURATION: "PENDING_CONFIGURATION",
  CALENDAR_EVENT_LIST_FETCH_PENDING: "CALENDAR_EVENT_LIST_FETCH_PENDING",
  CALENDAR_EVENT_LIST_FETCH_SCHEDULED: "CALENDAR_EVENT_LIST_FETCH_SCHEDULED",
  CALENDAR_EVENT_LIST_FETCH_ONGOING: "CALENDAR_EVENT_LIST_FETCH_ONGOING",
  CALENDAR_EVENTS_IMPORT_PENDING: "CALENDAR_EVENTS_IMPORT_PENDING",
  CALENDAR_EVENTS_IMPORT_SCHEDULED: "CALENDAR_EVENTS_IMPORT_SCHEDULED",
  CALENDAR_EVENTS_IMPORT_ONGOING: "CALENDAR_EVENTS_IMPORT_ONGOING",
  FAILED: "FAILED"
}, vl = {
  METADATA: "METADATA",
  SHARE_EVERYTHING: "SHARE_EVERYTHING"
}, Ul = {
  AS_PARTICIPANT_AND_ORGANIZER: "AS_PARTICIPANT_AND_ORGANIZER",
  AS_PARTICIPANT: "AS_PARTICIPANT",
  AS_ORGANIZER: "AS_ORGANIZER",
  NONE: "NONE"
}, kl = {
  METADATA: "METADATA",
  SUBJECT: "SUBJECT",
  SHARE_EVERYTHING: "SHARE_EVERYTHING"
}, Vl = {
  EMAIL: "EMAIL",
  SMS: "SMS",
  EMAIL_GROUP: "EMAIL_GROUP"
}, Wl = {
  SENT_AND_RECEIVED: "SENT_AND_RECEIVED",
  SENT: "SENT",
  NONE: "NONE"
}, Gl = {
  ALL_FOLDERS: "ALL_FOLDERS",
  SELECTED_FOLDERS: "SELECTED_FOLDERS"
}, Bl = {
  GROUP_EMAILS_DELETION: "GROUP_EMAILS_DELETION",
  GROUP_EMAILS_IMPORT: "GROUP_EMAILS_IMPORT",
  NONE: "NONE"
}, xl = {
  NOT_SYNCED: "NOT_SYNCED",
  ONGOING: "ONGOING",
  ACTIVE: "ACTIVE",
  FAILED_INSUFFICIENT_PERMISSIONS: "FAILED_INSUFFICIENT_PERMISSIONS",
  FAILED_UNKNOWN: "FAILED_UNKNOWN"
}, Kl = {
  PENDING_CONFIGURATION: "PENDING_CONFIGURATION",
  MESSAGE_LIST_FETCH_PENDING: "MESSAGE_LIST_FETCH_PENDING",
  MESSAGE_LIST_FETCH_SCHEDULED: "MESSAGE_LIST_FETCH_SCHEDULED",
  MESSAGE_LIST_FETCH_ONGOING: "MESSAGE_LIST_FETCH_ONGOING",
  MESSAGES_IMPORT_PENDING: "MESSAGES_IMPORT_PENDING",
  MESSAGES_IMPORT_SCHEDULED: "MESSAGES_IMPORT_SCHEDULED",
  MESSAGES_IMPORT_ONGOING: "MESSAGES_IMPORT_ONGOING",
  FAILED: "FAILED"
}, Hl = {
  FOLDER_DELETION: "FOLDER_DELETION",
  NONE: "NONE"
}, Yl = {
  fieldMetadata: "fieldMetadata",
  objectMetadata: "objectMetadata",
  view: "view",
  viewField: "viewField",
  viewFieldGroup: "viewFieldGroup",
  viewGroup: "viewGroup",
  viewSort: "viewSort",
  rowLevelPermissionPredicate: "rowLevelPermissionPredicate",
  rowLevelPermissionPredicateGroup: "rowLevelPermissionPredicateGroup",
  viewFilterGroup: "viewFilterGroup",
  index: "index",
  logicFunction: "logicFunction",
  viewFilter: "viewFilter",
  role: "role",
  roleTarget: "roleTarget",
  agent: "agent",
  skill: "skill",
  pageLayout: "pageLayout",
  pageLayoutWidget: "pageLayoutWidget",
  pageLayoutTab: "pageLayoutTab",
  commandMenuItem: "commandMenuItem",
  navigationMenuItem: "navigationMenuItem",
  rolePermissionFlag: "rolePermissionFlag",
  permissionFlag: "permissionFlag",
  objectPermission: "objectPermission",
  fieldPermission: "fieldPermission",
  frontComponent: "frontComponent",
  webhook: "webhook",
  applicationVariable: "applicationVariable",
  connectionProvider: "connectionProvider"
}, Ql = {
  WORKSPACE_EVENT: "WORKSPACE_EVENT",
  PAGEVIEW: "PAGEVIEW",
  OBJECT_EVENT: "OBJECT_EVENT",
  USAGE_EVENT: "USAGE_EVENT",
  APPLICATION_LOG: "APPLICATION_LOG"
}, ql = {
  AI_CHAT_TOKEN: "AI_CHAT_TOKEN",
  AI_WORKFLOW_TOKEN: "AI_WORKFLOW_TOKEN",
  WORKFLOW_EXECUTION: "WORKFLOW_EXECUTION",
  CODE_EXECUTION: "CODE_EXECUTION",
  WEB_SEARCH: "WEB_SEARCH"
}, zl = {
  PAGEVIEW: "PAGEVIEW",
  TRACK: "TRACK"
}, Xl = {
  delete: "delete",
  create: "create",
  update: "update"
}, $l = {
  ProfilePicture: "ProfilePicture",
  WorkspaceLogo: "WorkspaceLogo",
  Attachment: "Attachment",
  PersonPicture: "PersonPicture",
  CorePicture: "CorePicture",
  File: "File",
  AgentChat: "AgentChat",
  BuiltLogicFunction: "BuiltLogicFunction",
  BuiltFrontComponent: "BuiltFrontComponent",
  PublicAsset: "PublicAsset",
  Source: "Source",
  FilesField: "FilesField",
  Dependencies: "Dependencies",
  Workflow: "Workflow",
  EmailAttachment: "EmailAttachment",
  AppTarball: "AppTarball",
  GeneratedSdkClient: "GeneratedSdkClient"
}, i_ = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  enumAggregateOperations: qp,
  enumAllMetadataName: Yl,
  enumAnalyticsType: zl,
  enumApplicationRegistrationSourceType: Up,
  enumAxisNameDisplay: ll,
  enumBarChartGroupMode: _l,
  enumBarChartLayout: dl,
  enumBillingEntitlementKey: bl,
  enumBillingPlanKey: gl,
  enumBillingProductKey: Il,
  enumBillingUsageType: El,
  enumCalendarChannelContactAutoCreationPolicy: Ul,
  enumCalendarChannelSyncStage: Nl,
  enumCalendarChannelSyncStatus: Ml,
  enumCalendarChannelVisibility: vl,
  enumCaptchaDriverType: Dl,
  enumCommandMenuItemAvailabilityType: Hp,
  enumDatabaseEventAction: Ol,
  enumEmailingDomainDriver: ml,
  enumEmailingDomainStatus: yl,
  enumEngineComponentKey: Kp,
  enumEventLogTable: Ql,
  enumFeatureFlagKey: hl,
  enumFieldDisplayMode: cl,
  enumFieldMetadataType: Yp,
  enumFileFolder: $l,
  enumGraphOrderBy: pl,
  enumIdentityProviderType: wl,
  enumIndexType: Qp,
  enumLogicFunctionExecutionStatus: Rl,
  enumMessageChannelContactAutoCreationPolicy: Wl,
  enumMessageChannelPendingGroupEmailsAction: Bl,
  enumMessageChannelSyncStage: Kl,
  enumMessageChannelSyncStatus: xl,
  enumMessageChannelType: Vl,
  enumMessageChannelVisibility: kl,
  enumMessageFolderImportPolicy: Gl,
  enumMessageFolderPendingSyncAction: Hl,
  enumMetadataEventAction: Cl,
  enumModelFamily: Pl,
  enumNavigationMenuItemType: Sl,
  enumObjectRecordGroupByDateGranularity: rl,
  enumOnboardingStatus: nl,
  enumPageLayoutTabLayoutMode: sl,
  enumPageLayoutType: ul,
  enumPermissionFlagType: Wp,
  enumRelationType: Ll,
  enumRowLevelPermissionPredicateGroupLogicalOperator: kp,
  enumRowLevelPermissionPredicateOperand: Vp,
  enumSsoIdentityProviderStatus: fl,
  enumSubscriptionInterval: Al,
  enumSubscriptionStatus: Tl,
  enumSupportDriver: Fl,
  enumUsageOperationType: ql,
  enumViewCalendarLayout: el,
  enumViewFilterGroupLogicalOperator: zp,
  enumViewFilterOperand: Xp,
  enumViewKey: Zp,
  enumViewOpenRecordIn: jp,
  enumViewSortDirection: $p,
  enumViewType: Jp,
  enumViewVisibility: tl,
  enumWidgetConfigurationType: ol,
  enumWidgetType: al,
  enumWorkspaceActivationStatus: il,
  enumWorkspaceMemberDateFormatEnum: Gp,
  enumWorkspaceMemberNumberFormatEnum: xp,
  enumWorkspaceMemberTimeFormatEnum: Bp,
  enumWorkspaceMigrationActionType: Xl,
  isAgent: j,
  isAgentChatEvent: lp,
  isAgentChatThread: jr,
  isAgentMessage: Jr,
  isAgentMessagePart: Sr,
  isAgentTurn: up,
  isAgentTurnEvaluation: dp,
  isAggregateChartConfiguration: ft,
  isAiSystemPromptPreview: np,
  isAiSystemPromptSection: tp,
  isAnalytics: Li,
  isApiConfig: Ta,
  isApiKey: M,
  isApiKeyForRole: se,
  isApiKeyToken: So,
  isAppToken: st,
  isApplication: Be,
  isApplicationConnectionProvider: hi,
  isApplicationConnectionProviderOAuthConfig: Oi,
  isApplicationRegistration: k,
  isApplicationRegistrationStats: Na,
  isApplicationRegistrationSummary: le,
  isApplicationRegistrationVariable: v,
  isApplicationRegistrationVariableDTO: zn,
  isApplicationTokenPair: ye,
  isApplicationVariable: de,
  isApprovedAccessDomain: Wi,
  isAuthBypassProviders: ia,
  isAuthProviders: ea,
  isAuthToken: ue,
  isAuthTokenPair: eo,
  isAuthTokens: Oo,
  isAuthorizeApp: Zs,
  isAutocompleteResult: Jo,
  isAvailableWorkspace: Ds,
  isAvailableWorkspaces: bs,
  isAvailableWorkspacesAndAccessTokens: io,
  isBarChartConfiguration: Wt,
  isBarChartData: fr,
  isBarChartSeries: hr,
  isBilling: ca,
  isBillingEndTrialPeriod: cn,
  isBillingEntitlement: Us,
  isBillingLicensedProduct: an,
  isBillingMeteredProduct: on,
  isBillingPlan: gn,
  isBillingPriceLicensed: Xi,
  isBillingPriceMetered: ji,
  isBillingPriceTier: Ji,
  isBillingProduct: tn,
  isBillingProductDTO: L,
  isBillingProductMetadata: qi,
  isBillingResourceCreditUsage: mn,
  isBillingSession: In,
  isBillingSubscription: _n,
  isBillingSubscriptionItem: pn,
  isBillingSubscriptionSchedulePhase: Yi,
  isBillingSubscriptionSchedulePhaseItem: Ki,
  isBillingTrialPeriod: $n,
  isBillingUpdate: Tn,
  isCalendarChannel: yp,
  isCalendarConfiguration: Bt,
  isCaptcha: Ia,
  isChannelSyncSuccess: Or,
  isChatStreamCatchupChunks: sp,
  isCheckUserExist: fo,
  isClientAiModelConfig: _a,
  isClientConfig: Pa,
  isClientConfigMaintenanceMode: wa,
  isCollectionHash: Op,
  isCommandMenuItem: Ae,
  isCommandMenuItemPayload: Se,
  isConnectedAccountPublicDTO: rr,
  isConnectedImapSmtpCaldavAccount: ur,
  isCreateApplicationRegistration: Ua,
  isCreateEmailGroupChannelOutput: Ap,
  isDeleteSso: Is,
  isDeleteTwoFactorAuthenticationMethod: Qs,
  isDeletedWorkspaceMember: Ns,
  isDevelopmentApplication: Wo,
  isDomainRecord: Vs,
  isDomainValidRecords: Gs,
  isDuplicatedDashboard: Wr,
  isEditSso: Ts,
  isEmailPasswordResetLink: ao,
  isEmailThreadConfiguration: qt,
  isEmailingDomain: vi,
  isEmailsConfiguration: Yt,
  isEnterpriseLicenseInfoDTO: fi,
  isEnterpriseSubscriptionStatusDTO: Fi,
  isEventLogPageInfo: Yr,
  isEventLogQueryResult: qr,
  isEventLogRecord: Kr,
  isEventSubscription: Gn,
  isFeatureFlag: Hn,
  isField: Le,
  isFieldConfiguration: Xt,
  isFieldConnection: ms,
  isFieldEdge: _s,
  isFieldPermission: te,
  isFieldRichTextConfiguration: Jt,
  isFieldsConfiguration: jt,
  isFile: Ko,
  isFileWithSignedUrl: Bi,
  isFilesConfiguration: ti,
  isFindAvailableSSOIDP: Rs,
  isFrontComponent: Ee,
  isFrontComponentConfiguration: Kt,
  isFullName: X,
  isGaugeChartConfiguration: kt,
  isGetAuthorizationUrlForSSO: oo,
  isGridPosition: ut,
  isIframeConfiguration: vt,
  isImapSmtpCaldavConnectionSuccess: yr,
  isImapSmtpCaldavPublicConnectionParameters: dr,
  isImapSmtpCaldavPublicConnectionParams: lr,
  isImpersonate: Lo,
  isIndex: ve,
  isIndexConnection: za,
  isIndexEdge: Ha,
  isIndexField: Me,
  isIndexFieldEdge: $a,
  isIndexIndexFieldMetadatasConnection: Za,
  isIndexObjectMetadataConnection: is,
  isInitiateTwoFactorAuthenticationProvisioning: zs,
  isInvalidatePassword: po,
  isLineChartConfiguration: Mt,
  isLineChartData: Mr,
  isLineChartDataPoint: Fr,
  isLineChartSeries: Lr,
  isLocation: jo,
  isLogicFunction: fe,
  isLogicFunctionExecutionResult: xn,
  isLogicFunctionLogs: Hs,
  isLoginToken: ho,
  isMarketplaceApp: Yo,
  isMarketplaceAppDetail: qo,
  isMessageChannel: Ep,
  isMessageFolder: Sp,
  isMetadataEvent: Nn,
  isMinimalMetadata: Fp,
  isMinimalObjectMetadata: hp,
  isMinimalView: fp,
  isMutation: Mp,
  isNativeModelCapabilities: pa,
  isNavigationMenuItem: Dn,
  isNotesConfiguration: ni,
  isObject: We,
  isObjectConnection: os,
  isObjectEdge: es,
  isObjectFieldsConnection: cs,
  isObjectIndexMetadatasConnection: ps,
  isObjectMetadataCommandMenuItemPayload: he,
  isObjectPermission: Y,
  isObjectRecordCount: as,
  isObjectRecordEvent: Un,
  isObjectRecordEventProperties: bn,
  isObjectRecordEventWithQueryIds: Vn,
  isObjectStandardOverrides: ke,
  isOnboardingStepSuccess: Cn,
  isPageInfo: Qa,
  isPageLayout: Si,
  isPageLayoutTab: Ai,
  isPageLayoutWidget: yt,
  isPageLayoutWidgetCanvasPosition: Ot,
  isPageLayoutWidgetGridPosition: At,
  isPageLayoutWidgetPosition: Et,
  isPageLayoutWidgetVerticalListPosition: St,
  isPathCommandMenuItemPayload: Oe,
  isPieChartConfiguration: Lt,
  isPieChartData: kr,
  isPieChartDataItem: vr,
  isPlaceDetailsResult: tr,
  isPublicApplicationRegistration: Va,
  isPublicConnectionParametersOutput: nr,
  isPublicDomain: Xo,
  isPublicFeatureFlag: Ra,
  isPublicFeatureFlagMetadata: Ca,
  isPublicImapSmtpCaldavConnectionParameters: sr,
  isPublicWorkspaceData: aa,
  isPublicWorkspaceDataSummary: oa,
  isQuery: Lp,
  isRatioAggregateConfig: lt,
  isRecordIdentifier: Pn,
  isRecordTableConfiguration: di,
  isRelation: xa,
  isResendEmailVerificationToken: gs,
  isRichTextBody: dt,
  isRole: re,
  isRolePermissionFlag: ne,
  isRotateClientSecret: Ga,
  isRowLevelPermissionPredicate: K,
  isRowLevelPermissionPredicateGroup: B,
  isSSOConnection: Ps,
  isSSOIdentityProvider: Zn,
  isSendChatMessageResult: rp,
  isSendEmailOutput: Br,
  isSendEmailViaDomainOutput: ki,
  isSendInvitations: wn,
  isSentry: ga,
  isSetupSso: ws,
  isSignUp: uo,
  isSkill: Xr,
  isStandaloneRichTextConfiguration: Ft,
  isStandardOverrides: Fe,
  isSubscription: vp,
  isSupport: ma,
  isTasksConfiguration: si,
  isTimelineConfiguration: ri,
  isToolIndexEntry: Ar,
  isTransientToken: yo,
  isTwoFactorAuthenticationMethodSummary: W,
  isUpsertRowLevelPermissionPredicatesResult: xs,
  isUsageAnalytics: ko,
  isUsageBreakdownItem: Da,
  isUsageTimeSeries: Mo,
  isUsageUserDaily: vo,
  isUser: rt,
  isUserWorkspace: q,
  isValidatePasswordResetToken: Eo,
  isVerificationRecord: Mi,
  isVerifyEmailAndGetLoginToken: Ao,
  isVerifyTwoFactorAuthenticationMethod: $s,
  isVersionDistributionEntry: ba,
  isView: tt,
  isViewConfiguration: li,
  isViewField: Ke,
  isViewFieldGroup: je,
  isViewFilter: qe,
  isViewFilterGroup: Ye,
  isViewGroup: Xe,
  isViewSort: Je,
  isWebhook: Er,
  isWidgetConfiguration: ht,
  isWorkflowConfiguration: ui,
  isWorkflowRunConfiguration: yi,
  isWorkflowVersionConfiguration: Ei,
  isWorkspace: nt,
  isWorkspaceInvitation: Rn,
  isWorkspaceInviteHashValid: Fo,
  isWorkspaceMember: J,
  isWorkspaceMigration: Bo,
  isWorkspaceNameAndId: Cs,
  isWorkspaceUrls: Qn,
  isWorkspaceUrlsAndId: _o
}, Symbol.toStringTag, { value: "Module" })), g = f(F), Jl = function(e) {
  return w({
    url: void 0,
    ...e,
    queryRoot: g.Query,
    mutationRoot: g.Mutation,
    subscriptionRoot: g.Subscription
  });
}, C = "TWENTY_APP_ACCESS_TOKEN", Zl = "TWENTY_API_KEY", O = () => globalThis.process?.env ?? {}, E = (e) => {
  if (typeof e != "string")
    return null;
  const t = e.trim();
  return t.length === 0 || t === "Bearer" ? null : t.startsWith("Bearer ") ? t.slice(7).trim() : t;
}, jl = (e) => {
  if (!e)
    return null;
  if (e instanceof Headers)
    return E(
      e.get("Authorization") ?? void 0
    );
  if (Array.isArray(e)) {
    const n = e.find(
      ([i]) => i.toLowerCase() === "authorization"
    );
    return E(n?.[1]);
  }
  const t = e;
  return E(
    t.Authorization ?? t.authorization
  );
}, e_ = (e) => e?.errors ? e.errors.some((t) => t.extensions?.code === "UNAUTHENTICATED" || t.message?.toLowerCase() === "unauthorized") : !1, t_ = {
  url: `${process.env.TWENTY_API_URL}/metadata`,
  headers: {
    "Content-Type": "application/json"
  }
};
class n_ {
  constructor(t) {
    this.refreshAccessTokenPromise = null;
    const n = {
      ...t_,
      ...t
    }, {
      url: i,
      headers: a,
      fetch: p,
      fetcher: o,
      batch: l,
      ...r
    } = n;
    this.url = i ?? "", this.requestOptions = r, this.headers = a ?? {}, this.fetchImplementation = p ?? globalThis.fetch ?? null;
    const s = O(), _ = jl(
      typeof a == "function" ? void 0 : a
    );
    this.authorizationToken = _ ?? s[C] ?? s[Zl] ?? null, this.client = Jl({
      ...n,
      headers: void 0,
      fetcher: async (d) => this.executeGraphqlRequestWithOptionalRefresh({
        operation: d
      })
    });
  }
  query(t) {
    return this.client.query(t);
  }
  mutation(t) {
    return this.client.mutation(t);
  }
  async uploadFile(t, n, i = "application/octet-stream", a) {
    const p = new FormData();
    p.append(
      "operations",
      JSON.stringify({
        query: `mutation UploadFilesFieldFileByUniversalIdentifier($file: Upload!, $fieldMetadataUniversalIdentifier: String!) {
        uploadFilesFieldFileByUniversalIdentifier(file: $file, fieldMetadataUniversalIdentifier: $fieldMetadataUniversalIdentifier) { id path size createdAt url }
      }`,
        variables: {
          file: null,
          fieldMetadataUniversalIdentifier: a
        }
      })
    ), p.append("map", JSON.stringify({ 0: ["variables.file"] })), p.append(
      "0",
      new Blob([t], { type: i }),
      n
    );
    const o = await this.executeGraphqlRequestWithOptionalRefresh({
      operation: p,
      headers: {},
      requestInit: {
        method: "POST"
      }
    });
    if (o.errors)
      throw new u(o.errors, o.data);
    return o.data.uploadFilesFieldFileByUniversalIdentifier;
  }
  async executeGraphqlRequestWithOptionalRefresh({
    operation: t,
    headers: n,
    requestInit: i
  }) {
    const a = await this.executeGraphqlRequest({
      operation: t,
      headers: n,
      requestInit: i,
      token: this.authorizationToken
    });
    if (this.shouldRefreshToken(a)) {
      const p = await this.requestRefreshedAccessToken();
      if (p) {
        const o = await this.executeGraphqlRequest({
          operation: t,
          headers: n,
          requestInit: i,
          token: p
        });
        return this.assertResponseIsSuccessful(o);
      }
    }
    return this.assertResponseIsSuccessful(a);
  }
  async executeGraphqlRequest({
    operation: t,
    headers: n,
    requestInit: i,
    token: a
  }) {
    if (!this.fetchImplementation)
      throw new Error(
        "Global `fetch` function is not available, pass a fetch implementation to the Twenty client"
      );
    const p = await this.resolveHeaders(), o = new Headers(p);
    n && new Headers(n).forEach(
      (_, d) => o.set(d, _)
    ), t instanceof FormData ? o.delete("Content-Type") : o.set("Content-Type", "application/json"), a ? o.set("Authorization", `Bearer ${a}`) : o.delete("Authorization");
    const l = await this.fetchImplementation.call(globalThis, this.url, {
      ...this.requestOptions,
      ...i,
      method: i?.method ?? "POST",
      headers: o,
      body: t instanceof FormData ? t : JSON.stringify(t)
    }), r = await l.text();
    let s = null;
    if (r.trim().length > 0)
      try {
        s = JSON.parse(r);
      } catch {
        s = null;
      }
    return {
      status: l.status,
      statusText: l.statusText,
      payload: s,
      rawBody: r
    };
  }
  async resolveHeaders() {
    return typeof this.headers == "function" ? await this.headers() ?? {} : this.headers ?? {};
  }
  shouldRefreshToken(t) {
    return t.status === 401 ? !0 : e_(t.payload);
  }
  assertResponseIsSuccessful(t) {
    if (t.status < 200 || t.status >= 300)
      throw new Error(`${t.statusText}: ${t.rawBody}`);
    if (t.payload === null)
      throw new Error("Invalid JSON response");
    return t.payload;
  }
  async requestRefreshedAccessToken() {
    const t = globalThis.frontComponentHostCommunicationApi?.requestAccessTokenRefresh;
    return typeof t != "function" ? null : (this.refreshAccessTokenPromise || (this.refreshAccessTokenPromise = t().then((n) => typeof n != "string" || n.length === 0 ? null : (this.setAuthorizationToken(n), n)).catch((n) => (console.error("Twenty client: token refresh failed", n), null)).finally(() => {
      this.refreshAccessTokenPromise = null;
    })), this.refreshAccessTokenPromise);
  }
  setAuthorizationToken(t) {
    this.authorizationToken = t;
    const n = O();
    n[C] = t;
  }
}
export {
  n_ as MetadataApiClient,
  i_ as MetadataSchema
};
