...
Object.defineProperty(exports, "FixSelectAllCommandMenuItemsCommand", {
    enumerable: true,
    get: function() {
        return FixSelectAllCommandMenuItemsCommand;
    }
});
...
let FixSelectAllCommandMenuItemsCommand = class FixSelectAllCommandMenuItemsCommand extends _activeorsuspendedworkspacecommandrunner.ActiveOrSuspendedWorkspaceCommandRunner {
    async runOnWorkspace({ workspaceId, options }) {
        const isDryRun = options.dryRun ?? false;
        this.logger.log(`${isDryRun ? '[DRY RUN] ' : ''}Starting select-all expression fix for workspace ${workspaceId}`);
        ...
        if (itemsToUpdate.length === 0) {
            this.logger.log(`Select-all command menu item expressions already up to date for workspace ${workspaceId}`);
            return;
        }
        ...
        if (validateAndBuildResult.status === 'fail') {
            this.logger.error(`Failed to fix select-all expressions:\n${JSON.stringify(validateAndBuildResult, null, 2)}`);
            throw new Error(`Failed to fix select-all command menu item expressions for workspace ${workspaceId}`);
        }
...
FixSelectAllCommandMenuItemsCommand = _ts_decorate([
    (0, _registeredworkspacecommanddecorator.RegisteredWorkspaceCommand)('1.21.0', 1775500007000),
    (0, _nestcommander.Command)({
        name: 'upgrade:1-21:fix-select-all-command-menu-items',
        description: 'Fix delete/restore/destroy command menu items to work in select-all (exclusion) mode'
    }),
...
//# sourceMappingURL=1-21-workspace-command-1775500007000-fix-select-all-command-menu-items.command.js.map