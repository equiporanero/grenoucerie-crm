...
const VERSION_A = '10.20.0';
const VERSION_B = '30.40.0';
const FIXED_TIMESTAMP = 1775000000000;
const buildMockDataSource = (upQueries, downQueries)=>({
        driver: {
            createSchemaBuilder: ()=>({
...
describe('InstanceCommandGenerationService', ()=>{
    const buildService = async (upQueries = [], downQueries = [])=>{
        ...
        return module.get(_instancecommandgenerationservice.InstanceCommandGenerationService);
    };
    ...
    it('should generate a migration with multiple queries', async ()=>{
        const service = await buildService([
            {
                query: 'CREATE TABLE "core"."task" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "name" varchar NOT NULL)'
            },
            {
                query: 'ALTER TABLE "core"."task" ADD CONSTRAINT "PK_task" PRIMARY KEY ("id")'
            }
        ], [
            {
    ...
    it('should use correct file naming for slow instance commands', async ()=>{
        ...
        expect(result?.fileName).toBe(`${versionSlug}-instance-command-slow-${FIXED_TIMESTAMP}-backfill-data.ts`);
        expect(result?.className).toBe('BackfillDataSlowInstanceCommand');
    });
});

//# sourceMappingURL=instance-command-generation.service.spec.js.map