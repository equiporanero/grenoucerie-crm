...
const encryptLegacySecret = ({ plaintext, appSecret })=>{
    const encryptionKey = (0, _crypto.createHash)('sha256').update(appSecret).digest().slice(0, 32);
    const iv = (0, _crypto.randomBytes)(16);
    const cipher = (0, _crypto.createCipheriv)('aes-256-cbc', encryptionKey, iv);
        ...
        const module = await _testing.Test.createTestingModule({
            providers: [
                _simplesecretencryptionutil.SimpleSecretEncryptionUtil,
                {
                    ...
                    useValue: {
                        generateAppSecret: jest.fn().mockImplementation((_type, purpose)=>`${mockAppSecret}-${purpose}`)
                    }
                }
            ]
        }).compile();
