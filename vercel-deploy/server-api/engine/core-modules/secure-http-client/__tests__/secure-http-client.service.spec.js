...
const _axiosfetch = require("@lifeomic/axios-fetch");
...
const mockBuildAxiosFetch = jest.mocked(_axiosfetch.buildAxiosFetch);
jest.mock('src/engine/core-modules/secure-http-client/utils/resolve-and-validate-hostname.util', ()=>({
        resolveAndValidateHostname: jest.fn()
    }));
const mockResolveAndValidate = jest.mocked(_resolveandvalidatehostnameutil.resolveAndValidateHostname);
...
describe('SecureHttpClientService', ()=>{
    ...
    describe('getValidatedHost', ()=>{
        it('should return the original hostname when safe mode is off', async ()=>{
            const service = new _securehttpclientservice.SecureHttpClientService(createMockConfigService());
            const result = await service.getValidatedHost('https://caldav.icloud.com/principals/');
        ...
        it('should validate and return resolved IP when safe mode is on', async ()=>{
            ...
            const result = await service.getValidatedHost('https://caldav.icloud.com/principals/');
            expect(mockResolveAndValidate).toHaveBeenCalledWith('https://caldav.icloud.com/principals/');
            expect(result).toBe('17.248.239.66');
        });
        it('should throw when hostname resolves to a private IP', async ()=>{
            mockResolveAndValidate.mockRejectedValue(new Error('Connection to internal IP address 192.168.1.1 is not allowed.'));
            const service = new _securehttpclientservice.SecureHttpClientService(createMockConfigService({
                OUTBOUND_HTTP_SAFE_MODE_ENABLED: true
            }));
            await expect(service.getValidatedHost('https://my-local-server.local/')).rejects.toThrow('internal IP address');
        });
    });
    describe('createSsrfSafeFetch', ()=>{
        ...
        it('should wrap an SSRF-protected axios client when safe mode is on', ()=>{
            ...
            const axiosClient = mockBuildAxiosFetch.mock.calls[0][0];
            expect(axiosClient.defaults.httpAgent).toBeInstanceOf(_http.Agent);
            expect(axiosClient.defaults.httpsAgent).toBeInstanceOf(_https.Agent);
        });
    });
