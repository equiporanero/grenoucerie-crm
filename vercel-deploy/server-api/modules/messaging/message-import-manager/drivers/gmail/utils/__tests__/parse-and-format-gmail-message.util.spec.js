...
const _parseandformatgmailmessageutil = require("../parse-and-format-gmail-message.util");
const connectedAccount = {
    handle: 'me@example.com',
    handleAliases: null
};
const buildMessage = (headers, overrides = {})=>({
        id: 'msg-1',
        threadId: 'thread-1',
        historyId: '42',
...
describe('parseAndFormatGmailMessage', ()=>{
    it('should emit one participant per recipient in a multi-address `To` header', ()=>{
        ...
        const result = (0, _parseandformatgmailmessageutil.parseAndFormatGmailMessage)(buildMessage([
            {
                name: 'From',
        ...
        const toHandles = result?.participants.filter((p)=>p.role === 'TO').map((p)=>p.handle);
    ...
    it('should preserve the display name on the FROM participant', ()=>{
        const result = (0, _parseandformatgmailmessageutil.parseAndFormatGmailMessage)(buildMessage([
            {
                name: 'From',
                value: '"Doe, John" <john.doe@example.com>'
            },
            {
                name: 'To',
                value: 'me@example.com'
    ...
    it('should mark messages from the connected account as OUTGOING', ()=>{
        ...
        expect(result?.direction).toBe(_messagedirectionenum.MessageDirection.OUTGOING);
    });
...
//# sourceMappingURL=parse-and-format-gmail-message.util.spec.js.map