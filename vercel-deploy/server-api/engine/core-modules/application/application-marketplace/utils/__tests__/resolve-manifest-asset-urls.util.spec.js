...
const _resolvemanifestasseturlsutil = require("../resolve-manifest-asset-urls.util");
const buildMinimalManifest = (overrides = {})=>({
        application: {
...
describe('resolveManifestAssetUrls', ()=>{
    const urlBuilder = (filePath)=>`https://cdn.example.com/pkg/${filePath}`;
    it('should resolve a relative logoUrl', ()=>{
        const manifest = buildMinimalManifest({
            logoUrl: 'logo.png'
        });
        const result = (0, _resolvemanifestasseturlsutil.resolveManifestAssetUrls)(manifest, urlBuilder);
        expect(result.application.logoUrl).toBe('https://cdn.example.com/pkg/logo.png');
    });
...
//# sourceMappingURL=resolve-manifest-asset-urls.util.spec.js.map